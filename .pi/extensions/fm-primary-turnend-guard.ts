...
// separate session_compact event fires after a compaction.
// "new" is Pi's /clear (a fresh session in the SAME process, so the fleet lock
// is still ours), while reload, resume, and fork all keep prior context.
// bin/fm-sessionstart-run.sh owns what each source means; this maps Pi's
// vocabulary onto its --source names and injects whatever it prints.
...
async function injectSessionstart(pi: ExtensionAPI, source: string): Promise<void> {
  ...
  try {
    ...
    // would have to guess whether it was captain-authored.
    // The wrapper already returns an encoded nudge on a context-preserving
    // open, so only an unencoded digest needs the marker added here.
...
export default function (pi: ExtensionAPI) {
  ...
  // Pi's compaction equivalent.
  // The digest is what a compacted session has just lost, so re-emitting it
