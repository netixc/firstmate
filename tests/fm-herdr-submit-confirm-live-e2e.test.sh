...
#
# Herdr's native agent_status can stay idle for a whole landed Pi turn, and a
# busy-queued Enter can keep proven pending text visible. A stub cannot prove
# either signal. This guard launches exact Pi 0.84.4 in an isolated Herdr lab
...
# quietly.
#
# Run explicitly with FM_HERDR_SUBMIT_CONFIRM_LIVE=1 after a Herdr or Pi
...
[ "$PI_VERSION" = 0.84.4 ] || fail "FM_HERDR_SUBMIT_CONFIRM_LIVE=1 requires exact Pi 0.84.4, found ${PI_VERSION:-unknown}"
...
lab pane run "$PANE" "pi --approve --no-session" >/dev/null \
  || fail "could not launch Pi $PI_VERSION in the isolated Herdr pane"
...
[ "$idle" = 1 ] || fail "Pi $PI_VERSION on $HERDR_VER never registered an idle agent in the lab pane"
...
verdict=$(fm_backend_herdr_send_text_submit "$TARGET" "Reply with exactly $TOKEN and nothing else." 3 0.4 0.4) \
  || fail "send_text_submit failed to run against Pi $PI_VERSION on $HERDR_VER"
CHECKED=1
[ "$verdict" = empty ] \
  || fail "Pi $PI_VERSION on $HERDR_VER: a landed idle steer must confirm empty, got '$verdict'"

# Confirm the instruction reached Pi, not merely that the composer cleared.
# The token occurs once in the submitted prompt and once in Pi's reply.
landed=0
i=0
screen=''
while [ "$i" -lt 45 ]; do
...
[ "$landed" = 1 ] \
  || fail "Pi $PI_VERSION on $HERDR_VER: submit reported '$verdict' but the expected reply never rendered"
pass "live Herdr submit confirm: Pi $PI_VERSION on $HERDR_VER reports empty and renders the requested reply in isolated session $SESSION"
