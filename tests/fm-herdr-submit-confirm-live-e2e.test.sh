...
#
# Herdr's native agent_status can stay idle for a whole landed Codex turn, and
# a busy-queued Enter can keep proven pending text visible. A stub cannot prove
# either signal. This guard launches real Codex Code in an isolated Herdr lab
...
# quietly.
#
# Run explicitly with FM_HERDR_SUBMIT_CONFIRM_LIVE=1 after a Herdr or Codex
...
fm_live_gate opt-in FM_HERDR_SUBMIT_CONFIRM_LIVE herdr jq codex
...
TARGET="$SESSION:$PANE"
VERSION=$(PATH="$ORIGINAL_PATH" codex --version 2>/dev/null | head -1 || printf 'version-unknown')
HERDR_VER=$(PATH="$ORIGINAL_PATH" herdr --version 2>/dev/null | head -1 || printf 'herdr-unknown')

lab pane run "$PANE" "codex --dangerously-skip-permissions --settings '{\"feedbackDrafts\":\"off\"}'" >/dev/null \
  || fail "could not launch Codex Code ($VERSION) in the isolated Herdr pane"
...
[ "$idle" = 1 ] || fail "Codex Code ($VERSION) on $HERDR_VER never registered an idle agent in the lab pane"
...
verdict=$(fm_backend_herdr_send_text_submit "$TARGET" "Reply with exactly $TOKEN and nothing else." 3 0.4 0.4) \
  || fail "send_text_submit failed to run against Codex Code ($VERSION) on $HERDR_VER"
CHECKED=1
[ "$verdict" = empty ] \
  || fail "Codex Code ($VERSION) on $HERDR_VER: a landed idle steer must confirm empty, got '$verdict'"

# Confirm the instruction reached Codex, not merely that the composer cleared.
# The token occurs once in the submitted prompt and once in Codex's reply.
landed=0
i=0
screen=''
while [ "$i" -lt 45 ]; do
...
[ "$landed" = 1 ] \
  || fail "Codex Code ($VERSION) on $HERDR_VER: submit reported '$verdict' but the expected reply never rendered"
pass "live Herdr submit confirm: Codex Code ($VERSION) on $HERDR_VER reports empty and renders the requested reply in isolated session $SESSION"
