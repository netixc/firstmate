...
# this is proven against real plain Pi, launched idle in an isolated tmux
# server and steered through the REAL fm-send (durable record + doorbell).
# Pi must both ACT on the instruction (create a named file) and ACKNOWLEDGE it
# (the mv into handled/), failing loudly with its version.
#
# Run explicitly with FM_SEND_INBOX_LIVE_E2E=1. This test spends a small
# number of real model tokens for one short turn, authorized by the
# harness-dependent-checks rule. An unavailable Pi fails explicitly, and tune
# the wait with FM_SEND_INBOX_LIVE_TIMEOUT (seconds, default 240).
...
check_harness_doorbell() {  # <name>
  local name=$1 version cmd=pi win="hx-$1" home task acted rec handled i ready_rc
...
command -v pi >/dev/null 2>&1 || {
  printf 'not ok - plain Pi is not installed\n' >&2
  exit 1
}
...
if [ "$CHECKED" -eq 0 ]; then
  printf 'not ok - live steering-inbox doorbell guard did not verify plain Pi\n' >&2
  exit 1
fi
pass "live steering-inbox doorbell guard: plain Pi honored the doorbell contract"
