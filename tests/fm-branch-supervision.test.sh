...
test_home_without_branch_is_untouched() {
  ...
  # a no-lock Codex home; the guard removes the leftover and passes silently.
  ...
  [ "$out" = "silent-pass" ] || fail "guard helpers honored a leftover Pi lease in a no-lock Codex home: $out"
  ...
  [ "$out" = "silent-pass" ] || fail "guard helpers honored a reused-pid Pi lease in a Codex context: $out"
  [ ! -e "$home/state/.lease-task-reused" ] || fail "Codex context kept a Pi lease whose old pid matched its current lock"
