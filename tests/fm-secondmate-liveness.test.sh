...
test_tmux_agent_state_classifies() {
  ...
  for harness in codex codex opencode grok kimi pi pi-signed pi-launcher Pi; do
...
test_agent_state_dispatcher_and_compatibility() {
  ...
  fb=$(make_probe_tmux "$TMP_ROOT/dispatch-tmux" codex)
  out=$(PATH="$fb:$BASE_PATH" bash -c '. "$0/bin/fm-backend.sh"; fm_backend_agent_state tmux sess:win' "$ROOT")
...
add_sm_home() {
  local w=$1 id=$2 window=$3 harness=${4:-codex}
  local home="$w/$id"
...
test_sweep_leaves_alive_secondmate_untouched() {
  ...
  out=$(run_bootstrap "$tmuxfb:$fb" "$w/home" codex "$log")
  ...
  out=$(run_bootstrap "$tmuxfb:$fb" "$w/home" codex "$log" FM_BOOTSTRAP_VERBOSE_FACTS=1)
...
test_sweep_converges_no_retouch_once_alive() {
  ...
  : > "$log"
  out2=$(run_bootstrap "$tmuxfb:$fb" "$w/home" codex "$log")
