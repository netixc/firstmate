...
write_herdr_ship_meta() {  # <file> <worktree> <project> [window]
  local file=$1 worktree=$2 project=$3 id window session pane
  id=$(basename "$file" .meta)
  window=${4:-firstmate:w1:p-$id}
  session=${window%%:*}
  pane=${window#*:}
  fm_write_meta "$file" \
    "window=$window" \
...
EOF
    ...
    grep -F 'tab create' "$log" >/dev/null && fail "secondmate spawn created an endpoint before $case_name refusal"
  done
...
SH
  ...
  grep -F 'tab create' "$log" >/dev/null && fail "spawn created an endpoint before validation"
...
test_secondmate_spawn_refuses_operational_dirs_outside_subhome() {
  ...
  for opdir in data state config projects; do
    ...
    grep -F 'tab create' "$log" >/dev/null && fail "spawn created an endpoint before unsafe $opdir directory validation"
  done
...
test_fm_send_refuses_bare_window_without_home_meta() {
  ...
  # foreign same-named endpoint that a runtime listing happens to return.
  ...
  if PATH="$fakebin:$PATH" FM_HOME="$home" FM_FAKE_HERDR_WINDOW="other-session:w1:p-missing" FM_FAKE_HERDR_LOG="$log" FM_FAKE_HERDR_CAPTURE="$TMP_ROOT/send-fake/pane.txt" \
  ...
  grep -F 'p-missing' "$log" >/dev/null \
    && fail "fm-send fell back to a foreign same-name endpoint"
...
test_secondmate_teardown_retires_empty_home() {
  ...
  fm_write_secondmate_meta "$home/state/domain.meta" "$subhome" \
    "firstmate:fm-domain" alpha echo
  ...
  printf 'domain\n' > "$lease"
  local teardown_out
  teardown_out=$(PATH="$fakebin:$PATH" FM_ROOT_OVERRIDE="$fmroot" FM_HOME="$home" FM_FAKE_HERDR_LOG="$log" FM_FAKE_HERDR_CAPTURE="$TMP_ROOT/teardown-fake/pane.txt" \
    FM_FAKE_TREEHOUSE_LEASE_FILE="$lease" \
    "$ROOT/bin/fm-teardown.sh" domain 2>&1) \
    || fail "teardown failed for empty secondmate home: $teardown_out"
...
test_secondmate_force_teardown_discards_child_work() {
  ...
  fm_write_secondmate_meta "$home/state/domain.meta" "$subhome"
  printf '%s\n' '- domain - design domain (home: '"$subhome"'; scope: design domain; projects: alpha; added 2026-06-22)' > "$home/data/secondmates.md"
  write_herdr_ship_meta "$subhome/state/child.meta" "$childwt" "$childproj"
  ...
  grep -F 'pane close w1:p-child --session firstmate' "$log" >/dev/null || fail "force teardown did not close the child pane"
  grep -F 'pane close w1:p-domain --session firstmate' "$log" >/dev/null || fail "force teardown did not close the parent pane"
...
test_secondmate_force_teardown_refuses_child_quarantine_symlink() {
  ...
  grep -F 'pane close' "$log" >/dev/null && fail "force teardown killed a window before child quarantine validation"
...
test_secondmate_force_teardown_allows_operational_dir_symlinks_inside_home() {
  ...
  for opdir in data state config projects; do
    ...
    grep -F 'pane close w1:p-domain --session firstmate' "$log" >/dev/null || fail "force teardown did not close the parent pane for inside $opdir symlink"
  done
...
test_secondmate_force_teardown_refuses_operational_dir_symlink_outside_home() {
  ...
  grep -F 'pane close' "$log" >/dev/null && fail "teardown killed a window before symlinked state refusal"
...
SH
    ...
    grep -F 'pane close' "$log" >/dev/null && fail "teardown ($row) killed a window before validation"
  done <<'ROWS'
...
test_secondmate_teardown_refuses_registered_nested_home() {
  ...
  fm_write_secondmate_meta "$home/state/domain.meta" "$subhome"
  fm_write_secondmate_meta "$home/state/nested.meta" "$nested" "firstmate:fm-nested" beta
  cat > "$home/data/secondmates.md" <<EOF
- domain - design domain (home: $subhome; scope: design domain; projects: alpha; added 2026-06-22)
...
EOF
  ...
  grep -F 'pane close' "$log" >/dev/null && fail "teardown killed a window before nested-home refusal"
...
test_secondmate_teardown_refuses_child_registry_nested_home() {
  ...
  grep -F 'pane close' "$log" >/dev/null && fail "teardown killed a window before child-registry refusal"
...
test_secondmate_force_teardown_prevalidates_before_child_cleanup() {
  ...
  grep -F 'pane close' "$log" >/dev/null && fail "force teardown killed windows before subhome validation"
...
test_secondmate_force_teardown_refuses_child_active_home_descendant() {
  ...
  grep -F 'pane close' "$log" >/dev/null && fail "force teardown killed windows before child validation refusal"
...
SH
  ...
  grep -F 'pane close' "$log" >/dev/null && fail "force teardown killed windows before repo child validation refusal"
...
test_secondmate_force_teardown_refuses_unregistered_child_worktree() {
  ...
  grep -F 'pane close' "$log" >/dev/null && fail "force teardown killed windows before unregistered child refusal"
...
test_secondmate_idle_pane_is_not_stale() {
  ...
  fm_write_secondmate_meta "$home/state/domain.meta" "$TMP_ROOT/watch-subhome" "$window"
  fakebin=$(make_fake_herdr "$TMP_ROOT/watch-fake")
  out="$TMP_ROOT/watch-fake/watch.out"
  PATH="$fakebin:$PATH" FM_HOME="$home" FM_FAKE_HERDR_WINDOW="$window" FM_FAKE_HERDR_LOG="$TMP_ROOT/watch-fake/herdr.log" FM_FAKE_HERDR_CAPTURE="$TMP_ROOT/watch-fake/pane.txt" \
