...
test_fm_send_refuses_bare_window_without_home_meta() {
  ...
  if PATH="$fakebin:$PATH" FM_HOME="$home" FM_FAKE_HERDR_WINDOW="other-session:fm-missing" FM_FAKE_HERDR_LOG="$log" FM_FAKE_HERDR_CAPTURE="$TMP_ROOT/send-fake/pane.txt" \
...
EOF
  ...
  grep -F 'pane close w1:p2' "$log" >/dev/null || fail "force teardown did not kill child window"
  grep -F 'pane close w1:p1' "$log" >/dev/null || fail "force teardown did not kill parent window"
...
EOF
    ...
    grep -F 'pane close w1:p1' "$log" >/dev/null || fail "force teardown did not kill parent window for inside $opdir symlink"
  done
...
EOF
  ...
  # An in-flight item is refused (active ownership lives in Herdr plus durable state).
  before=$(cat "$home/data/backlog.md")
