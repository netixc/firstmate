...
test_home_seed_no_projects_end_to_end() {
  ...
  PATH="$fakebin:$PATH" FM_HOME="$home" FM_FAKE_TMUX_LOG="$log" \
    ...
    "$ROOT/bin/fm-spawn.sh" fdev "$sub" --harness pi --secondmate >/dev/null 2>&1 \
...
test_secondmate_spawn_resolves_punctuated_registry_projects() {
  ...
  PATH="$fakebin:$PATH" FM_HOME="$home" FM_FAKE_TMUX_LOG="$log" \
    ...
    "$ROOT/bin/fm-spawn.sh" punctuated --harness pi --secondmate >/dev/null 2>&1 \
...
EOF
    ...
    if [ "$case_name" = metadata-mismatch ]; then
      ...
      if PATH="$fakebin:$PATH" FM_HOME="$home" FM_FAKE_TMUX_LOG="$log" \
        ...
        "$ROOT/bin/fm-spawn.sh" domain --harness pi --secondmate >/dev/null 2>"$err"; then
      ...
      if PATH="$fakebin:$PATH" FM_HOME="$home" FM_FAKE_TMUX_LOG="$log" \
        FM_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-binding-$case_name-fake/pane.txt" \
        "$ROOT/bin/fm-spawn.sh" domain "$sub" --harness pi --secondmate >/dev/null 2>"$err"; then
        fail "secondmate spawn accepted $case_name registry binding"
      fi
...
SH
  ...
  if PATH="$fakebin:$PATH" FM_HOME="$home" FM_FAKE_TMUX_LOG="$log" FM_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/fm-spawn.sh" domain "$subhome" --harness pi --secondmate >/dev/null 2>"$err"; then
  ...
  if PATH="$fakebin:$PATH" FM_HOME="$home" FM_FAKE_TMUX_LOG="$log" FM_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/fm-spawn.sh" domain "$wronghome" --harness pi --secondmate >/dev/null 2>"$err"; then
  ...
  if PATH="$fakebin:$PATH" FM_HOME="$home" FM_FAKE_TMUX_LOG="$log" FM_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/fm-spawn.sh" domain "$marker_only" --harness pi --secondmate >/dev/null 2>"$err"; then
  ...
  if PATH="$fakebin:$PATH" FM_HOME="$home" FM_FAKE_TMUX_LOG="$log" FM_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/fm-spawn.sh" domain "$home" --harness pi --secondmate >/dev/null 2>"$err"; then
  ...
  if PATH="$fakebin:$PATH" FM_HOME="$home" FM_FAKE_TMUX_LOG="$log" FM_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/fm-spawn.sh" domain "$ROOT" --harness pi --secondmate >/dev/null 2>"$err"; then
  ...
  if PATH="$fakebin:$PATH" FM_HOME="$home" FM_FAKE_TMUX_LOG="$log" FM_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/fm-spawn.sh" domain "$active_descendant" --harness pi --secondmate >/dev/null 2>"$err"; then
  ...
  if PATH="$fakebin:$PATH" FM_HOME="$ancestor_active_home" FM_FAKE_TMUX_LOG="$log" FM_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/fm-spawn.sh" domain "$active_ancestor" --harness pi --secondmate >/dev/null 2>"$err"; then
  ...
  if PATH="$fakebin:$PATH" FM_ROOT_OVERRIDE="$fakeroot" FM_HOME="$home" FM_FAKE_TMUX_LOG="$log" FM_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/fm-spawn.sh" domain "$root_descendant" --harness pi --secondmate >/dev/null 2>"$err"; then
  ...
  if PATH="$fakebin:$PATH" FM_ROOT_OVERRIDE="$root_inside" FM_HOME="$home" FM_FAKE_TMUX_LOG="$log" FM_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/fm-spawn.sh" domain "$root_ancestor" --harness pi --secondmate >/dev/null 2>"$err"; then
