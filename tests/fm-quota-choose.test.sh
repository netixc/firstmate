...
out=$(call_choose --snapshot "$LAB/captured.json" --candidate pi-signed:default)
[ "$out" = "pi-signed default" ] || fail "default scope: expected provider-wide quota, got '$out'"
...
if err=$(call_choose --snapshot "$LAB/captured.json" --candidate pi:default --candidate 'pi-signed:' 2>&1); then
  fail "trailing empty model was hidden by an earlier selection"
fi
[ "$err" = "error: invalid candidate: pi-signed:" ] || fail "trailing empty model returned: $err"
...
if err=$(call_choose --snapshot "$LAB/captured.json" --candidate 'pi-signed:' 2>&1); then
  fail "empty model candidate unexpectedly dispatched"
fi
[ "$err" = "error: invalid candidate: pi-signed:" ] || fail "empty model candidate returned: $err"
ok "empty model candidate fails closed"

# A bare harness with no colon means the default model.
out=$(call_choose --snapshot "$LAB/captured.json" --candidate pi-signed)
[ "$out" = "pi-signed default" ] || fail "bare harness: expected 'pi-signed default', got '$out'"
