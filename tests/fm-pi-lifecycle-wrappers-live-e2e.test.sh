...
# the exact record into handled/. The provider fixture records any call so this
# regression fails deterministically if the abort happens too early or too late.
...
export default function (pi: any) {
  pi.registerProvider("fm-test-offline", {
    ...
    streamSimple() {
      ...
      throw new Error("credential-safe lifecycle fixture reached the provider");
    },
  });
...
! jq -e 'select(.kind == "provider-call")' "$CAPTURE" >/dev/null \
  || fail "credential-safe lifecycle reached the provider"
