...
test_bootstrap_opt_out_cleanup() {
  ...
  # Pin Pi remediation wording while blinding structural ancestry.
  ...
  printf 'FMX_PAIRING_TOKEN=\n' > "$home/.env"
  out=$(PATH="$blind:$PATH" PI_CODING_AGENT=true FM_PI_HARNESS=pi \
  ...
  assert_contains "$out" "Pi tool fm_watch_arm_pi" "opt-out remediation must use Pi's recovery tool"
