...
test_bootstrap_reports_missing_x_dependency() {
  ...
  fakebin=$(fm_fakebin "$home")
  fm_fake_exit0 "$fakebin" legacy-provider node no-mistakes chrome-devtools-axi curl
...
test_bootstrap_opt_out_cleanup() {
  ...
  assert_contains "$out" "repair a missing or failed watcher cycle with the Pi tool fm_watch_arm_pi" "opt-out remediation must use the Pi recovery path"
