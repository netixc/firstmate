...
#   NON-VISIBLE separate terminal (a herdr dedicated workspace, a dedicated Herdr workspace, never a split of the captain's pane. The
#   test runs on a throwaway, NEVER-default HERDR_SESSION and asserts the
#   default session is byte-identical via the fm-herdr-lab.sh fleet-state
#   tripwire. A harmless
...
unit_record_failure_closes_terminal() {
  ...
  FM_HOME="$st" FM_STATE_OVERRIDE="$st/state" CLOSED="$closed" bash -c '
    ...
    ! fm_afk_launch_commit_terminal herdr lab:pane-exact ws-exact
  ' _ "$LAUNCH"
  if [ "$(cat "$closed" 2>/dev/null || true)" = "herdr:lab:pane-exact" ]; then
...
unit_readiness_failure_rolls_back_terminal() {
  ...
  FM_HOME="$st" FM_STATE_OVERRIDE="$st/state" CLOSED="$closed" bash -c '
    ...
    ! fm_afk_launch_commit_terminal herdr lab:pane-exact ws-exact
  ' _ "$LAUNCH"
  if [ "$(cat "$closed" 2>/dev/null || true)" = "herdr:lab:pane-exact" ] \
...
unit_close_failure_preserves_record() {
  ...
  mkdir -p "$st/state"
  printf 'herdr\tlab:pane-exact\tws-exact\n' > "$st/state/.afk-daemon-terminal"
  FM_HOME="$st" FM_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    fm_afk_launch_close_terminal() { return 1; }
    fm_afk_launch_terminal_absent() { return 1; }
...
unit_record_publication_atomic() {
  ...
  mkdir -p "$st/state"
  printf 'herdr\tlab:pane-old\tws-old\n' > "$st/state/.afk-daemon-terminal"
  if FM_HOME="$st" FM_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    mv() { return 1; }
    ! fm_afk_launch_record_write herdr lab:pane-new ws-new
  ' _ "$LAUNCH" \
    && [ "$(cat "$st/state/.afk-daemon-terminal")" = $'herdr\tlab:pane-old\tws-old' ] \
