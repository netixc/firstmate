...
test_harness_switch_resolves_a_prefixed_recorded_harness() {
  ...
  add_ship_task "$dir" rl32 codex-2
  printf 'codex-2' > "$dir/fake/command"
  ...
  [ "$(journal_field "$dir" rl32 from_harness)" = codex-2 ] \
    || fail "relaunch should retain the recorded harness basename in its provenance"
  assert_contains "$out" "harness=pi from=codex-2" \
...
test_prefixed_recorded_harness_requires_explicit_replacement() {
  ...
  add_ship_task "$dir" rl34 codex-2
  printf 'codex-2' > "$dir/fake/command"
...
# `codex-2` would have its turn-end token and hook pointer armed and never
