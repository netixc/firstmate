#!/usr/bin/env bash
# tests/fm-herdr-respawn-idem-e2e.test.sh - isolated real-herdr
...
# agent-less shell in the saved cwd - and bin/fm-herdr.sh's
# fm_herdr_create_task refused to spawn into it unconditionally,
...
# fm_herdr_create_task now closes-and-replaces the resulting husk
...
# before. Adapter-level (fm_herdr_container_ensure/create_task), not
# through the full bin/fm-spawn.sh + treehouse pipeline - mirrors
# tests/fm-herdr-prune-safety-e2e.test.sh's own style, and avoids any
...
# fm_herdr_create_task is the ONE function both bin/fm-spawn.sh's
...
# only in FM_HOME-shadowing (tests/fm-herdr-workspace-per-home-e2e.test.sh),
# never in this duplicate-guard logic, which has no home-specific branching.

PROJ_CWD="$SCRATCH/proj"
mkdir -p "$PROJ_CWD"

RAW=$(fm_herdr_container_ensure "$PROJ_CWD") || fail "container_ensure failed"
CONTAINER=${RAW%%$'\t'*}
SEEDED_TAB_ID=${RAW#*$'\t'}
WSID=${CONTAINER#*:}

CREW_LABEL="fm-respawn-crew1"
CREW_IDS=$(fm_herdr_create_task "$CONTAINER" "$CREW_LABEL" "$PROJ_CWD" "$SEEDED_TAB_ID") \
...
SM_IDS=$(fm_herdr_create_task "$CONTAINER" "$SM_LABEL" "$PROJ_CWD") \
...
sleep 0.5
fm_herdr_server_ensure "$SESSION" || fail "the isolated session's server did not come back up after the restart"
...
RESPAWN_CREW_IDS=$(fm_herdr_create_task "$CONTAINER" "$CREW_LABEL" "$PROJ_CWD") \
...
RESPAWN_SM_IDS=$(fm_herdr_create_task "$CONTAINER" "$SM_LABEL" "$PROJ_CWD") \
...
if fm_herdr_create_task "$CONTAINER" "$CREW_LABEL" "$PROJ_CWD" >/dev/null 2>&1; then
...
fm_herdr_kill "$SESSION:$NEW_CREW_PANE_ID"
fm_herdr_kill "$SESSION:$NEW_SM_PANE_ID"
