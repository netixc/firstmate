...
assert_absent "$CASE_HOME/.local/bin/pi" "--fix wrapped Pi when it already satisfied readiness"

rm -f "$CASE_BIN/pi"
doctor --fix
expect_code 0 "$DOCTOR_RC" "--fix did not wrap discoverable Pi when it was not on PATH"
