...
# not by inspecting the wrapper's source.
# docs/sessionstart-nudge.md owns the tier assignment and the source table these
# pin.
set -u
...
# ancestor.
# This also prevents a developer's ambient harness from making the portable
# regression pass locally while failing on a harness-free CI runner.
...
# bin/fm-session-start.sh can execute: a git repo on main so the tangle check
# behaves, plus the home directories the digest reads.
# The deliberately bare PATH keeps every bootstrap probe fast and hermetic - it
# reports missing tools instead of reaching the host's real tooling.
...
test_pi_large_sessionstart_digest_is_delivered_loudly() {
  ...
  cp "$ROOT/bin/fm-sessionstart-run.sh" "$ROOT/bin/fm-sessionstart-nudge.sh" \
    ...
    "$ROOT/bin/fm-session-lock-lib.sh" "$ROOT/bin/fm-operational-input.sh" "$fixture/bin/"
