...
printf '#!/usr/bin/env bash\nexit 0\n' > "$DOCTOR_BIN/codex"
chmod +x "$DOCTOR_BIN/jq" "$DOCTOR_BIN/herdr" "$DOCTOR_BIN/tasks-axi" "$DOCTOR_BIN/treehouse" "$DOCTOR_BIN/codex"
set +e
out=$(HOME="$DOCTOR_HOME" PATH="$DOCTOR_BIN:/usr/bin:/bin:/usr/sbin:/sbin" "$ROOT/bin/fm-remote-doctor.sh" 2>&1)
rc=$?
set -e
...
assert_contains "$out" "required harness=codex:$DOCTOR_BIN/codex" "the remote doctor did not require a verified harness"
