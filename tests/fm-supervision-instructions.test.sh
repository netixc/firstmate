...
test_selected_harness_block_only() {
  ...
  assert_contains "$out" "SUPERVISION OPERATING INSTRUCTIONS - primary harness: opencode" "opencode heading missing"
  assert_contains "$out" "Mode: OpenCode TUI plugin background wake." "opencode snippet missing"
...
test_conditional_stanzas() {
  ...
  out=$(FM_HOME="$home" FM_CONFIG_OVERRIDE="$config" "$RENDER" --harness opencode --read-only 1 --afk 1 --x-mode 1)
...
test_quiet_mode_stanzas() {
  ...
  out=$(FM_HOME="$home" FM_CONFIG_OVERRIDE="$config" "$RENDER" --harness opencode --afk 1 --afk-mode quiet)
  ...
  out=$(FM_HOME="$home" "$RENDER" --harness opencode --afk 1 --afk-mode quiet --repair-line)
  assert_contains "$out" "Quiet mode owns watcher supervision; load /quiet" "quiet repair line did not name /quiet"

  out=$(FM_HOME="$home" "$RENDER" --harness opencode --afk 1)
  ...
  out=$(FM_HOME="$home" "$RENDER" --harness opencode --afk 1 --afk-mode not-a-real-mode)
  assert_contains "$out" "- Away mode: active" "unrecognized --afk-mode value did not fall back to away"

  out=$(FM_HOME="$home" "$RENDER" --harness opencode --afk 0)
...
test_repair_lines() {
  ...
  out=$(FM_HOME="$home" "$RENDER" --harness opencode --queue-pending 1 --repair-line)
  assert_contains "$out" "After draining queued wakes" "queue-pending prefix missing"
  assert_contains "$out" "manual recovery probe" "queue-pending OpenCode repair lost its recovery probe"

  : > "$home/config/x-mode.env"
  out=$(FM_HOME="$home" "$RENDER" --harness opencode --x-mode 1 --repair-line)
  assert_contains "$out" "source '$home/config/x-mode.env' first" "x-mode repair line did not source the effective cadence config"
  assert_contains "$out" "manual recovery probe" "x-mode OpenCode repair lost its recovery probe"
