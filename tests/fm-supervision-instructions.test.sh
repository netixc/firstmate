...
test_selected_harness_block_only() {
  ...
  assert_not_contains "$out" "Mode: Codex Stop-hook-owned supervision." "renderer printed the codex snippet too"
...
test_repair_lines() {
  ...
  out=$(FM_HOME="$home" "$RENDER" --harness codex --queue-pending 1 --repair-line)
  assert_contains "$out" "After draining queued wakes" "queue-pending prefix missing"
  assert_contains "$out" "foreground checkpoint" "queue-pending Codex repair lost its checkpoint"
...
test_grok_is_background_notify() {
  ...
  pass "grok supervision is Codex-shaped background notify with passive Stop-hook backstop"
}
