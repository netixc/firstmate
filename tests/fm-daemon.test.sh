...
test_stale_diagnostic_wedge_survives_busy_housekeeping() {
  ...
  for case_name in working prior-terminal paused; do
    ...
    fm_write_meta "$state/$task.meta" "window=$win" "backend=herdr"
    case "$case_name" in
    ...
    (
      ...
      PATH="$fakebin:$PATH" FM_FAKE_HERDR_WINDOW="$win" FM_FAKE_HERDR_CAPTURE="$pane" \
...
test_housekeeping_paused_resurfaces_and_resets() {
  ...
  fm_write_meta "$state/held-w11.meta" "window=$win" "backend=herdr"
  printf 'idle prompt $\n' > "$pane"
...
test_housekeeping_paused_resumed_cleared() {
  ...
  printf 'Working...\n' > "$pane"
  fm_write_meta "$state/held-w12.meta" \
...
test_housekeeping_paused_unpaused_cleared() {
  ...
  fm_write_meta "$state/held-w13.meta" "window=$win" "backend=herdr"
  printf 'idle prompt $\n' > "$pane"
...
test_housekeeping_stale_marker_transitions_to_pause() {
  ...
  fm_write_meta "$state/held-w14.meta" "window=$win" "backend=herdr"
  printf 'idle prompt $\n' > "$pane"
...
test_housekeeping_pause_marker_transitions_to_clear() {
  ...
  fm_write_meta "$state/held-w15.meta" "window=$win" "backend=herdr"
  printf 'idle prompt $\n' > "$pane"
...
test_housekeeping_persistent_stale_escalates() {
  ...
  fm_write_meta "$state/pers-w5.meta" "window=$win" "backend=herdr"
  printf 'idle prompt $\n' > "$pane"
...
test_housekeeping_resumed_stale_cleared() {
  ...
  fm_write_meta "$state/res-w6.meta" \
    "window=$win" "backend=herdr" "worktree=$dir/wt" "kind=ship" "harness=pi"
...
test_afk_absent_daemon_does_not_inject() {
  ...
  if PATH="$fakebin:$PATH" FM_FAKE_HERDR_PANE_ALIVE=1 FM_FAKE_HERDR_SENT="$sent" \
...
test_pane_input_pending_blank_is_not_pending() {
  ...
  fakebin="$dir/fakebin"
  capture="$dir/pane.txt"
  # A blank row is safe only inside a structurally proven composer box.
  printf 'some output\n╭─────╮\n│ >   │\n╰─────╯\n' > "$capture"
  PATH="$fakebin:$PATH" FM_FAKE_HERDR_CAPTURE="$capture" \
    pane_input_pending "default:w1:p1" \
    && fail "blank bordered composer falsely detected as pending"
  pass "pane_input_pending: blank bordered composer is not pending"
}

test_pane_input_pending_requires_proven_empty_prompt() {
  local dir fakebin capture prompt
...
test_herdr_composer_state_bare_shell_is_unknown() {
  ...
  pass "Herdr composer state treats a bare shell prompt as unknown"
}

# A bordered composer box and bare agent prompt glyphs remain genuine empty
# agent composers.
...
test_herdr_composer_state_requires_matching_box_borders() {
  ...
  pass "Herdr composer state requires matching box borders"
}
...
test_pane_input_pending_honors_idle_override_after_border_strip() {
  ...
  printf '╭────────────────╮\n│ custom idle>   │\n╰────────────────╯\n' > "$capture"
  PATH="$fakebin:$PATH" FM_FAKE_HERDR_CAPTURE="$capture" \
    FM_BACKEND_HERDR_IDLE_RE='^custom idle>$' pane_input_pending "default:w1:p1" \
    && fail "the Herdr idle override was not applied after border stripping"
  pass "pane_input_pending honors the Herdr idle override after border stripping"
}
...
test_afk_nonterminal_working_merged_keeps_wedge_aging() {
  ...
  fm_write_meta "$state/wishlist-w1.meta" "window=$win" "backend=herdr"
  printf 'idle prompt $\n' > "$pane"
...
test_submit_ack_confirms_on_bordered_empty_composer() {
  ...
  verdict=$(PATH="$fakebin:$PATH" FM_FAKE_HERDR_CAPTURE="$dir/composer" FM_FAKE_HERDR_SENT="$sent" \
    fm_backend_herdr_send_text_submit "default:w1:p1" "the digest" 3 0.05 0.05)
...
test_submit_ack_reports_pending_on_persistent_swallow() {
  ...
  touch "$dir/.swallow"
  verdict=$(PATH="$fakebin:$PATH" FM_FAKE_HERDR_CAPTURE="$dir/composer" FM_FAKE_HERDR_SENT="$sent" \
    FM_FAKE_HERDR_SWALLOW_FILE="$dir/.swallow" FM_FAKE_PERSIST_SWALLOW=1 \
    fm_backend_herdr_send_text_submit "default:w1:p1" "the digest" 3 0.05 0.05)
...
test_max_defer_empty_swallow_types_once_and_alarms() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" FM_FAKE_HERDR_CAPTURE="$dir/composer" FM_FAKE_HERDR_SENT="$sent" \
    FM_FAKE_HERDR_SWALLOW_FILE="$dir/.swallow" FM_FAKE_PERSIST_SWALLOW=1 FM_INJECT_CONFIRM_SLEEP=0.05 \
...
test_wedge_alarm_off_disables_active_alert_regardless_of_position() {
  ...
  pass "off disables every active alert regardless of directive position (marker and herdr flash are unaffected)"
}
...
test_fm_send_exits_nonzero_on_confirmed_swallow() {
  ...
  # Clean submit -> exit 0.
  PATH="$fakebin:$PATH" FM_HOME="$dir" FM_STATE_OVERRIDE="$dir/state" FM_FAKE_HERDR_CAPTURE="$dir/composer" \
  ...
  printf '╭─────╮\n│ >   │\n╰─────╯\n' > "$dir/composer"
  touch "$dir/.swallow"
  if PATH="$fakebin:$PATH" FM_HOME="$dir" FM_STATE_OVERRIDE="$dir/state" FM_FAKE_HERDR_CAPTURE="$dir/composer" \
    FM_FAKE_HERDR_SWALLOW_FILE="$dir/.swallow" FM_FAKE_PERSIST_SWALLOW=1 FM_SEND_SLEEP=0.05 \
...
test_fm_send_exits_nonzero_on_pending_submit() {
  ...
  if PATH="$fakebin:$PATH" FM_HOME="$dir" FM_STATE_OVERRIDE="$dir/state" FM_FAKE_HERDR_CAPTURE="$dir/composer" \
    ...
    fail "fm-send exited zero when submit proof remained pending"
  fi
  grep -F 'verdict=pending' "$err" >/dev/null \
    || fail "fm-send did not preserve the pending-submit verdict: $(cat "$err")"
...
test_discover_supervisor_target_herdr() {
  ...
  if out=$(FM_SUPERVISOR_TARGET='' HERDR_ENV='' HERDR_PANE_ID='' discover_supervisor_target); then
    fail "missing endpoint identity should return non-zero"
  fi
  [ -z "$out" ] || fail "missing endpoint identity should not invent a target: $out"

  pass "discover_supervisor_target: explicit override or exact Herdr endpoint identity only"
}
