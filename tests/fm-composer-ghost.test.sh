...
#
# A harness fills an otherwise-empty composer with dim or faint ghost text that
...
test_titled_box_bottom_is_unknown() {
  local dir fb capture out
  dir="$TMP_ROOT/titled-bottom"; mkdir -p "$dir"
  fb=$(make_fake_tmux "$dir")
  capture="$dir/styled.txt"
  printf '╭────────────────────────╮\n│ >                      │\n╰──────── Model ─────────╯\n' > "$capture"
  out=$(PATH="$fb:$PATH" FM_FAKE_STYLED="$capture" FM_FAKE_CY=2 \
    fm_tmux_composer_state "fakepane")
  [ "$out" = unknown ] \
    || fail "a titled bottom no longer proves a composer box, got '$out'"
  pass "fm_tmux_composer_state: a titled box bottom remains unknown"
}
