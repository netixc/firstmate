...
assert_eq 'idle pi-ext' "$(fm_busy_classify legacy-provider target pi task "$TMP/state")" "Pi semantic state is accepted"
assert_eq 'unknown unsupported-harness' "$(fm_busy_classify legacy-provider target pi-signed task "$TMP/state")" "old metadata is not normalized"
