...
# A home that configured nothing follows the version floor: it is projected on
# a release at or above it and takes the ordinary flat layout with one naming
# warning below it. The only difference from the opted-out spawn above is the
# removed file, so this case is the floor's live end-user proof on the lab's
# actual Herdr release.
