...
test_provisions_the_pin_when_shellcheck_is_absent() {
  # Local adaptation: a runner with no ShellCheck on PATH must still lint with
  # the pinned build rather than skipping the CI rule set. The installer itself
  # is covered above, so this pins only that fm-lint.sh invokes it by path.
  local tmp fakebin out rc
  tmp=$(fm_test_tmproot fm-lint-provision)
  fakebin=$(fm_fakebin "$tmp")
  # An empty fake PATH entry plus a stub installer proves the absent-ShellCheck
  # branch runs without downloading anything during the test.
  cat > "$fakebin/fm-install-shellcheck.sh" <<'SH'
#!/usr/bin/env bash
printf 'stub installer refused\n' >&2
exit 1
SH
  chmod +x "$fakebin/fm-install-shellcheck.sh"
  # shellcheck disable=SC2016 # Literal script bytes are the assertion, not an expansion.
  assert_grep 'bin/fm-install-shellcheck.sh" "$LOCAL_SHELLCHECK_DIR"' "$LINT" \
    "fm-lint.sh must provision the pinned build when ShellCheck is absent from PATH"
  # shellcheck disable=SC2016 # Literal script bytes are the assertion, not an expansion.
  assert_grep 'SHELLCHECK_BIN="$LOCAL_SHELLCHECK_DIR/shellcheck"' "$LINT" \
    "fm-lint.sh must run the provisioned build by path rather than relying on PATH"
  rc=0
  out=$(PATH="$fakebin" "$LINT" 2>&1) || rc=$?
  [ "$rc" -ne 0 ] || fail "fm-lint.sh linted without any resolvable ShellCheck"$'\n'"$out"
  pass "fm-lint.sh provisions and runs the pinned ShellCheck when PATH has none"
}
...
test_source_graph_boundaries_keep_every_owner() {
  ...
  # Herdr is this fork's only session provider, so the dispatcher has exactly one
  # adapter import to stop static source following at.
  [ "$(grep -Fc '# shellcheck source=/dev/null' "$ROOT/bin/fm-backend.sh")" -eq 1 ] \
    || fail "the dispatcher must stop static source following at its adapter import"
  assert_present "$ROOT/bin/backends/herdr.sh" "canonical adapter root is missing: herdr"
