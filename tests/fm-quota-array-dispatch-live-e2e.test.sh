...
run_case \
  ...
  "TOON" \
  "Resolve this matched dispatch profile array now. Load quota-array-dispatch and run quota-axi with no flags (default TOON) exactly once. Do not pass --json. Both profiles have comparable required task fit and the same strongest reasoning class. The authoritative catalogs already prove Pi with Anthropic/Claude Sonnet and Pi with OpenAI/GPT models supported in their stated provider families, and their selected authentication surfaces are usable. The likely task-completion horizon is two hours with established confidence. Both candidates have known runway that supports that horizon. Return exact lines FACT=pi|headroom=80|spendPriority=-1.1111|runway_seconds=241920 and FACT=openai-profile|headroom=20|spendPriority=-0.8333|runway_seconds=90720 to preserve candidate accounting, then an exact final line SELECTED=<pi|openai-profile>. Do not use other vendor or model commands and do not modify files." \
  "FACT=pi|headroom=80|spendPriority=-1.1111|runway_seconds=241920" \
  "FACT=openai-profile|headroom=20|spendPriority=-0.8333|runway_seconds=90720"
...
run_case \
  ...
  "DECISION=OPENAI_PROFILE" \
  "TOON
JSON" \
  "Resolve this matched dispatch profile array now. Load quota-array-dispatch and consult quota-axi's default TOON first. Because the Pi/Anthropic candidate spendPriority is the literal unknown, use the permitted quota-axi --json fallback once before deciding. Both profiles have comparable required task fit and the same strongest reasoning class. The authoritative catalogs already prove both models supported in their stated provider families, and their selected authentication surfaces are usable. The likely task-completion horizon is two hours with established confidence. The Pi/Anthropic candidate has higher known headroom but explicitly unmeasurable runway and unknown spendPriority, while The Pi/OpenAI profile has lower known headroom, known spendPriority, and established runway that supports completion. The Pi/Anthropic candidate remains eligible and its uncertainty must be disclosed. Never read unknown spendPriority as 0. Return exact lines FACT=pi|eligible=yes|headroom=55|runway=unknown|spendPriority=unknown|unmeasurable=weekly and FACT=openai-profile|eligible=yes|headroom=45|spendPriority=-0.404|runway_seconds=222676|supports_horizon=yes, then an exact final line DECISION=OPENAI_PROFILE. Do not use other vendor or model commands and do not modify files." \
  "FACT=pi|eligible=yes|headroom=55|runway=unknown|spendPriority=unknown|unmeasurable=weekly" \
  "FACT=openai-profile|eligible=yes|headroom=45|spendPriority=-0.404|runway_seconds=222676|supports_horizon=yes"
...
run_case \
  ...
  "SELECTED=openai-profile" \
  "TOON" \
  "Resolve this matched dispatch profile array now. Load quota-array-dispatch and run quota-axi with no flags (default TOON) exactly once. Do not pass --json. Both profiles have comparable required task fit and the same strongest reasoning class. The authoritative catalogs already prove Pi with Anthropic/Claude Sonnet and Pi with OpenAI/GPT models supported in their stated provider families, and their selected authentication surfaces are usable. The likely task-completion horizon is two hours with established confidence. The Pi/Anthropic candidate has known spendPriority of -0.8333 and runway of 2700 seconds. The Pi/OpenAI profile has known spendPriority of -1.8 and runway of 15916 seconds. Return exact lines FACT=pi|spendPriority=-0.8333|runway_seconds=2700|supports_horizon=no and FACT=openai-profile|spendPriority=-1.8|runway_seconds=15916|supports_horizon=yes to preserve candidate accounting, then an exact final line SELECTED=<pi|openai-profile>. Do not use other vendor or model commands and do not modify files." \
  "FACT=pi|spendPriority=-0.8333|runway_seconds=2700|supports_horizon=no" \
  "FACT=openai-profile|spendPriority=-1.8|runway_seconds=15916|supports_horizon=yes"
