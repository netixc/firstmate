...

{
  ...
  printf '%s\n' \
    ...
    'model-effort.configured-profile pi' \
    'verify.default pi'
...
pass "local model $MODEL selected every operation scenario and the Pi-only runtime map"
...
if binary=$(resolve_native_binary "$harness"); then
  ...
  printf '# native loader not claimed: %s %s is installed, but this Pi evaluation does not exercise provider transport\n' "$harness" "$version"
  CHECKED=$((CHECKED + 1))
else
