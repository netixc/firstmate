...
test_composer_state_borderless_pi_prompt_is_empty() {
  local dir fb out
  dir="$TMP_ROOT/composer-borderless-pi"; mkdir -p "$dir/responses"
  cmux_panes_response "$dir" 1 "bbbbbbbb-1111-1111-1111-111111111111"
  cmux_read_screen_response "$dir" 2 $'────────────────────────\n❯\n────────────────────────\nPi 0.84.4'
  fb=$(make_cmux_fakebin "$dir")
  out=$( PATH="$fb:$PATH" FM_CMUX_LOG="$dir/log" FM_CMUX_RESPONSES="$dir/responses" \
    bash -c '. "$0/bin/backends/cmux.sh"; fm_backend_cmux_composer_state "aaaaaaaa-0000-0000-0000-000000000000:bbbbbbbb-1111-1111-1111-111111111111"' "$ROOT" )
  [ "$out" = empty ] || fail "a borderless Pi '❯' row bounded by horizontal rules should read empty, got '$out'"
  pass "fm_backend_cmux_composer_state: a borderless Pi '❯' composer row reads empty"
}

test_composer_state_borderless_pi_prompt_outranks_stale_bordered_row() {
  local dir fb out
  dir="$TMP_ROOT/composer-borderless-pi-after-bordered"; mkdir -p "$dir/responses"
  cmux_panes_response "$dir" 1 "bbbbbbbb-1111-1111-1111-111111111111"
  cmux_read_screen_response "$dir" 2 $'│ ❯ stale input │\n────────────────────────\n❯\n────────────────────────\nPi 0.84.4'
  fb=$(make_cmux_fakebin "$dir")
  out=$( PATH="$fb:$PATH" FM_CMUX_LOG="$dir/log" FM_CMUX_RESPONSES="$dir/responses" \
    bash -c '. "$0/bin/backends/cmux.sh"; fm_backend_cmux_composer_state "aaaaaaaa-0000-0000-0000-000000000000:bbbbbbbb-1111-1111-1111-111111111111"' "$ROOT" )
  [ "$out" = empty ] || fail "a current borderless Pi row should outrank stale bordered scrollback, got '$out'"
  pass "fm_backend_cmux_composer_state: a borderless Pi row outranks stale bordered scrollback"
}

test_composer_state_borderless_pi_nbsp_prompt_is_empty() {
  local dir fb out
  dir="$TMP_ROOT/composer-borderless-pi-nbsp"; mkdir -p "$dir/responses"
  cmux_panes_response "$dir" 1 "bbbbbbbb-1111-1111-1111-111111111111"
  cmux_read_screen_response "$dir" 2 $'────────────────────────\n❯\302\240\n────────────────────────\nPi 0.84.4'
  fb=$(make_cmux_fakebin "$dir")
  out=$( LC_ALL=C PATH="$fb:$PATH" FM_CMUX_LOG="$dir/log" FM_CMUX_RESPONSES="$dir/responses" \
    bash -c '. "$0/bin/backends/cmux.sh"; fm_backend_cmux_composer_state "aaaaaaaa-0000-0000-0000-000000000000:bbbbbbbb-1111-1111-1111-111111111111"' "$ROOT" )
  [ "$out" = empty ] || fail "a borderless Pi '❯'+NBSP row bounded by horizontal rules should read empty under LC_ALL=C, got '$out'"
  pass "fm_backend_cmux_composer_state: a borderless Pi '❯'+NBSP composer row reads empty under LC_ALL=C"
}

test_composer_state_borderless_pi_text_is_unknown_plain() {
  ...
  # it may be Pi's own idle suggestion, which a plain read cannot distinguish
  # from typed input. The verdict is `unknown` (defer, loud refusal at fm-send),
  # never a false `pending` that would misreport an idle pane as unsent text.
  ...
  local dir fb out
  dir="$TMP_ROOT/composer-borderless-pi-text"; mkdir -p "$dir/responses"
  cmux_panes_response "$dir" 1 "bbbbbbbb-1111-1111-1111-111111111111"
  cmux_read_screen_response "$dir" 2 $'────────────────────────\n❯ retain this message\n────────────────────────\nPi 0.84.4'
  fb=$(make_cmux_fakebin "$dir")
  out=$( PATH="$fb:$PATH" FM_CMUX_LOG="$dir/log" FM_CMUX_RESPONSES="$dir/responses" \
