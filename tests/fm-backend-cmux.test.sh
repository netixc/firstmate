...
test_composer_state_borderless_omp_prompt_is_empty() {
  local dir fb out
  dir="$TMP_ROOT/composer-borderless-omp"; mkdir -p "$dir/responses"
  cmux_panes_response "$dir" 1 "bbbbbbbb-1111-1111-1111-111111111111"
  cmux_read_screen_response "$dir" 2 $'────────────────────────\n❯\n────────────────────────\nHaiku 4.5'
  fb=$(make_cmux_fakebin "$dir")
  out=$( PATH="$fb:$PATH" FM_CMUX_LOG="$dir/log" FM_CMUX_RESPONSES="$dir/responses" \
    bash -c '. "$0/bin/backends/cmux.sh"; fm_backend_cmux_composer_state "aaaaaaaa-0000-0000-0000-000000000000:bbbbbbbb-1111-1111-1111-111111111111"' "$ROOT" )
  [ "$out" = empty ] || fail "a borderless omp '❯' row bounded by horizontal rules should read empty, got '$out'"
  pass "fm_backend_cmux_composer_state: a borderless omp '❯' composer row reads empty"
}

test_composer_state_borderless_omp_prompt_outranks_stale_bordered_row() {
  local dir fb out
  dir="$TMP_ROOT/composer-borderless-omp-after-bordered"; mkdir -p "$dir/responses"
  cmux_panes_response "$dir" 1 "bbbbbbbb-1111-1111-1111-111111111111"
  cmux_read_screen_response "$dir" 2 $'│ ❯ stale input │\n────────────────────────\n❯\n────────────────────────\nHaiku 4.5'
  fb=$(make_cmux_fakebin "$dir")
  out=$( PATH="$fb:$PATH" FM_CMUX_LOG="$dir/log" FM_CMUX_RESPONSES="$dir/responses" \
    bash -c '. "$0/bin/backends/cmux.sh"; fm_backend_cmux_composer_state "aaaaaaaa-0000-0000-0000-000000000000:bbbbbbbb-1111-1111-1111-111111111111"' "$ROOT" )
  [ "$out" = empty ] || fail "a current borderless omp row should outrank stale bordered scrollback, got '$out'"
  pass "fm_backend_cmux_composer_state: a borderless omp row outranks stale bordered scrollback"
}

test_composer_state_borderless_omp_nbsp_prompt_is_empty() {
  local dir fb out
  dir="$TMP_ROOT/composer-borderless-omp-nbsp"; mkdir -p "$dir/responses"
  cmux_panes_response "$dir" 1 "bbbbbbbb-1111-1111-1111-111111111111"
  cmux_read_screen_response "$dir" 2 $'────────────────────────\n❯\302\240\n────────────────────────\nHaiku 4.5'
  fb=$(make_cmux_fakebin "$dir")
  out=$( LC_ALL=C PATH="$fb:$PATH" FM_CMUX_LOG="$dir/log" FM_CMUX_RESPONSES="$dir/responses" \
    bash -c '. "$0/bin/backends/cmux.sh"; fm_backend_cmux_composer_state "aaaaaaaa-0000-0000-0000-000000000000:bbbbbbbb-1111-1111-1111-111111111111"' "$ROOT" )
  [ "$out" = empty ] || fail "a borderless omp '❯'+NBSP row bounded by horizontal rules should read empty under LC_ALL=C, got '$out'"
  pass "fm_backend_cmux_composer_state: a borderless omp '❯'+NBSP composer row reads empty under LC_ALL=C"
}

test_composer_state_borderless_omp_text_is_unknown_plain() {
  ...
  # it may be the harness's own idle suggestion (omp's rotating dim hint,
  ...
  local dir fb out
  dir="$TMP_ROOT/composer-borderless-omp-text"; mkdir -p "$dir/responses"
  cmux_panes_response "$dir" 1 "bbbbbbbb-1111-1111-1111-111111111111"
  cmux_read_screen_response "$dir" 2 $'────────────────────────\n❯ retain this message\n────────────────────────\nHaiku 4.5'
  fb=$(make_cmux_fakebin "$dir")
  out=$( PATH="$fb:$PATH" FM_CMUX_LOG="$dir/log" FM_CMUX_RESPONSES="$dir/responses" \
