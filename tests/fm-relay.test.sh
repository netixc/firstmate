#!/usr/bin/env bash
# Behavior tests for Relay: the relay poll client (fm-relay-poll.sh), the answer
# poster (fm-relay-reply.sh), and bootstrap's .env-presence activation.
#
# Relay must be INERT by default (no token -> the poll is a hard no-op and
...
TMP_ROOT=$(fm_test_tmproot fm-relay-mode-tests)
...
assert_no_private_artifact_temps() {
  ...
  [ -d "$dir" ] || return 0
  leftovers=$(find "$dir" -name '*.fm-relay.*' -print 2>/dev/null)
...
test_poll_no_token_is_hard_noop() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FMX_PAIRING_TOKEN='' \
    "$ROOT/bin/fm-relay-poll.sh"); rc=$?
  expect_code 0 "$rc" "poll no-token exit"
  [ -z "$out" ] || fail "poll no-token must be silent (got: $out)"
  assert_absent "$home/state/relay-inbox" "poll no-token must not create an inbox"
  pass "fm-relay-poll is a hard no-op without a token (inert default)"
}
...
test_poll_empty_env_token_overrides_env_file() {
  ...
  [ -z "$out" ] || fail "empty env token must disable Relay despite .env token (got: $out)"
  [ ! -f "$log" ] || fail "empty env token must not call the relay"
  assert_absent "$home/state/relay-inbox" "empty env token must not create an inbox"
  pass "fm-relay-poll treats an explicitly empty env token as configured"
}
...
test_poll_204_is_silent() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    FAKE_CURL_LOG="$log" FAKE_POLL_CODE=204 \
    "$ROOT/bin/fm-relay-poll.sh"); rc=$?
  ...
  ls "$home/state/relay-inbox/"*.json >/dev/null 2>&1 && fail "poll 204 must not stash an inbox file"
  pass "fm-relay-poll stays silent on HTTP 204 (the common case)"
}
...
test_poll_empty_env_relay_overrides_env_file() {
  ...
  fakebin=$(make_fake_curl "$home")
  log="$home/curl.log"
  printf 'FMX_PAIRING_TOKEN=tok-relay\nFM_RELAY_URL=https://dotenv-relay.test/\n' > "$home/.env"
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL='' \
    FAKE_CURL_LOG="$log" FAKE_POLL_CODE=204 \
    "$ROOT/bin/fm-relay-poll.sh"); rc=$?
  ...
  pass "fm-relay-poll lets an explicitly empty relay env override .env"
}
...
test_poll_auth_error_reports_once() {
  ...
  [ "$out" = "relay-error relay returned HTTP 401" ] \
    || fail "poll auth error must emit one visible diagnostic (got: $out)"
  assert_present "$home/state/relay-poll.error" "poll auth error must write a dedupe marker"
  [ "$(path_mode "$home/state")" = 700 ] || fail "poll auth error must create private state"
  [ "$(path_mode "$home/state/relay-poll.error")" = 600 ] || fail "poll auth error marker must be private"
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    FAKE_POLL_CODE=401 \
    "$ROOT/bin/fm-relay-poll.sh"); rc=$?
  ...
  assert_absent "$home/state/relay-poll.error" "poll 204 must clear the auth diagnostic marker"
  pass "fm-relay-poll surfaces auth/config errors once and clears on recovery"
}
...
test_poll_error_private_publication_rejects_unsafe_paths() {
  ...
  assert_absent "$home/external-state/relay-poll.error" "poll must not write the diagnostic through a linked state directory"
  ...
  ln -s "$target" "$home/state/relay-poll.error"
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
  ...
  [ -L "$home/state/relay-poll.error" ] || fail "poll must not replace a rejected linked diagnostic marker"

  home="$TMP_ROOT/poll-error-hardlink-marker"; mkdir -p "$home/state"
  fakebin=$(make_fake_curl "$home")
  chmod 700 "$home/state"
  marker="$home/state/relay-poll.error"
  hardlink="$home/state/relay-poll.alias"
  printf 'relay returned HTTP 401\n' > "$marker"
  chmod 600 "$marker"
  ln "$marker" "$hardlink"
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
  ...
  pass "fm-relay-poll diagnostic markers use private guarded publication"
}
...
test_poll_question_stashes_and_marks() {
  ...
  body='{"request_id":"req-7","platform":"discord","reply_max_chars":1900,"author_id":"42","text":"what are you building?"}'
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    FAKE_POLL_CODE=200 FAKE_POLL_BODY="$body" \
    "$ROOT/bin/fm-relay-poll.sh"); rc=$?
  expect_code 0 "$rc" "poll question exit"
  [ "$out" = "relay-mention req-7" ] || fail "poll must print compact marker (got: $out)"
  assert_present "$home/state/relay-inbox/req-7.json" "poll must stash the question"
  [ "$(jq -r .text "$home/state/relay-inbox/req-7.json")" = "what are you building?" ] \
    || fail "stashed inbox must preserve the question text"
  [ "$(jq -r .platform "$home/state/relay-inbox/req-7.json")" = "discord" ] \
    || fail "stashed inbox must preserve the explicit Discord platform"
  pass "fm-relay-poll stashes the question and prints the compact marker"
}
...
test_poll_mentions_wake_once_per_durable_offer() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_NOW_OVERRIDE=1700000000 \
    FM_RELAY_URL="https://relay.test" FAKE_POLL_CODE=200 FAKE_POLL_BODY="$body" \
    "$ROOT/bin/fm-relay-poll.sh"); rc=$?
  expect_code 0 "$rc" "first offered mention poll exit"
  [ "$out" = "relay-mention req-repeat" ] \
    || fail "a newly offered mention must wake once (got: $out)"
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_NOW_OVERRIDE=1700000030 \
    FM_RELAY_URL="https://relay.test" FAKE_POLL_CODE=200 FAKE_POLL_BODY="$body" \
    "$ROOT/bin/fm-relay-poll.sh"); rc=$?
  ...
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    FAKE_DISMISS_CODE=200 "$ROOT/bin/fm-relay-dismiss.sh" req-repeat); rc=$?
  ...
  rm -f "$home/state/relay-inbox/req-repeat.json"
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_NOW_OVERRIDE=1700000060 \
    FM_RELAY_URL="https://relay.test" FAKE_POLL_CODE=200 FAKE_POLL_BODY="$body" \
    "$ROOT/bin/fm-relay-poll.sh"); rc=$?
  expect_code 0 "$rc" "post-answer re-offer poll exit"
  [ -z "$out" ] || fail "a relay re-offer after inbox cleanup must stay silent (got: $out)"
  assert_absent "$home/state/relay-inbox/req-repeat.json" \
    "a suppressed post-answer re-offer must not recreate the drained inbox"
  marker="$home/state/relay-context/req-repeat.offered.json"
  assert_present "$marker" "the durable offer marker must survive inbox cleanup"
  rm -f "$marker"
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_NOW_OVERRIDE=1700000090 \
    FM_RELAY_URL="https://relay.test" FAKE_POLL_CODE=200 FAKE_POLL_BODY="$body" \
    "$ROOT/bin/fm-relay-poll.sh"); rc=$?
  ...
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_NOW_OVERRIDE=1700000120 \
    FM_RELAY_URL="https://relay.test" FAKE_POLL_CODE=200 FAKE_POLL_BODY="$body" \
    "$ROOT/bin/fm-relay-poll.sh"); rc=$?
  expect_code 0 "$rc" "genuinely new mention poll exit"
  [ "$out" = "relay-mention req-new" ] \
    || fail "a genuinely new request_id must wake once (got: $out)"
  marker="$home/state/relay-context/req-new.offered.json"
  [ "$(path_mode "$marker")" = 600 ] \
    || fail "the durable offer marker must be a private file"
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_NOW_OVERRIDE=1700604921 \
    FM_RELAY_URL="https://relay.test" FAKE_POLL_CODE=200 FAKE_POLL_BODY="$body" \
    "$ROOT/bin/fm-relay-poll.sh"); rc=$?
  ...
  pass "fm-relay-poll wakes once per durable request offer across inbox cleanup"
}
...
test_poll_offer_claim_failure_reports_once() {
  ...
  fakebin=$(make_fake_curl "$home")
  chmod 700 "$home/state"
  ln -s "$home/external-context" "$home/state/relay-context"
  body='{"request_id":"req-claim-failure","platform":"discord","reply_max_chars":1900,"text":"status?"}'
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
  ...
  [ "$out" = "relay-error cannot record mention offer" ] \
    || fail "an offer claim failure must emit one diagnostic (got: $out)"
  assert_present "$home/state/relay-poll.claim-error" "offer claim failure must write a dedupe marker"
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    FMX_PAIRING_TOKEN=tok-claim-failure FAKE_POLL_CODE=200 FAKE_POLL_BODY="$body" \
    "$ROOT/bin/fm-relay-poll.sh"); rc=$?
  ...
  assert_present "$home/state/relay-poll.claim-error" "a repeated offer claim failure must retain its dedupe marker"
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
  ...
  assert_present "$home/state/relay-poll.claim-error" \
  ...
  rm "$home/state/relay-context"
  mkdir "$home/state/relay-context"
  chmod 700 "$home/state/relay-context"
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    FMX_PAIRING_TOKEN=tok-claim-failure FAKE_POLL_CODE=200 FAKE_POLL_BODY="$body" \
    "$ROOT/bin/fm-relay-poll.sh"); rc=$?
  expect_code 0 "$rc" "recovered offer claim poll exit"
  [ "$out" = "relay-mention req-claim-failure" ] \
    || fail "a recovered offer claim must emit the mention wake (got: $out)"
  assert_absent "$home/state/relay-poll.claim-error" "a successful offer claim must clear the diagnostic marker"
  pass "fm-relay-poll retains offer claim diagnostics until recovery"
}
...
test_poll_preserves_conversation_context() {
  ...
  # A follow-up reply: the relay includes in_reply_to with the parent Discord message.
  body='{"request_id":"req-c","platform":"discord","reply_max_chars":1900,"author_id":"42","text":"and then what?","in_reply_to":{"author_handle":"@asker","text":"are you shipping today?"}}'
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    FAKE_POLL_CODE=200 FAKE_POLL_BODY="$body" \
    "$ROOT/bin/fm-relay-poll.sh"); rc=$?
  expect_code 0 "$rc" "poll conversation exit"
  [ "$out" = "relay-mention req-c" ] || fail "poll must mark the follow-up mention (got: $out)"
  f="$home/state/relay-inbox/req-c.json"
  ...
  fakebin=$(make_fake_curl "$home")
  printf 'FMX_PAIRING_TOKEN=tok-c\n' > "$home/.env"
  body='{"request_id":"req-f","platform":"discord","reply_max_chars":1900,"author_id":"42","text":"what are you up to?","in_reply_to":null}'
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    FAKE_POLL_CODE=200 FAKE_POLL_BODY="$body" \
    "$ROOT/bin/fm-relay-poll.sh"); rc=$?
  expect_code 0 "$rc" "poll fresh-mention exit"
  [ "$(jq -r '.in_reply_to' "$home/state/relay-inbox/req-f.json")" = "null" ] \
    || fail "a fresh mention must round-trip in_reply_to as null"
  pass "fm-relay-poll preserves in_reply_to conversation context in the inbox"
}
...
SH
  chmod +x "$fakebin/mv"
  printf 'FMX_PAIRING_TOKEN=tok-q\n' > "$home/.env"
  body='{"request_id":"req-rename","platform":"discord","reply_max_chars":1900,"author_id":"42","text":"what are you building?"}'
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    FAKE_POLL_CODE=200 FAKE_POLL_BODY="$body" \
    "$ROOT/bin/fm-relay-poll.sh"); rc=$?
  expect_code 0 "$rc" "poll inbox commit failure exit"
  [ "$out" = "relay-error cannot write inbox" ] \
    || fail "poll inbox commit failure must emit an error, not a wake marker (got: $out)"
  assert_absent "$home/state/relay-inbox/req-rename.json" "poll must not report a committed inbox file that was not created"
  assert_absent "$home/state/relay-inbox/req-rename.json.tmp" "poll must clean up the failed inbox temp file"
  assert_present "$home/state/relay-poll.error" "poll inbox commit failure must write a dedupe marker"
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    FAKE_POLL_CODE=200 FAKE_POLL_BODY="$body" \
    "$ROOT/bin/fm-relay-poll.sh"); rc=$?
  ...
  [ "$out" = "relay-mention req-rename" ] \
    || fail "poll must emit the mention marker once the inbox write succeeds (got: $out)"
  assert_absent "$home/state/relay-poll.error" "successful inbox write must clear the diagnostic marker"
  pass "fm-relay-poll reports inbox commit failures without emitting a mention wake"
}

test_poll_inbox_private_publication_rejects_unsafe_paths() {
  local home fakebin out rc body target dir dest hardlink
  body='{"request_id":"req-r","platform":"discord","reply_max_chars":1900,"author_id":"42","text":"what are you building?"}'

  home="$TMP_ROOT/poll-inbox-linked-dir"; mkdir -p "$home/state" "$home/external"
  fakebin=$(make_fake_curl "$home")
  ln -s "$home/external" "$home/state/relay-inbox"
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
  ...
  assert_absent "$home/external/req-r.json" "poll must not write through a linked inbox directory"
  [ -L "$home/state/relay-inbox" ] || fail "poll must leave the rejected inbox symlink in place"

  home="$TMP_ROOT/poll-inbox-public-dir"; mkdir -p "$home/state/relay-inbox"
  fakebin=$(make_fake_curl "$home")
  chmod 755 "$home/state/relay-inbox"
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
  ...
  assert_absent "$home/state/relay-inbox/req-r.json" "poll must not publish into a nonprivate inbox directory"
  assert_no_private_artifact_temps "$home/state/relay-inbox"

  home="$TMP_ROOT/poll-inbox-linked-dest"; mkdir -p "$home/state/relay-inbox"
  fakebin=$(make_fake_curl "$home")
  chmod 700 "$home/state/relay-inbox"
  target="$home/external-target.json"
  printf 'external sentinel\n' > "$target"
  ln -s "$target" "$home/state/relay-inbox/req-r.json"
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
  ...
  [ -L "$home/state/relay-inbox/req-r.json" ] || fail "poll must not replace a rejected linked destination"
  assert_no_private_artifact_temps "$home/state/relay-inbox"

  home="$TMP_ROOT/poll-inbox-hardlink-dest"; mkdir -p "$home/state/relay-inbox"
  fakebin=$(make_fake_curl "$home")
  chmod 700 "$home/state/relay-inbox"
  dest="$home/state/relay-inbox/req-r.json"
  hardlink="$home/state/relay-inbox/req-r.alias"
  printf '{"request_id":"req-r","text":"old"}\n' > "$dest"
  chmod 600 "$dest"
  ln "$dest" "$hardlink"
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
  ...
  [ "$out" = "relay-mention req-r" ] || fail "poll must still emit a wake after private publication (got: $out)"
  dir="$home/state/relay-inbox"
  [ "$(path_mode "$dir")" = 700 ] || fail "poll must create the inbox directory as private"
  [ "$(path_mode "$dir/req-r.json")" = 600 ] || fail "poll must publish the inbox file as private"
  assert_no_private_artifact_temps "$dir"
  pass "fm-relay-poll publishes inbox records only through private guarded artifacts"
}
...
test_poll_rejects_unsafe_request_id() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    FAKE_POLL_CODE=200 FAKE_POLL_BODY='{"request_id":"../../etc/path","platform":"discord","reply_max_chars":1900,"text":"hi"}' \
    "$ROOT/bin/fm-relay-poll.sh"); rc=$?
  expect_code 0 "$rc" "poll unsafe id exit"
  [ -z "$out" ] || fail "poll must not emit a marker for an unsafe request_id (got: $out)"
  assert_absent "$home/state/relay-inbox/../../etc/x.json" "poll must not write outside the inbox"
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    FAKE_POLL_CODE=200 FAKE_POLL_BODY='{"request_id":".hidden","platform":"discord","reply_max_chars":1900,"text":"hi"}' \
    "$ROOT/bin/fm-relay-poll.sh"); rc=$?
  expect_code 0 "$rc" "poll hidden id exit"
  [ -z "$out" ] || fail "poll must not emit a marker for a hidden request_id (got: $out)"
  assert_absent "$home/state/relay-inbox/.hidden.json" "poll must not stash a hidden inbox file"
  pass "fm-relay-poll rejects an unsafe request_id (path-traversal guard)"
}
...
test_reply_success_posts_request_bound_only() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    ...
    "$ROOT/bin/fm-relay-reply.sh" "req-7" "Aye, charting a couple of fixes."); rc=$?
  ...
  # The body must be exactly {request_id, text} - never a platform message id.
  local data
  ...
  pass "fm-relay-reply posts a request-bound answer and echoes only the request_id"
}
...
SH
  ...
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    ...
    "$ROOT/bin/fm-relay-reply.sh" "req-clean" "Hello." 2>"$home/err"); rc=$?
  ...
  pass "fm-relay-reply cleans up auth header temp files on interrupted posts"
}
...
test_reply_usage_error() {
  ...
  err="$home/err.txt"
  PATH="$BASE_PATH" FM_HOME="$home" "$ROOT/bin/fm-relay-reply.sh" "only-one" >/dev/null 2>"$err"; rc=$?
  expect_code 2 "$rc" "reply usage error exit"
  assert_grep "--image <path>" "$err" "reply usage must mention --image"
  pass "fm-relay-reply rejects missing arguments with a usage error"
}

test_reply_help_mentions_image() {
  local home out rc
  home="$TMP_ROOT/reply-help"; mkdir -p "$home"
  out=$(PATH="$BASE_PATH" FM_HOME="$home" "$ROOT/bin/fm-relay-reply.sh" --help); rc=$?
  expect_code 0 "$rc" "reply --help exit"
  assert_contains "$out" "--image <path>" "reply help must mention --image"
  assert_contains "$out" "threaded replies attach it to the opening Discord message" \
    "reply help must document thread image placement"
  pass "fm-relay-reply --help makes image support discoverable"
}
...
test_reply_whitespace_text_rejected() {
  ...
  err="$home/err.txt"
  out=$(PATH="$BASE_PATH" FM_HOME="$home" FM_RELAY_DRY_RUN=1 \
    "$ROOT/bin/fm-relay-reply.sh" "req-space" "   " 2>"$err"); rc=$?
  ...
  assert_absent "$home/state/relay-outbox/req-space.json" "whitespace-only dry-run must not record an outbox preview"
  pass "fm-relay-reply rejects whitespace-only reply text"
}
...
test_bootstrap_activates_on_env_token() {
  ...
  assert_contains "$out" "RELAY: Relay on" "bootstrap must announce Relay"
  assert_present "$home/state/relay-watch.check.sh" "bootstrap must drop the check shim"
  [ -x "$home/state/relay-watch.check.sh" ] || fail "the check shim must be executable"
  assert_grep "fm-relay-poll.sh" "$home/state/relay-watch.check.sh" "the shim must exec the poll script"
  assert_present "$home/config/relay.env" "bootstrap must drop the cadence config"
  assert_grep "export FM_CHECK_INTERVAL=30" "$home/config/relay.env" "cadence must be 30s"
  ...
  inherited=$( . "$home/config/relay.env" && bash -c 'echo "${FM_CHECK_INTERVAL:-300}"' )
  [ "$inherited" = "30" ] \
  ...
  sum1=$(cat "$home/state/relay-watch.check.sh" "$home/config/relay.env" | shasum)
  FM_HOME="$home" "$ROOT/bin/fm-bootstrap.sh" >/dev/null 2>&1
  sum2=$(cat "$home/state/relay-watch.check.sh" "$home/config/relay.env" | shasum)
  [ "$sum1" = "$sum2" ] || fail "bootstrap Relay setup must be idempotent"
  n=$(find "$home/state" -maxdepth 1 -name 'relay-watch*' | wc -l | tr -d ' ')
  [ "$n" = "1" ] || fail "bootstrap must not duplicate the shim (found $n)"
  pass "bootstrap activates Relay from an .env token, idempotently"
}
...
test_bootstrap_relative_home_writes_absolute_poll_shim() {
  ...
  assert_contains "$out" "RELAY: Relay on" "relative-home bootstrap must announce Relay"
  quoted_home=$(printf '%q' "$home")
  assert_grep "export FM_HOME=$quoted_home" "$home/state/relay-watch.check.sh" \
    "relative FM_HOME leaked into the durable Relay poll shim"
  pass "bootstrap ignores CDPATH when writing absolute FM_HOME into the durable Relay poll shim"
}

test_bootstrap_reports_missing_relay_dependency() {
  local home fakebin out tool tool_path
  home="$TMP_ROOT/boot-missing-relay"; mkdir -p "$home"
  fakebin=$(fm_fakebin "$home")
  fm_fake_exit0 "$fakebin" herdr node no-mistakes ego-browser curl
...
SH
  ...
  assert_contains "$out" "MISSING: jq" "bootstrap must report missing jq when Relay is opted in"
  assert_not_contains "$out" "RELAY: Relay on" "bootstrap must not announce Relay when a dependency is missing"
  assert_absent "$home/state/relay-watch.check.sh" "missing jq must not arm the check shim"
  assert_absent "$home/config/relay.env" "missing jq must not write the cadence config"
  pass "bootstrap reports missing Relay dependencies before arming"
}
...
test_bootstrap_does_not_announce_when_arm_fails() {
  ...
  assert_contains "$out" "RELAY: Relay off - failed to arm relay poll shim or 30s cadence" \
    "bootstrap must report a failed Relay activation"
  assert_not_contains "$out" "RELAY: Relay on" \
    "bootstrap must not announce Relay when the shim or cadence was not armed"
  assert_absent "$home/state/relay-watch.check.sh" "failed Relay activation must not leave an armed shim"
  pass "bootstrap does not report Relay on when activation artifacts cannot be written"
}

test_bootstrap_does_not_follow_relay_artifact_symlinks() {
  ...
  ln -s "$shim_target" "$home/state/relay-watch.check.sh"
  ln -s "$cadence_target" "$home/config/relay.env"
  ...
  assert_contains "$out" "RELAY: Relay off - failed to arm relay poll shim or 30s cadence" \
    "bootstrap must reject linked Relay destinations"
  assert_not_contains "$out" "RELAY: Relay on" \
    "bootstrap must not announce Relay after rejecting linked destinations"
  ...
  assert_absent "$home/state/relay-watch.check.sh" "bootstrap must remove the rejected shim link"
  assert_absent "$home/config/relay.env" "bootstrap must remove the rejected cadence link"
  pass "bootstrap rejects linked Relay artifacts without touching their targets"
}
...
test_bootstrap_inert_without_token() {
  ...
  out=$(FM_HOME="$home" "$ROOT/bin/fm-bootstrap.sh" 2>/dev/null)
  assert_not_contains "$out" "RELAY:" "bootstrap must say nothing about Relay without a token"
  assert_absent "$home/state/relay-watch.check.sh" "no token -> no check shim"
  assert_absent "$home/config/relay.env" "no token -> no cadence config"
  # .env present but token empty -> still off.
  home="$TMP_ROOT/boot-empty"; mkdir -p "$home"
  printf 'FMX_PAIRING_TOKEN=\n' > "$home/.env"
  out=$(FM_HOME="$home" "$ROOT/bin/fm-bootstrap.sh" 2>/dev/null)
  assert_not_contains "$out" "RELAY:" "an empty token must be treated as off"
  assert_absent "$home/state/relay-watch.check.sh" "empty token -> no check shim"
  pass "bootstrap is inert without a non-empty .env token (non-Relay users unaffected)"
}
...
test_poll_empty_text_is_silent() {
  ...
  assert_absent "$home/state/relay-inbox/req-9.json" "poll must not stash an empty question"
  ...
  assert_absent "$home/state/relay-inbox/req-10.json" "poll must not stash when .text is absent"
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
  ...
  assert_absent "$home/state/relay-inbox/req-11.json" "poll must not stash a whitespace-only question"
  pass "fm-relay-poll requires a non-empty question before waking"
}
...
test_reply_text_file_and_stdin() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    FAKE_CURL_LOG="$log" FAKE_ANSWER_CODE=200 \
    "$ROOT/bin/fm-relay-reply.sh" "req-1" --text-file "$home/reply.txt"); rc=$?
  ...
  out=$(printf '%s' 'reply via stdin' | PATH="$fakebin:$BASE_PATH" FM_HOME="$home" \
    FM_RELAY_URL="https://relay.test" FAKE_CURL_LOG="$log" FAKE_ANSWER_CODE=200 \
    "$ROOT/bin/fm-relay-reply.sh" "req-2" -); rc=$?
  expect_code 0 "$rc" "reply stdin exit"
  data=$(grep '^data=' "$log" | tail -1 | sed 's/^data=//')
  [ "$(printf '%s' "$data" | jq -r .text)" = 'reply via stdin' ] \
    || fail "reply via stdin must send the piped text"
  pass "fm-relay-reply accepts the reply via --text-file and stdin (safe, unexpanded)"
}

test_bootstrap_migrates_legacy_local_relay_state_once() {
  local home out meta
  home="$TMP_ROOT/boot-local-migration"
  mkdir -p "$home/state/x-inbox" "$home/state/x-context" "$home/state/x-outbox" "$home/config"
  chmod 700 "$home/state" "$home/state/x-inbox" "$home/state/x-context" "$home/state/x-outbox" "$home/config"
  printf '%s\n' '#!/usr/bin/env bash' 'exit 0' > "$home/state/x-watch.check.sh"
  chmod 700 "$home/state/x-watch.check.sh"
  printf 'export FM_CHECK_INTERVAL=30\n' > "$home/config/x-mode.env"
  chmod 600 "$home/config/x-mode.env"
  jq -cn '{request_id:"req-migrate",platform:"discord",reply_max_chars:1900,text:"question"}' \
    > "$home/state/x-inbox/req-migrate.json"
  jq -cn '{request_id:"req-migrate",platform:"discord",reply_max_chars:"1900",recorded_at:1700000000}' \
    > "$home/state/x-context/req-migrate.json"
  jq -cn '{request_id:"req-migrate",text:"preview"}' > "$home/state/x-outbox/req-migrate.json"
  chmod 600 "$home/state/x-inbox/req-migrate.json" "$home/state/x-context/req-migrate.json" \
    "$home/state/x-outbox/req-migrate.json"
  meta="$home/state/task.meta"
  printf 'kind=ship\nx_request=req-migrate\nx_request_ts=1700000000\nx_followups=1\nx_platform=discord\nx_reply_max_chars=1900\n' > "$meta"
  chmod 600 "$meta"
  printf 'FMX_PAIRING_TOKEN=tok-migrate\nFMX_RELAY_URL=https://relay.test/\nFMX_DISCORD_REPLY_MAX_CHARS=1800\nFMX_X_THREAD_MAX=12\n' > "$home/.env"
  chmod 600 "$home/.env"

  out=$(FM_HOME="$home" "$ROOT/bin/fm-bootstrap.sh" 2>/dev/null)
  assert_contains "$out" "RELAY: migrated legacy local connector state and settings to Relay names" \
    "bootstrap must report the one-time local migration"
  assert_present "$home/state/relay-local-migration-v1" "migration must publish its completion marker"
  assert_present "$home/state/relay-inbox/req-migrate.json" "migration must retain the pending Discord mention"
  assert_present "$home/state/relay-context/req-migrate.json" "migration must retain Discord reply context"
  assert_present "$home/state/relay-outbox/req-migrate.json" "migration must retain dry-run previews"
  assert_absent "$home/state/x-inbox" "migration must retire the old inbox owner"
  assert_absent "$home/state/x-context" "migration must retire the old context owner"
  assert_absent "$home/state/x-outbox" "migration must retire the old outbox owner"
  assert_grep 'relay_request=req-migrate' "$meta" "migration must rename task request metadata"
  assert_grep 'relay_platform=discord' "$meta" "migration must preserve Discord task-link context"
  ! grep -Eq '^x_' "$meta" || fail "migration must remove old task metadata names"
  assert_grep 'FM_RELAY_URL=https://relay.test/' "$home/.env" "migration must rename the optional Relay URL"
  assert_grep 'FM_RELAY_REPLY_MAX_CHARS=1800' "$home/.env" "migration must preserve the Discord reply budget"
  assert_grep 'FM_RELAY_THREAD_MAX=12' "$home/.env" "migration must preserve the shared thread cap"
  ! grep -Eq '^FMX_(RELAY_URL|DISCORD_REPLY_MAX_CHARS|X_THREAD_MAX)=' "$home/.env" \
    || fail "migration must retire old optional setting names"
  assert_present "$home/state/relay-watch.check.sh" "bootstrap must regenerate the Relay poll shim"
  assert_present "$home/config/relay.env" "bootstrap must regenerate the Relay cadence config"
  assert_absent "$home/state/x-watch.check.sh" "migration must retire the old poll shim"
  assert_absent "$home/config/x-mode.env" "migration must retire the old cadence config"

  out=$(FM_HOME="$home" "$ROOT/bin/fm-bootstrap.sh" 2>/dev/null)
  assert_not_contains "$out" "migrated legacy local connector state" \
    "completed local migration must not run or report twice"
  pass "bootstrap migrates existing Discord Relay state and optional settings exactly once"
}
...
test_bootstrap_opt_out_cleanup() {
  ...
  FM_HOME="$home" "$ROOT/bin/fm-bootstrap.sh" >/dev/null 2>&1
  assert_present "$home/state/relay-watch.check.sh" "opt-in must create the shim"
  assert_present "$home/config/relay.env" "opt-in must create the cadence config"
  ...
  assert_contains "$out" "RELAY: Relay off" "opt-out must announce Relay off when it removed artifacts"
  assert_contains "$out" "repair a missing or failed watcher cycle with the Pi tool" "opt-out remediation must use Pi recovery guidance"
  ...
  assert_absent "$home/state/relay-watch.check.sh" "opt-out must remove the shim"
  assert_absent "$home/config/relay.env" "opt-out must remove the cadence config"
  # Steady-state off: another run with nothing to remove is silent.
  out=$(FM_HOME="$home" "$ROOT/bin/fm-bootstrap.sh" 2>/dev/null)
  assert_not_contains "$out" "RELAY:" "steady-state off must be silent"
  pass "bootstrap cleans up Relay artifacts on opt-out and is silent once off"
}
...
test_bootstrap_opt_out_reports_cleanup_failure() {
  ...
  printf 'FMX_PAIRING_TOKEN=tok-out\n' > "$home/.env"
  FM_HOME="$home" "$ROOT/bin/fm-bootstrap.sh" >/dev/null 2>&1
  assert_present "$home/state/relay-watch.check.sh" "opt-in must create the shim before cleanup failure"
  assert_present "$home/config/relay.env" "opt-in must create the cadence config before cleanup failure"
  fakebin=$(fm_fakebin "$home")
  cat > "$fakebin/rm" <<'SH'
#!/usr/bin/env bash
exit 1
SH
  ...
  assert_contains "$out" "RELAY: Relay off - failed to remove relay poll shim or 30s cadence" \
    "opt-out cleanup failure must be reported"
  assert_present "$home/state/relay-watch.check.sh" "failed opt-out cleanup must leave the stale shim visible"
  assert_present "$home/config/relay.env" "failed opt-out cleanup must leave the stale cadence visible"
  pass "bootstrap reports failed Relay artifact cleanup on opt-out"
}
...
test_reply_dry_run_records_not_posts() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    FM_RELAY_DRY_RUN=1 FAKE_CURL_LOG="$log" \
    "$ROOT/bin/fm-relay-reply.sh" "req-1" "Aye, a couple of fixes underway." 2>"$home/err"); rc=$?
  ...
  assert_present "$home/state/relay-outbox/req-1.json" "dry-run must record the would-be reply"
  [ "$(jq -r .text "$home/state/relay-outbox/req-1.json")" = "Aye, a couple of fixes underway." ] \
    || fail "outbox record must hold the would-be reply text"
  [ "$(jq -r .request_id "$home/state/relay-outbox/req-1.json")" = "req-1" ] \
    || fail "outbox record must hold the request_id"
  assert_grep "DRY RUN" "$home/err" "dry-run must surface a DRY RUN summary on stderr"
  pass "fm-relay-reply dry-run records the would-be reply and never posts"
}
...
test_reply_dry_run_needs_no_token() {
  ...
  out=$(PATH="$BASE_PATH" FM_HOME="$home" FM_RELAY_DRY_RUN=1 \
    "$ROOT/bin/fm-relay-reply.sh" "req-2" "preview without creds" 2>/dev/null); rc=$?
  expect_code 0 "$rc" "dry-run no-token exit"
  [ "$out" = "req-2" ] || fail "dry-run without a token must still echo the request_id (got: $out)"
  assert_present "$home/state/relay-outbox/req-2.json" "dry-run without a token must still record the preview"
  pass "fm-relay-reply dry-run works without a token"
}
...
test_reply_dry_run_from_env_file() {
  ...
  fakebin=$(make_fake_curl "$home")
  log="$home/curl.log"
  # FM_RELAY_DRY_RUN read from .env (not just the environment).
  printf 'FMX_PAIRING_TOKEN=tok-d\nFM_RELAY_DRY_RUN=1\n' > "$home/.env"
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    FAKE_CURL_LOG="$log" "$ROOT/bin/fm-relay-reply.sh" "req-3" "from dotenv" 2>/dev/null); rc=$?
  ...
  assert_present "$home/state/relay-outbox/req-3.json" "dry-run from .env must record the preview"
  pass "fm-relay-reply honors FM_RELAY_DRY_RUN from .env"
}
...
test_reply_empty_env_dry_run_overrides_env_file() {
  ...
  printf 'FMX_PAIRING_TOKEN=tok-d\nFM_RELAY_DRY_RUN=1\n' > "$home/.env"
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    FM_RELAY_DRY_RUN='' FAKE_CURL_LOG="$log" FAKE_ANSWER_CODE=200 \
    "$ROOT/bin/fm-relay-reply.sh" "req-5" "empty env disables dry run" 2>/dev/null); rc=$?
  ...
  assert_absent "$home/state/relay-outbox/req-5.json" "empty dry-run env override must not record an outbox preview"
  pass "fm-relay-reply lets an explicitly empty dry-run env override .env"
}
...
test_reply_dry_run_fails_when_outbox_unwritable() {
  ...
  err="$home/err.txt"
  printf '%s\n' 'not a directory' > "$home/state/relay-outbox"
  out=$(PATH="$BASE_PATH" FM_HOME="$home" FM_RELAY_DRY_RUN=1 \
    "$ROOT/bin/fm-relay-reply.sh" "req-4" "preview text" 2>"$err"); rc=$?
  ...
  pass "fm-relay-reply dry-run fails when it cannot record the preview"
}
...
test_reply_dry_run_outbox_private_publication_rejects_unsafe_paths() {
  ...
  err="$home/err.txt"
  ln -s "$home/external" "$home/state/relay-outbox"
  out=$(FM_HOME="$home" FM_RELAY_DRY_RUN=1 "$ROOT/bin/fm-relay-reply.sh" req-r "preview text" 2>"$err"); rc=$?
  ...
  assert_absent "$home/external/req-r.json" "reply dry-run must not write through a linked outbox directory"

  home="$TMP_ROOT/reply-outbox-linked-dest"; mkdir -p "$home/state/relay-outbox"
  err="$home/err.txt"
  chmod 700 "$home/state/relay-outbox"
  target="$home/external-target.json"
  printf 'external sentinel\n' > "$target"
  ln -s "$target" "$home/state/relay-outbox/req-r.json"
  out=$(FM_HOME="$home" FM_RELAY_DRY_RUN=1 "$ROOT/bin/fm-relay-reply.sh" req-r "preview text" 2>"$err"); rc=$?
  ...
  [ -L "$home/state/relay-outbox/req-r.json" ] || fail "reply dry-run must not replace a rejected linked destination"
  assert_no_private_artifact_temps "$home/state/relay-outbox"

  home="$TMP_ROOT/reply-outbox-hardlink-dest"; mkdir -p "$home/state/relay-outbox"
  err="$home/err.txt"
  chmod 700 "$home/state/relay-outbox"
  dest="$home/state/relay-outbox/req-r.json"
  hardlink="$home/state/relay-outbox/req-r.alias"
  printf '{"request_id":"req-r","text":"old"}\n' > "$dest"
  chmod 600 "$dest"
  ln "$dest" "$hardlink"
  out=$(FM_HOME="$home" FM_RELAY_DRY_RUN=1 "$ROOT/bin/fm-relay-reply.sh" req-r "preview text" 2>"$err"); rc=$?
  ...
  home="$TMP_ROOT/reply-outbox-wrong-mode-dest"; mkdir -p "$home/state/relay-outbox"
  err="$home/err.txt"
  chmod 700 "$home/state/relay-outbox"
  dest="$home/state/relay-outbox/req-r.json"
  printf '{"request_id":"req-r","text":"old"}\n' > "$dest"
  ...
  out=$(FM_HOME="$home" FM_RELAY_DRY_RUN=1 "$ROOT/bin/fm-relay-reply.sh" req-r "preview text" 2>/dev/null); rc=$?
  expect_code 0 "$rc" "reply private outbox success exit"
  [ "$out" = "req-r" ] || fail "reply dry-run must still echo the request_id after private publication (got: $out)"
  [ "$(path_mode "$home/state/relay-outbox")" = 700 ] || fail "reply dry-run must create the outbox directory as private"
  [ "$(path_mode "$home/state/relay-outbox/req-r.json")" = 600 ] || fail "reply dry-run must publish the outbox file as private"
  assert_no_private_artifact_temps "$home/state/relay-outbox"
  pass "fm-relay-reply dry-run publishes outbox records only through private guarded artifacts"
}
...
test_split_thread_lib() {
  ...
  # A reply that fits one Discord message stays a single, UNNUMBERED chunk.
  out=$(printf 'Aye, all shipshape.' | fm_relay_split_thread 280 25)
  ...
  out=$(printf '%s' "$txt" | fm_relay_split_thread 30 25)
  ...
  out=$(printf 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa' | fm_relay_split_thread 20 25)
  ...
  out=$(printf 'one two three four five six seven eight nine ten' | fm_relay_split_thread 20 2)
...
TXT
  ...
  out=$(printf '%s' "$txt" | fm_relay_split_thread 120 25)
  ...
  pass "fm_relay_split_thread: word-boundary, fence-aware, within-limit, numbered, lossless, capped"
}

test_reply_single_no_texts() {
  local home out
  home="$TMP_ROOT/reply-single"; mkdir -p "$home"
  out=$(FM_HOME="$home" FM_RELAY_DRY_RUN=1 "$ROOT/bin/fm-relay-reply.sh" req-s "Short and sweet." 2>/dev/null)
  [ "$out" = "req-s" ] || fail "single dry-run must echo the request_id (got: $out)"
  jq -e 'has("texts")|not' "$home/state/relay-outbox/req-s.json" >/dev/null || fail "a one-Discord message reply must not include texts"
  [ "$(jq -r '.text' "$home/state/relay-outbox/req-s.json")" = "Short and sweet." ] || fail "single reply text must be verbatim and unnumbered"
  pass "fm-relay-reply keeps a concise reply as a single unnumbered Discord message"
}
...
test_reply_thread_dry_run() {
  ...
  out=$(FM_HOME="$home" FM_RELAY_DRY_RUN=1 FM_RELAY_REPLY_MAX_CHARS=50 \
    "$ROOT/bin/fm-relay-reply.sh" req-t "$long" 2>/dev/null)
  [ "$out" = "req-t" ] || fail "thread dry-run must echo the request_id (got: $out)"
  assert_present "$home/state/relay-outbox/req-t.json" "thread dry-run must record the outbox preview"
  jq -e '.texts and (.texts|length>1)' "$home/state/relay-outbox/req-t.json" >/dev/null || fail "a long reply must record a texts[] thread"
  [ "$(jq '.texts|map(length)|max' "$home/state/relay-outbox/req-t.json")" -le 50 ] || fail "each thread Discord message must be within the limit"
  [ "$(jq -r '.text' "$home/state/relay-outbox/req-t.json")" = "$(jq -r '.texts[0]' "$home/state/relay-outbox/req-t.json")" ] || fail "text must equal the first chunk"
  pass "fm-relay-reply auto-splits a long reply into a numbered thread (texts[])"
}

test_reply_discord_inbox_uses_discord_budget() {
  local home out reply
  home="$TMP_ROOT/reply-discord-budget"; private_artifact_dir "$home/state/relay-inbox"
  jq -cn '{request_id:"req-discord",platform:"discord",reply_max_chars:1900,text:"question"}' \
    > "$home/state/relay-inbox/req-discord.json"
  private_artifact_file "$home/state/relay-inbox/req-discord.json"
  reply=$(cat <<'TXT'
First paragraph stays intact in a single Discord reply even though it is comfortably within the Discord message budget.
...
TXT
  ...
  out=$(FM_HOME="$home" FM_RELAY_DRY_RUN=1 "$ROOT/bin/fm-relay-reply.sh" req-discord "$reply" 2>/dev/null)
  [ "$out" = "req-discord" ] || fail "Discord dry-run must echo the request_id (got: $out)"
  jq -e 'has("texts")|not' "$home/state/relay-outbox/req-discord.json" >/dev/null \
    || fail "Discord reply below its message budget must not be split into texts[]"
  assert_contains "$(jq -r '.text' "$home/state/relay-outbox/req-discord.json")" '```bash' \
    "Discord reply must preserve the fenced code block"
  pass "fm-relay-reply uses the explicit Discord request budget from the inbox"
}

test_poll_rejects_non_discord_platforms_and_message_id_fallbacks() {
  local home fakebin out rc
  home="$TMP_ROOT/poll-discord-only"; mkdir -p "$home"
  fakebin=$(make_fake_curl "$home")
  printf 'FMX_PAIRING_TOKEN=tok-discord-only\n' > "$home/.env"

  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FAKE_POLL_CODE=200 \
    FAKE_POLL_BODY='{"request_id":"req-unsupported","platform":"unsupported","reply_max_chars":1900,"text":"question"}' \
    "$ROOT/bin/fm-relay-poll.sh"); rc=$?
  expect_code 0 "$rc" "unsupported platform poll exit"
  [ "$out" = "relay-error relay payload did not identify Discord explicitly" ] \
    || fail "unsupported platform payload must produce the bounded Relay diagnostic"
  assert_absent "$home/state/relay-inbox/req-unsupported.json" \
    "unsupported platform payload must never enter the Relay inbox"

  rm -f "$home/state/relay-poll.error"
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FAKE_POLL_CODE=200 \
    FAKE_POLL_BODY='{"request_id":"req-message-id-only","message_id":"1234567890","reply_max_chars":1900,"text":"question"}' \
    "$ROOT/bin/fm-relay-poll.sh"); rc=$?
  expect_code 0 "$rc" "message-id-only poll exit"
  [ "$out" = "relay-error relay payload did not identify Discord explicitly" ] \
    || fail "a numeric message id must not select a platform"
  assert_absent "$home/state/relay-inbox/req-message-id-only.json" \
    "message-id inference must never create a Relay inbox record"
  pass "Relay accepts only explicit Discord payloads and has no numeric platform fallback"
}

test_reply_inbox_explicit_limit_wins() {
  local home out long
  home="$TMP_ROOT/reply-explicit-limit"; private_artifact_dir "$home/state/relay-inbox"
  jq -cn '{request_id:"req-limit",platform:"discord",reply_max_chars:90,text:"question"}' \
    > "$home/state/relay-inbox/req-limit.json"
  private_artifact_file "$home/state/relay-inbox/req-limit.json"
  ...
  out=$(FM_HOME="$home" FM_RELAY_DRY_RUN=1 "$ROOT/bin/fm-relay-reply.sh" req-limit "$long" 2>/dev/null)
  [ "$out" = "req-limit" ] || fail "explicit-limit dry-run must echo the request_id (got: $out)"
  jq -e '.texts and (.texts|length>1)' "$home/state/relay-outbox/req-limit.json" >/dev/null \
    || fail "explicit reply_max_chars must force a split even on Discord"
  [ "$(jq '.texts|map(length)|max' "$home/state/relay-outbox/req-limit.json")" -le 90 ] \
    || fail "explicit-limit chunks must stay within the relay-provided budget"
  pass "fm-relay-reply prefers an explicit relay-provided reply limit"
}

test_reply_rejects_unsafe_inbox_context_reads() {
  local home out rc reply target dest hardlink
  reply="This reply is intentionally longer than the configured test budget, but shorter than a normal Discord message. If an unsafe inbox artifact is trusted it will stay one message; if it is rejected it will split at the default Discord budget, which is the fail-closed behavior for a local lookalike context record."
  ...
  ln -s "$home/external-inbox" "$home/state/relay-inbox"
  out=$(FM_HOME="$home" FM_RELAY_DRY_RUN=1 FM_RELAY_REPLY_MAX_CHARS=50 "$ROOT/bin/fm-relay-reply.sh" req-linked-dir "$reply" 2>/dev/null); rc=$?
  expect_code 0 "$rc" "reply linked inbox dir read exit"
  jq -e '.texts and (.texts|length>1)' "$home/state/relay-outbox/req-linked-dir.json" >/dev/null \
    || fail "reply must not trust a linked inbox directory for Discord budget context"

  home="$TMP_ROOT/reply-inbox-linked-file"; private_artifact_dir "$home/state/relay-inbox"
  ...
  ln -s "$target" "$home/state/relay-inbox/req-linked-file.json"
  out=$(FM_HOME="$home" FM_RELAY_DRY_RUN=1 FM_RELAY_REPLY_MAX_CHARS=50 "$ROOT/bin/fm-relay-reply.sh" req-linked-file "$reply" 2>/dev/null); rc=$?
  expect_code 0 "$rc" "reply linked inbox file read exit"
  jq -e '.texts and (.texts|length>1)' "$home/state/relay-outbox/req-linked-file.json" >/dev/null \
    || fail "reply must not trust a linked inbox file for Discord budget context"

  home="$TMP_ROOT/reply-inbox-hardlink-file"; private_artifact_dir "$home/state/relay-inbox"
  dest="$home/state/relay-inbox/req-hardlink.json"
  hardlink="$home/state/relay-inbox/req-hardlink.alias"
  jq -cn '{request_id:"req-hardlink",platform:"discord",reply_max_chars:1900,text:"question"}' > "$dest"
  private_artifact_file "$dest"
  ln "$dest" "$hardlink"
  out=$(FM_HOME="$home" FM_RELAY_DRY_RUN=1 FM_RELAY_REPLY_MAX_CHARS=50 "$ROOT/bin/fm-relay-reply.sh" req-hardlink "$reply" 2>/dev/null); rc=$?
  expect_code 0 "$rc" "reply hardlinked inbox file read exit"
  jq -e '.texts and (.texts|length>1)' "$home/state/relay-outbox/req-hardlink.json" >/dev/null \
  ...
  home="$TMP_ROOT/reply-inbox-public-dir"; mkdir -p "$home/state/relay-inbox"
  chmod 755 "$home/state/relay-inbox"
  jq -cn '{request_id:"req-public-dir",platform:"discord",reply_max_chars:1900,text:"question"}' \
    > "$home/state/relay-inbox/req-public-dir.json"
  private_artifact_file "$home/state/relay-inbox/req-public-dir.json"
  out=$(FM_HOME="$home" FM_RELAY_DRY_RUN=1 FM_RELAY_REPLY_MAX_CHARS=50 "$ROOT/bin/fm-relay-reply.sh" req-public-dir "$reply" 2>/dev/null); rc=$?
  expect_code 0 "$rc" "reply public inbox dir read exit"
  jq -e '.texts and (.texts|length>1)' "$home/state/relay-outbox/req-public-dir.json" >/dev/null \
    || fail "reply must not trust a nonprivate inbox directory for Discord budget context"
  pass "fm-relay-reply rejects unsafe inbox context artifacts"
}
...
test_reply_max_chars_floor_clamps_to_minimum() {
  ...
  out=$(FM_HOME="$home" FM_RELAY_DRY_RUN=1 FM_RELAY_REPLY_MAX_CHARS=49 \
    "$ROOT/bin/fm-relay-reply.sh" req-floor "$long" 2>/dev/null)
  [ "$out" = "req-floor" ] || fail "reply max floor dry-run must echo the request_id (got: $out)"
  jq -e '.texts and (.texts|length>1)' "$home/state/relay-outbox/req-floor.json" >/dev/null || fail "a below-floor max must clamp to 50 and still split"
  [ "$(jq '.texts|map(length)|max' "$home/state/relay-outbox/req-floor.json")" -le 50 ] || fail "clamped thread Discord messages must be within the 50 character floor"
  pass "fm-relay-reply clamps a below-floor max to 50 characters"
}
...
test_reply_thread_live_posts_texts() {
  ...
  # 50 is the configured minimum per-Discord message budget; the text is well over it so it
  # must split into a multi-Discord thread.
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    FM_RELAY_REPLY_MAX_CHARS=50 FAKE_CURL_LOG="$log" FAKE_ANSWER_CODE=200 \
    "$ROOT/bin/fm-relay-reply.sh" req-l "alpha bravo charlie delta echo foxtrot golf hotel india juliet kilo lima mike november oscar papa quebec romeo")
  ...
  pass "fm-relay-reply posts a thread payload (texts[]) to the relay"
}
...
test_reply_image_live_posts_image_object() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    FAKE_CURL_LOG="$log" FAKE_ANSWER_CODE=200 \
    "$ROOT/bin/fm-relay-reply.sh" "req-img" --image "$img" "Here is the illustration."); rc=$?
  ...
  pass "fm-relay-reply --image posts an image object on answer"
}
...
test_reply_image_live_streams_payload_file() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    FAKE_CURL_LOG="$log" FAKE_ANSWER_CODE=200 \
    "$ROOT/bin/fm-relay-reply.sh" "req-img-stream" --image "$img" "Here is the illustration."); rc=$?
  ...
  pass "fm-relay-reply streams large image payloads outside curl argv"
}
...
test_reply_image_thread_dry_run_records_compact_marker() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_DRY_RUN=1 FM_RELAY_REPLY_MAX_CHARS=50 \
    FAKE_CURL_LOG="$log" \
    "$ROOT/bin/fm-relay-reply.sh" "req-img-dry" --image "$img" \
  ...
  assert_present "$home/state/relay-outbox/req-img-dry.json" "image dry-run must record the preview"
  jq -e '.texts and (.texts|length>1)' "$home/state/relay-outbox/req-img-dry.json" >/dev/null \
    || fail "image dry-run thread must keep texts[]"
  [ "$(jq -r '.image.media_type' "$home/state/relay-outbox/req-img-dry.json")" = "image/webp" ] \
    || fail "image dry-run marker must hold media_type"
  [ "$(jq -r '.image.bytes' "$home/state/relay-outbox/req-img-dry.json")" = "$bytes" ] \
    || fail "image dry-run marker must hold byte count"
  [ "$(jq -r '.image.source_path' "$home/state/relay-outbox/req-img-dry.json")" = "$img" ] \
    || fail "image dry-run marker must hold source_path"
  jq -e '.image | has("data_base64") | not' "$home/state/relay-outbox/req-img-dry.json" >/dev/null \
    || fail "image dry-run marker must not include base64 bytes"
  pass "fm-relay-reply dry-run records compact image metadata for threaded replies"
}
...
test_reply_image_dry_run_cleans_payload_temp_files() {
  ...
  make_sample_image "$img"
  out=$(PATH="$BASE_PATH" TMPDIR="$tmpdir" FM_HOME="$home" FM_RELAY_DRY_RUN=1 \
    "$ROOT/bin/fm-relay-reply.sh" "req-img-temp-clean" --image "$img" "Here is the image." \
    2>"$home/err"); rc=$?
  ...
  leftovers=$(find "$tmpdir" -type f -name 'fm-relay-reply.*' -print)
  [ -z "$leftovers" ] || fail "reply temp files must be cleaned (left: $leftovers)"
  pass "fm-relay-reply cleans image and payload temp files"
}
...
test_reply_image_path_errors_are_clear() {
  ...
  err="$home/err.txt"
  out=$(PATH="$BASE_PATH" FM_HOME="$home" FM_RELAY_DRY_RUN=1 \
    "$ROOT/bin/fm-relay-reply.sh" "req-missing" --image "$home/missing.png" "text" 2>"$err"); rc=$?
  ...
  out=$(PATH="$BASE_PATH" FM_HOME="$home" FM_RELAY_DRY_RUN=1 \
    "$ROOT/bin/fm-relay-reply.sh" "req-badtype" --image "$img" "text" 2>"$err"); rc=$?
  ...
  out=$(PATH="$BASE_PATH" FM_HOME="$home" FM_RELAY_DRY_RUN=1 \
    "$ROOT/bin/fm-relay-reply.sh" "req-noarg" --image 2>"$err"); rc=$?
  ...
  pass "fm-relay-reply --image rejects missing and unsupported image paths clearly"
}
...
test_reply_followup_live_posts_to_followup_endpoint() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    FM_RELAY_REQUEST_PLATFORM=discord FM_RELAY_REQUEST_REPLY_MAX_CHARS=280 \
    FAKE_CURL_LOG="$log" FAKE_FOLLOWUP_CODE=200 \
    "$ROOT/bin/fm-relay-reply.sh" "req-7" --followup "Done, captain - the fix has shipped."); rc=$?
  ...
  pass "fm-relay-reply --followup posts to /connector/followup with the same request-bound body"
}
...
test_reply_followup_409_marker_exits_distinctly() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    ...
    "$ROOT/bin/fm-relay-reply.sh" "req-409-marker" --followup "Late follow-up." 2>"$err"); rc=$?
  ...
  pass "fm-relay-reply maps a followup_unavailable follow-up 409 to exit 9"
}
...
test_reply_followup_409_without_marker_still_exits_distinctly() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    ...
    "$ROOT/bin/fm-relay-reply.sh" "req-409-bare" --followup "Late follow-up." 2>"$err"); rc=$?
  ...
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    ...
    "$ROOT/bin/fm-relay-reply.sh" "req-409-other" --followup "Late follow-up." 2>"$err"); rc=$?
  ...
  pass "fm-relay-reply maps every follow-up 409 to exit 9 even without the marker"
}
...
test_reply_answer_409_is_generic_failure() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    ...
    "$ROOT/bin/fm-relay-reply.sh" "req-answer-409" "Normal answer." 2>"$err"); rc=$?
  ...
  pass "fm-relay-reply treats answer-endpoint 409 as a generic failure"
}
...
test_reply_followup_image_live_posts_image_object() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    FM_RELAY_REQUEST_PLATFORM=discord FM_RELAY_REQUEST_REPLY_MAX_CHARS=280 \
    FAKE_CURL_LOG="$log" FAKE_FOLLOWUP_CODE=200 \
    "$ROOT/bin/fm-relay-reply.sh" "req-fu-img" --followup --image "$img" \
  ...
  pass "fm-relay-reply --followup --image posts an image object"
}
...
test_reply_followup_flag_position_is_flexible() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    FM_RELAY_REQUEST_PLATFORM=discord FM_RELAY_REQUEST_REPLY_MAX_CHARS=280 \
    FAKE_CURL_LOG="$log" FAKE_FOLLOWUP_CODE=200 \
    "$ROOT/bin/fm-relay-reply.sh" "req-a" --text-file "$home/reply.txt" --followup); rc=$?
  ...
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    FAKE_CURL_LOG="$log" FAKE_ANSWER_CODE=200 \
    "$ROOT/bin/fm-relay-reply.sh" "req-a" --text-file "$home/reply.txt"); rc=$?
  ...
  pass "fm-relay-reply --followup is accepted in any position and leaves the answer path default"
}

test_reply_followup_dry_run_marks_endpoint() {
  local home out rc
  home="$TMP_ROOT/reply-followup-dry"; mkdir -p "$home"
  out=$(FM_HOME="$home" FM_RELAY_DRY_RUN=1 FM_RELAY_REQUEST_PLATFORM=discord FM_RELAY_REQUEST_REPLY_MAX_CHARS=280 \
    "$ROOT/bin/fm-relay-reply.sh" "req-d" --followup "Shipped - all green." 2>"$home/err"); rc=$?
  expect_code 0 "$rc" "followup dry-run exit"
  [ "$out" = "req-d" ] || fail "followup dry-run must echo the request_id (got: $out)"
  assert_present "$home/state/relay-outbox/req-d.json" "followup dry-run must record the preview"
  [ "$(jq -r '.endpoint' "$home/state/relay-outbox/req-d.json")" = "followup" ] \
    || fail "followup dry-run preview must carry the endpoint marker"
  [ "$(jq -r '.text' "$home/state/relay-outbox/req-d.json")" = "Shipped - all green." ] \
  ...
  out=$(FM_HOME="$home" FM_RELAY_DRY_RUN=1 "$ROOT/bin/fm-relay-reply.sh" "req-ans" "Aye." 2>/dev/null)
  jq -e 'has("endpoint")|not' "$home/state/relay-outbox/req-ans.json" >/dev/null \
    || fail "an answer dry-run preview must not gain an endpoint marker"
  pass "fm-relay-reply --followup dry-run marks the endpoint without changing the answer path"
}
...
test_reply_followup_thread_dry_run() {
  ...
  # fully resolved Discord follow-up.
  out=$(FM_HOME="$home" FM_RELAY_DRY_RUN=1 FM_RELAY_REPLY_MAX_CHARS=50 \
    FM_RELAY_REQUEST_PLATFORM=discord FM_RELAY_REQUEST_REPLY_MAX_CHARS=50 \
    "$ROOT/bin/fm-relay-reply.sh" req-ft --followup "$long" 2>/dev/null)
  [ "$out" = "req-ft" ] || fail "followup thread dry-run must echo the request_id (got: $out)"
  jq -e '.texts and (.texts|length>1)' "$home/state/relay-outbox/req-ft.json" >/dev/null \
    || fail "a long followup must record a texts[] thread"
  [ "$(jq -r '.endpoint' "$home/state/relay-outbox/req-ft.json")" = "followup" ] \
    || fail "followup thread preview must carry the endpoint marker"
  [ "$(jq -r '.text' "$home/state/relay-outbox/req-ft.json")" = "$(jq -r '.texts[0]' "$home/state/relay-outbox/req-ft.json")" ] \
    || fail "followup thread text must equal the first chunk"
  pass "fm-relay-reply --followup auto-splits a long follow-up into a marked thread"
}
...
test_reply_followup_image_dry_run_marks_endpoint_and_compacts_image() {
  ...
  make_sample_image "$img"
  out=$(FM_HOME="$home" FM_RELAY_DRY_RUN=1 FM_RELAY_REQUEST_PLATFORM=discord FM_RELAY_REQUEST_REPLY_MAX_CHARS=280 \
    "$ROOT/bin/fm-relay-reply.sh" "req-fu-img-dry" --followup --image "$img" "Done with art." \
    2>"$home/err"); rc=$?
  ...
  [ "$(jq -r '.endpoint' "$home/state/relay-outbox/req-fu-img-dry.json")" = "followup" ] \
    || fail "followup image dry-run must carry endpoint marker"
  [ "$(jq -r '.image.media_type' "$home/state/relay-outbox/req-fu-img-dry.json")" = "image/gif" ] \
    || fail "followup image dry-run must detect GIF media_type"
  jq -e '.image | has("data_base64") | not' "$home/state/relay-outbox/req-fu-img-dry.json" >/dev/null \
    || fail "followup image dry-run must omit base64 bytes"
  pass "fm-relay-reply followup dry-run keeps endpoint marker and compact image metadata"
}
...
# link, because one persistent secondmate's single relay_request slot was already
# taken by a concurrent request) after the inbox was drained could lose its
# Discord budget. The poll records a durable per-request reply-context registry,
# and fm-relay-reply resolves
...
test_poll_records_context_registry_from_relay_platform() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    FM_RELAY_NOW_OVERRIDE=1700000000 \
    FAKE_POLL_CODE=200 FAKE_POLL_BODY="$body" "$ROOT/bin/fm-relay-poll.sh"); rc=$?
  expect_code 0 "$rc" "poll discord registry exit"
  [ "$out" = "relay-mention req-disc" ] || fail "poll must still print the wake marker (got: $out)"
  reg="$home/state/relay-context/req-disc.json"
  ...
  # A smaller explicit Discord budget remains authoritative for that request.
  body=$(jq -cn '{request_id:"req-r",platform:"discord",reply_max_chars:280,text:"question from Discord"}')
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    FAKE_POLL_CODE=200 FAKE_POLL_BODY="$body" "$ROOT/bin/fm-relay-poll.sh"); rc=$?
  expect_code 0 "$rc" "poll Discord registry exit"
  [ "$(jq -r .platform "$home/state/relay-context/req-r.json")" = "discord" ] \
    || fail "registry must capture the explicit Discord platform"
  # A mention with no platform signal is rejected before it reaches the inbox.
  body=$(jq -cn '{request_id:"req-unk",text:"platformless question"}')
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    FAKE_POLL_CODE=200 FAKE_POLL_BODY="$body" "$ROOT/bin/fm-relay-poll.sh"); rc=$?
  expect_code 0 "$rc" "poll unknown-platform exit"
  [ "$out" = "relay-error relay payload did not identify Discord explicitly" ] \
    || fail "an unknown-platform mention must report the bounded Relay diagnostic"
  assert_absent "$home/state/relay-inbox/req-unk.json" \
    "an unknown-platform mention must not be stashed"
  assert_absent "$home/state/relay-context/req-unk.json" \
    "an unknown-platform mention must not create a context record"
  pass "fm-relay-poll records the durable per-request reply context from the relay payload"
}
...
test_context_registry_private_publication_rejects_unsafe_paths() {
  ...
  ln -s "$home/external" "$home/state/relay-context"
  fm_relay_context_registry_set "$home/state" req-r discord 280; rc=$?
  [ "$rc" -ne 0 ] || fail "context registry must reject a linked context directory"
  assert_absent "$home/external/req-r.json" "context registry must not write through a linked context directory"
  [ -L "$home/state/relay-context" ] || fail "context registry must leave the rejected directory symlink in place"

  home="$TMP_ROOT/context-public-dir"; mkdir -p "$home/state/relay-context"
  chmod 755 "$home/state/relay-context"
  fm_relay_context_registry_set "$home/state" req-r discord 280; rc=$?
  [ "$rc" -ne 0 ] || fail "context registry must reject a nonprivate context directory"
  assert_absent "$home/state/relay-context/req-r.json" "context registry must not publish into a nonprivate context directory"
  assert_no_private_artifact_temps "$home/state/relay-context"

  home="$TMP_ROOT/context-linked-dest"; mkdir -p "$home/state/relay-context"
  chmod 700 "$home/state/relay-context"
  target="$home/external-target.json"
  printf 'external sentinel\n' > "$target"
  ln -s "$target" "$home/state/relay-context/req-r.json"
  fm_relay_context_registry_set "$home/state" req-r discord 280; rc=$?
  ...
  [ -L "$home/state/relay-context/req-r.json" ] || fail "context registry must not replace a rejected linked destination"
  assert_no_private_artifact_temps "$home/state/relay-context"

  home="$TMP_ROOT/context-hardlink-dest"; mkdir -p "$home/state/relay-context"
  chmod 700 "$home/state/relay-context"
  dest="$home/state/relay-context/req-r.json"
  hardlink="$home/state/relay-context/req-r.alias"
  jq -cn '{request_id:"req-r",platform:"discord",reply_max_chars:"280",recorded_at:1700000000}' > "$dest"
  chmod 600 "$dest"
  ln "$dest" "$hardlink"
  fm_relay_context_registry_set "$home/state" req-r discord 1900; rc=$?
  [ "$rc" -ne 0 ] || fail "context registry must reject a hardlinked destination"
  [ "$(jq -r .platform "$dest")" = "discord" ] || fail "context registry must preserve a rejected hardlinked destination"
  [ "$(jq -r .platform "$hardlink")" = "discord" ] || fail "context registry must preserve the hardlink peer"
  assert_no_private_artifact_temps "$home/state/relay-context"

  home="$TMP_ROOT/context-wrong-mode-dest"; mkdir -p "$home/state/relay-context"
  chmod 700 "$home/state/relay-context"
  dest="$home/state/relay-context/req-r.json"
  jq -cn '{request_id:"req-r",platform:"discord",reply_max_chars:"280",recorded_at:1700000000}' > "$dest"
  chmod 644 "$dest"
  ...
  [ "$(jq -r .platform "$dest")" = "discord" ] || fail "context registry must preserve a rejected wrong-mode destination"
  ...
  out=$(FM_RELAY_NOW_OVERRIDE=1700000000 bash -c '. "$1/bin/fm-relay-lib.sh"; fm_relay_context_registry_set "$2/state" req-r discord 280' _ "$ROOT" "$home")
  rc=$?
  expect_code 0 "$rc" "context private publication success"
  [ -z "$out" ] || fail "context registry setter must stay silent on success"
  [ "$(path_mode "$home/state/relay-context")" = 700 ] || fail "context registry must create the context directory as private"
  [ "$(path_mode "$home/state/relay-context/req-r.json")" = 600 ] || fail "context registry must publish the context file as private"
  assert_no_private_artifact_temps "$home/state/relay-context"
...
test_context_registry_rejects_unsafe_reads() {
  ...
  ln -s "$home/external-context" "$home/state/relay-context"
  out=$(fm_relay_context_registry_get "$home/state" req-linked-dir)
  [ "$(printf '%s' "$out" | jq -r .platform)" = "" ] \
    || fail "context registry must not read through a linked context directory"

  home="$TMP_ROOT/context-read-linked-file"; private_artifact_dir "$home/state/relay-context"
  ...
  ln -s "$target" "$home/state/relay-context/req-linked-file.json"
  out=$(fm_relay_context_registry_get "$home/state" req-linked-file)
  [ "$(printf '%s' "$out" | jq -r .platform)" = "" ] \
    || fail "context registry must not read through a linked context file"
  [ -L "$home/state/relay-context/req-linked-file.json" ] \
    || fail "context registry must not replace a rejected linked context file"

  home="$TMP_ROOT/context-read-hardlink-file"; private_artifact_dir "$home/state/relay-context"
  dest="$home/state/relay-context/req-hardlink.json"
  hardlink="$home/state/relay-context/req-hardlink.alias"
  jq -cn '{request_id:"req-hardlink",platform:"discord",reply_max_chars:"1900",recorded_at:1700000000}' > "$dest"
  private_artifact_file "$dest"
  ln "$dest" "$hardlink"
  out=$(fm_relay_context_registry_get "$home/state" req-hardlink)
  [ "$(printf '%s' "$out" | jq -r .platform)" = "" ] \
  ...
  home="$TMP_ROOT/context-read-wrong-mode-file"; private_artifact_dir "$home/state/relay-context"
  dest="$home/state/relay-context/req-mode.json"
  jq -cn '{request_id:"req-mode",platform:"discord",reply_max_chars:"1900",recorded_at:1700000000}' > "$dest"
  chmod 644 "$dest"
  out=$(fm_relay_context_registry_get "$home/state" req-mode)
  [ "$(printf '%s' "$out" | jq -r .platform)" = "" ] \
...
test_private_artifact_publisher_runs_under_system_bash() {
  ...
  out=$(/bin/bash -c \
    '. "$1/bin/fm-relay-lib.sh"; printf "%s\n" "{\"request_id\":\"req-bash\"}" | fm_relay_private_artifact_publish_stdin "$2/state/relay-outbox" req-bash.json 600' \
    _ "$ROOT" "$home"); rc=$?
  ...
  assert_present "$home/state/relay-outbox/req-bash.json" "private artifact publisher must create the artifact under /bin/bash"
  [ "$(path_mode "$home/state/relay-outbox/req-bash.json")" = 600 ] \
...
test_context_registry_prunes_expired_records() {
  ...
  jq -cn '{request_id:"req-expired",platform:"discord",reply_max_chars:"280",recorded_at:1699395199}' \
    > "$dir/req-expired.json"
  jq -cn '{request_id:"req-keep",platform:"discord",reply_max_chars:"1900",recorded_at:1699395200}' \
    > "$keep"
  jq -cn '{request_id:"req-iP49shRy-8ue4dtxEo87Yw",platform:"discord",reply_max_chars:"280",recorded_at:1700000000}' \
    > "$preserved"
  jq -cn '{request_id:"req-legacy",platform:"discord",reply_max_chars:"1900"}' > "$legacy"
  printf '{not-json\n' > "$malformed"
  jq -cn '{request_id:"req-future",platform:"discord",reply_max_chars:"280",recorded_at:"9999999999999999999"}' \
    > "$future"
  chmod 600 "$dir/"*.json
  touch -t 202001010000 "$legacy" "$malformed" "$future"
  out=$(FM_RELAY_NOW_OVERRIDE=1700000000 bash -c \
    '. "$1/bin/fm-relay-lib.sh"; fm_relay_context_registry_get "$2" req-keep' _ "$ROOT" "$home/state")
  ...
  jq -cn '{request_id:"req-poll-expired",platform:"discord",reply_max_chars:"280",recorded_at:1699395199}' \
    > "$dir/req-poll-expired.json"
  private_artifact_file "$dir/req-poll-expired.json"
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_NOW_OVERRIDE=1700000000 \
    FM_RELAY_URL="https://relay.test" FAKE_POLL_CODE=204 "$ROOT/bin/fm-relay-poll.sh"); rc=$?
  ...
  jq -cn '{request_id:"req-short-window",platform:"discord",reply_max_chars:"280",recorded_at:1699999899}' \
    > "$dir/req-short-window.json"
  private_artifact_file "$dir/req-short-window.json"
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_NOW_OVERRIDE=1700000000 \
    FM_RELAY_FOLLOWUP_MAX_AGE_SECS=100 FM_RELAY_URL="https://relay.test" FAKE_POLL_CODE=204 \
    "$ROOT/bin/fm-relay-poll.sh"); rc=$?
  ...
  jq -cn '{request_id:"req-overlong-window",platform:"discord",reply_max_chars:"280",recorded_at:1699395199}' \
    > "$dir/req-overlong-window.json"
  private_artifact_file "$dir/req-overlong-window.json"
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_NOW_OVERRIDE=1700000000 \
    FM_RELAY_FOLLOWUP_MAX_AGE_SECS=999999999 FM_RELAY_URL="https://relay.test" FAKE_POLL_CODE=204 \
    "$ROOT/bin/fm-relay-poll.sh"); rc=$?
...
test_context_registry_preserves_first_seen_timestamp() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_NOW_OVERRIDE=1700000000 \
    FM_RELAY_URL="https://relay.test" FAKE_POLL_CODE=200 \
    FAKE_POLL_BODY='{"request_id":"req-repeat","platform":"discord","reply_max_chars":280,"text":"q"}' \
    "$ROOT/bin/fm-relay-poll.sh"); rc=$?
  expect_code 0 "$rc" "first registry poll exit"
  reg="$home/state/relay-context/req-repeat.json"
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_NOW_OVERRIDE=1700000100 \
    FM_RELAY_URL="https://relay.test" FAKE_POLL_CODE=200 \
    FAKE_POLL_BODY='{"request_id":"req-repeat","platform":"discord","reply_max_chars":280,"text":"q"}' \
    "$ROOT/bin/fm-relay-poll.sh"); rc=$?
...
test_context_registry_retention_starts_on_successful_live_answer() {
  ...
  reg="$home/state/relay-context/req-answer-window.json"
  [ "$(jq -r .recorded_at "$reg")" = "1700000000" ] \
  ...
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_NOW_OVERRIDE=1700000200 \
    FM_RELAY_URL="https://relay.test" FAKE_ANSWER_CODE=500 \
    "$ROOT/bin/fm-relay-reply.sh" req-answer-window "Working on it." 2>/dev/null); rc=$?
  [ "$rc" -ne 0 ] || fail "the failed answer fixture must fail"
  [ "$(jq -r .recorded_at "$reg")" = "1700000000" ] \
    || fail "a failed answer must not refresh context retention"
  out=$(FM_HOME="$home" FM_RELAY_NOW_OVERRIDE=1700000300 FM_RELAY_DRY_RUN=1 \
    "$ROOT/bin/fm-relay-reply.sh" req-answer-window "Working on it." 2>/dev/null); rc=$?
  expect_code 0 "$rc" "answer-window dry-run exit"
  [ "$(jq -r .recorded_at "$reg")" = "1700000000" ] \
    || fail "an answer dry-run must not refresh context retention"
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_NOW_OVERRIDE=1700604900 \
    FM_RELAY_URL="https://relay.test" FAKE_ANSWER_CODE=200 \
    "$ROOT/bin/fm-relay-reply.sh" req-answer-window "Working on it."); rc=$?
  ...
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_NOW_OVERRIDE=1700605000 \
    FM_RELAY_URL="https://relay.test" FAKE_FOLLOWUP_CODE=200 \
    "$ROOT/bin/fm-relay-reply.sh" req-answer-window --followup "Still working."); rc=$?
...
test_regression_discord_followup_survives_inbox_cleanup() {
  ...
  # 2. The acknowledgement drains the inbox file (relay-respond step 2f).
  rm -f "$home/state/relay-inbox/req-disc.json"
  # 3. The delayed milestone follow-up is posted DIRECTLY by request_id, with no
  #    task link at all - the exact path that regressed.
  reply=$(cat <<'TXT'
Aye captain, the sign-in redirect is patched and up for review. The fix restores the callback path that was dropping the return URL, adds a regression guard so it cannot silently break again, and keeps the existing session handling untouched. This message deliberately is long enough to prove a Discord follow-up stays one message after the inbox is gone.
TXT
)
  [ "$(printf '%s' "$reply" | wc -m | tr -d '[:space:]')" -gt 280 ] \
    || fail "the regression reply must exceed the Discord 280-char budget to be meaningful"
  out=$(FM_HOME="$home" FM_RELAY_DRY_RUN=1 "$ROOT/bin/fm-relay-reply.sh" req-disc --followup - <<<"$reply" 2>/dev/null); rc=$?
  expect_code 0 "$rc" "delayed discord follow-up exit"
  [ "$out" = "req-disc" ] || fail "follow-up must echo the request_id (got: $out)"
  jq -e 'has("texts")|not' "$home/state/relay-outbox/req-disc.json" >/dev/null \
...
# Regression case 2: a Discord follow-up >280 still splits correctly (not broken).
# Regression case 3: when the platform/budget cannot be authoritatively
# determined, a splitting follow-up is REFUSED (fail-safe) - never a silent Discord
...
test_regression_unresolved_followup_fails_safe() {
  ...
  err="$home/err.txt"
  out=$(FM_HOME="$home" FM_RELAY_DRY_RUN=1 "$ROOT/bin/fm-relay-reply.sh" req-none --followup - <<<"$reply" 2>"$err"); rc=$?
  ...
  assert_absent "$home/state/relay-outbox/req-none.json" "a refused follow-up must record NO outbox preview"
  ...
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    ...
    "$ROOT/bin/fm-relay-reply.sh" req-live-none --followup - <<<"$reply" 2>"$err"); rc=$?
...
test_followup_partial_registry_uses_relay_budget_live() {
  ...
  home="$TMP_ROOT/reg-relay-fallback"; private_artifact_dir "$home/state/relay-context"
  fakebin=$(make_fake_curl "$home")
  log="$home/curl.log"
  printf 'FMX_PAIRING_TOKEN=tok-rf\n' > "$home/.env"
  jq -cn '{request_id:"req-relay",platform:"discord",reply_max_chars:""}' \
    > "$home/state/relay-context/req-relay.json"
  private_artifact_file "$home/state/relay-context/req-relay.json"
  reply=$(cat <<'TXT'
Aye captain, that one is shipped and green. The change is landed, the regression guard is in place, and nothing else was disturbed along the way. This confirmation deliberately runs past a single Discord message so it proves the relay-recovered Discord budget keeps it one message.
TXT
)
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    FAKE_CURL_LOG="$log" FAKE_REQCTX_CODE=200 FAKE_REQCTX_BODY='{"reply_max_chars":1900}' \
    FAKE_FOLLOWUP_CODE=200 \
    "$ROOT/bin/fm-relay-reply.sh" req-relay --followup - <<<"$reply"); rc=$?
...
# single relay_request per task was.
test_dismiss_clears_context_registry() {
  local home out rc reg
  home="$TMP_ROOT/dismiss-clears-registry"; private_artifact_dir "$home/state/relay-context"
  reg="$home/state/relay-context/req-dis.json"
  ...
  out=$(FM_HOME="$home" FM_RELAY_DRY_RUN=1 "$ROOT/bin/fm-relay-dismiss.sh" req-dis 2>/dev/null); rc=$?
  ...
  pass "fm-relay-dismiss clears the durable per-request context (a dismissed mention gets no follow-up)"
}

# --- fm-relay-dismiss: drop a mention at the relay without replying ---------------
...
test_dismiss_success_posts_request_only() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    ...
    "$ROOT/bin/fm-relay-dismiss.sh" "req-9"); rc=$?
  ...
  # The body must be exactly {request_id} - no text, no platform message id.
  data=$(grep '^data=' "$log" | tail -1 | sed 's/^data=//')
  ...
  pass "fm-relay-dismiss posts a request-bound dismiss and echoes only the request_id"
}
...
test_dismiss_dry_run_records_not_posts() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    FM_RELAY_DRY_RUN=1 FAKE_CURL_LOG="$log" \
    "$ROOT/bin/fm-relay-dismiss.sh" "req-1" 2>"$home/err"); rc=$?
  ...
  assert_present "$home/state/relay-outbox/req-1.json" "dry-run dismiss must record the would-be body"
  [ "$(jq -r .request_id "$home/state/relay-outbox/req-1.json")" = "req-1" ] \
    || fail "dismiss outbox record must hold the request_id"
  [ "$(jq -r '.endpoint' "$home/state/relay-outbox/req-1.json")" = "dismiss" ] \
  ...
  pass "fm-relay-dismiss dry-run records the would-be body and never posts"
}
...
test_dismiss_dry_run_needs_no_token() {
  ...
  # No token at all: dry-run still previews (it neither authenticates nor posts).
  out=$(PATH="$BASE_PATH" FM_HOME="$home" FM_RELAY_DRY_RUN=1 \
    "$ROOT/bin/fm-relay-dismiss.sh" "req-2" 2>/dev/null); rc=$?
  ...
  assert_present "$home/state/relay-outbox/req-2.json" "dry-run dismiss without a token must still record the preview"
  pass "fm-relay-dismiss dry-run works without a token"
}
...
test_dismiss_dry_run_outbox_private_publication_rejects_unsafe_paths() {
  ...
  err="$home/err.txt"
  ln -s "$home/external" "$home/state/relay-outbox"
  out=$(FM_HOME="$home" FM_RELAY_DRY_RUN=1 "$ROOT/bin/fm-relay-dismiss.sh" req-r 2>"$err"); rc=$?
  ...
  assert_absent "$home/external/req-r.json" "dismiss dry-run must not write through a linked outbox directory"

  home="$TMP_ROOT/dismiss-outbox-linked-dest"; mkdir -p "$home/state/relay-outbox"
  err="$home/err.txt"
  chmod 700 "$home/state/relay-outbox"
  target="$home/external-target.json"
  printf 'external sentinel\n' > "$target"
  ...
  [ -L "$home/state/relay-outbox/req-r.json" ] || fail "dismiss dry-run must not replace a rejected linked destination"
  assert_no_private_artifact_temps "$home/state/relay-outbox"

  home="$TMP_ROOT/dismiss-outbox-private-success"; mkdir -p "$home"
  out=$(FM_HOME="$home" FM_RELAY_DRY_RUN=1 "$ROOT/bin/fm-relay-dismiss.sh" req-r 2>/dev/null); rc=$?
  expect_code 0 "$rc" "dismiss private outbox success exit"
  [ "$out" = "req-r" ] || fail "dismiss dry-run must still echo the request_id after private publication (got: $out)"
  [ "$(path_mode "$home/state/relay-outbox")" = 700 ] || fail "dismiss dry-run must create the outbox directory as private"
  [ "$(path_mode "$home/state/relay-outbox/req-r.json")" = 600 ] || fail "dismiss dry-run must publish the outbox file as private"
  assert_no_private_artifact_temps "$home/state/relay-outbox"
  pass "fm-relay-dismiss dry-run publishes outbox records only through private guarded artifacts"
}
...
test_dismiss_non_2xx_fails() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    ...
    "$ROOT/bin/fm-relay-dismiss.sh" "req-9" 2>"$err"); rc=$?
  ...
  pass "fm-relay-dismiss exits non-zero on a non-2xx relay response"
}
...
SH
  ...
  pass "fm-relay-dismiss exits non-zero on a transport failure"
}
...
test_dismiss_unsafe_request_id_rejected() {
  ...
  out=$(PATH="$BASE_PATH" FM_HOME="$home" FM_RELAY_DRY_RUN=1 \
    "$ROOT/bin/fm-relay-dismiss.sh" "../evil" 2>"$err"); rc=$?
  ...
  pass "fm-relay-dismiss rejects an unsafe request_id (path-traversal guard)"
}

test_dismiss_usage_error() {
  local home rc
  home="$TMP_ROOT/dismiss-usage"; mkdir -p "$home"
  PATH="$BASE_PATH" FM_HOME="$home" "$ROOT/bin/fm-relay-dismiss.sh" >/dev/null 2>&1; rc=$?
  expect_code 2 "$rc" "dismiss missing-arg usage exit"
  PATH="$BASE_PATH" FM_HOME="$home" "$ROOT/bin/fm-relay-dismiss.sh" req-1 extra >/dev/null 2>&1; rc=$?
  expect_code 2 "$rc" "dismiss extra-arg usage exit"
  pass "fm-relay-dismiss rejects missing or extra arguments with a usage error"
}

# --- fm-relay-link: task <-> Relay request association in meta -----------------------
...
test_link_records_request_and_timestamp() {
  ...
  # recording, not platform resolution, so fm-relay-link's no-platform warning to
  # stderr is expected and dropped.
  out=$(FM_HOME="$home" FM_RELAY_NOW_OVERRIDE=1700000000 \
    "$ROOT/bin/fm-relay-link.sh" fix-login-k3 req-42 2>/dev/null); rc=$?
  expect_code 0 "$rc" "link exit"
  assert_grep "relay_request=req-42" "$meta" "link must record the request_id"
  assert_grep "relay_request_ts=1700000000" "$meta" "link must record the timestamp"
  assert_grep "relay_followups=0" "$meta" "a fresh link must start the follow-up counter at 0"
  ...
  FM_HOME="$home" FM_RELAY_NOW_OVERRIDE=1700009999 "$ROOT/bin/fm-relay-link.sh" fix-login-k3 req-99 >/dev/null 2>&1
  [ "$(grep -c '^relay_request=' "$meta")" = "1" ] || fail "re-link must not duplicate relay_request"
  [ "$(grep -c '^relay_request_ts=' "$meta")" = "1" ] || fail "re-link must not duplicate relay_request_ts"
  [ "$(grep -c '^relay_followups=' "$meta")" = "1" ] || fail "re-link must not duplicate relay_followups"
  assert_grep "relay_request=req-99" "$meta" "re-link must replace the request_id"
  assert_grep "relay_request_ts=1700009999" "$meta" "re-link must refresh the timestamp"
  assert_grep "relay_followups=0" "$meta" "a plain re-link must reset the follow-up counter to 0"
  pass "fm-relay-link records and refreshes the Relay request link without disturbing meta"
}

test_link_records_discord_platform_for_followups() {
  local home meta out rc reply
  home="$TMP_ROOT/link-discord-platform"; private_artifact_dir "$home/state/relay-inbox"
  meta="$home/state/fix-discord.meta"
  printf 'window=w\nworktree=/wt\nkind=ship\nmode=no-mistakes\nyolo=off\n' > "$meta"
  jq -cn '{request_id:"req-discord-follow",platform:"discord",reply_max_chars:1900,reply_max_chars:1900,text:"question"}' \
    > "$home/state/relay-inbox/req-discord-follow.json"
  private_artifact_file "$home/state/relay-inbox/req-discord-follow.json"
  FM_HOME="$home" FM_RELAY_NOW_OVERRIDE=1700000000 \
    "$ROOT/bin/fm-relay-link.sh" fix-discord req-discord-follow >/dev/null; rc=$?
  expect_code 0 "$rc" "Discord link exit"
  assert_grep "relay_platform=discord" "$meta" "link must record Discord platform context"
  assert_grep "relay_reply_max_chars=1900" "$meta" "link must record the Discord split budget for follow-ups"
  rm -f "$home/state/relay-inbox/req-discord-follow.json"
  reply=$(cat <<'TXT'
The follow-up is longer than a small test budget but should stay in one Discord message because the linked task meta recorded the platform before the inbox was drained.

```bash
printf '%s\n' "this fenced block should stay whole"
```

The final sentence confirms that the follow-up path did not fall back to the Discord budget after the inbox file disappeared.
TXT
)
  out=$(FM_HOME="$home" FM_RELAY_DRY_RUN=1 FM_RELAY_NOW_OVERRIDE=1700003600 \
    "$ROOT/bin/fm-relay-followup.sh" fix-discord - <<<"$reply" 2>/dev/null); rc=$?
  ...
  jq -e 'has("texts")|not' "$home/state/relay-outbox/req-discord-follow.json" >/dev/null \
    || fail "Discord follow-up below its message budget must not split after inbox drain"
  pass "fm-relay-link records Discord platform context so follow-ups keep the Discord budget"
}
...
# inbox file, so the platform was lost and the splitter defaulted to Discord's 280-char
...
test_link_resolves_platform_by_request_id_after_inbox_cleanup() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    FM_RELAY_NOW_OVERRIDE=1700000000 FAKE_CURL_LOG="$log" \
    FAKE_REQCTX_CODE=200 FAKE_REQCTX_BODY='{"platform":"discord","reply_max_chars":1900}' \
    "$ROOT/bin/fm-relay-link.sh" fix-after-cleanup req-after-cleanup); rc=$?
  ...
  assert_grep "relay_platform=discord" "$meta" "relay lookup must record the Discord platform after inbox cleanup"
  assert_grep "relay_reply_max_chars=1900" "$meta" "relay lookup must record the Discord split budget after inbox cleanup"
  # The follow-up (still with the inbox gone) must post the ~470-char reply as ONE
  # Discord message, not an short-budget thread.
  reply=$(cat <<'TXT'
Aye captain, the sign-in redirect is patched and the change is up for review. The fix restores the callback path that was dropping the return URL, adds a regression guard so it cannot silently break again, and keeps the existing session handling untouched. This message is deliberately long enough for the test to prove a Discord follow-up stays in one message instead of splitting into a numbered thread.
TXT
)
  out=$(FM_HOME="$home" FM_RELAY_DRY_RUN=1 FM_RELAY_NOW_OVERRIDE=1700003600 \
    "$ROOT/bin/fm-relay-followup.sh" fix-after-cleanup - <<<"$reply" 2>/dev/null); rc=$?
  ...
  [ "$(printf '%s' "$reply" | wc -m | tr -d '[:space:]')" -gt 280 ] \
    || fail "the regression reply must exceed the Discord 280-char budget to be meaningful"
  jq -e 'has("texts")|not' "$home/state/relay-outbox/req-after-cleanup.json" >/dev/null \
    || fail "a >280 <2000 Discord follow-up must post as ONE message even when linked after inbox cleanup"
  pass "fm-relay-link resolves the platform by request_id so a post-cleanup link keeps the Discord budget"
}

# Criterion 2 loud-warning branch: when the inbox and relay cannot resolve both
# axes, the link is still recorded but fm-relay-link warns loudly and every follow-up
# is refused.
test_link_warns_loudly_when_platform_unresolvable() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    FM_RELAY_NOW_OVERRIDE=1700000000 FAKE_REQCTX_CODE=404 \
    "$ROOT/bin/fm-relay-link.sh" fix-unresolvable req-unresolvable 2>"$err"); rc=$?
  expect_code 0 "$rc" "link with unresolvable platform still records the link"
  [ "$out" = "linked fix-unresolvable to Relay request req-unresolvable" ] \
    || fail "link must still succeed on stdout even when the platform is unknown (got: $out)"
  assert_grep "WARNING" "$err" "an unresolvable platform must warn loudly, never silently default to another platform"
  assert_grep "req-unresolvable" "$err" "the warning must name the request it could not resolve"
  assert_grep "relay_request=req-unresolvable" "$meta" "the link itself must still be recorded"
  assert_no_grep "relay_platform=" "$meta" "no platform must be recorded when none could be resolved"
  assert_no_grep "relay_reply_max_chars=" "$meta" "no split budget must be recorded when none could be resolved"
  reply="Short follow-up."
  err="$home/fu-err.txt"
  out=$(FM_HOME="$home" FM_RELAY_DRY_RUN=1 FM_RELAY_NOW_OVERRIDE=1700003600 \
    "$ROOT/bin/fm-relay-followup.sh" fix-unresolvable - <<<"$reply" 2>"$err"); rc=$?
  ...
  assert_absent "$home/state/relay-outbox/req-unresolvable.json" \
  ...
  assert_grep "relay_request=req-unresolvable" "$meta" "a held follow-up must leave the link in place to retry"
  pass "fm-relay-link warns loudly and the follow-up is held (not wrongly split) when the platform is unknown"
}
...
test_link_carry_count_and_ts_preserve_followup_binding() {
  ...
  printf 'window=w\nkind=ship\n' > "$meta"
  FM_HOME="$home" FM_RELAY_NOW_OVERRIDE=1700999999 \
    "$ROOT/bin/fm-relay-link.sh" successor-task req-carry \
      --carry-count 2 --carry-ts 1700000000 --carry-platform discord --carry-max 280 >/dev/null; rc=$?
  expect_code 0 "$rc" "link paired carry flags exit"
  assert_grep "relay_request=req-carry" "$meta" "carried link must record the request_id"
  assert_grep "relay_request_ts=1700000000" "$meta" "--carry-ts must preserve the original timestamp, not the current time"
  assert_grep "relay_followups=2" "$meta" "--carry-count must seed the follow-up counter, not reset it"
  assert_grep "relay_platform=discord" "$meta" "--carry-platform must preserve the prior reply platform"
  assert_grep "relay_reply_max_chars=280" "$meta" "--carry-max must preserve the prior split budget"
  pass "fm-relay-link paired carry flags preserve a prior task's follow-up binding onto a successor"
}
...
test_link_recovery_relink_carries_discord_context_after_inbox_drain() {
  ...
  printf 'window=w\nkind=ship\n' > "$meta"
  FM_HOME="$home" FM_RELAY_NOW_OVERRIDE=1700999999 \
    "$ROOT/bin/fm-relay-link.sh" successor-discord req-discord-recovery \
  ...
  assert_grep "relay_platform=discord" "$meta" "Discord recovery relink must preserve the platform after inbox drain"
  assert_grep "relay_reply_max_chars=1900" "$meta" "Discord recovery relink must preserve the split budget after inbox drain"
  reply=$(cat <<'TXT'
The recovered task is reporting back with enough text to exceed a small test budget, but it is still comfortably within the Discord budget carried over from the prior task.
...
TXT
  ...
  out=$(FM_HOME="$home" FM_RELAY_DRY_RUN=1 FM_RELAY_NOW_OVERRIDE=1700003600 \
    "$ROOT/bin/fm-relay-followup.sh" successor-discord - <<<"$reply" 2>/dev/null); rc=$?
  ...
  jq -e 'has("texts")|not' "$home/state/relay-outbox/req-discord-recovery.json" >/dev/null \
    || fail "Discord recovery follow-up below its message budget must not fall back to Discord splitting"
  assert_grep "relay_followups=2" "$meta" "Discord recovery follow-up must increment the carried count"
  pass "fm-relay-link recovery relink preserves Discord platform context after inbox drain"
}
...
test_link_carry_count_validation() {
  ...
  PATH="$BASE_PATH" FM_HOME="$home" \
    "$ROOT/bin/fm-relay-link.sh" ok req-1 --carry-count abc >/dev/null 2>"$err"; rc=$?
  ...
  PATH="$BASE_PATH" FM_HOME="$home" \
    "$ROOT/bin/fm-relay-link.sh" ok req-1 --carry-ts abc >/dev/null 2>"$err"; rc=$?
  ...
  PATH="$BASE_PATH" FM_HOME="$home" \
    "$ROOT/bin/fm-relay-link.sh" ok req-1 --carry-count >/dev/null 2>&1; rc=$?
  expect_code 2 "$rc" "link --carry-count missing value exit"
  PATH="$BASE_PATH" FM_HOME="$home" \
    "$ROOT/bin/fm-relay-link.sh" ok req-1 --carry-count 1 >/dev/null 2>"$err"; rc=$?
  ...
  PATH="$BASE_PATH" FM_HOME="$home" \
    "$ROOT/bin/fm-relay-link.sh" ok req-1 --carry-ts 1700000000 >/dev/null 2>"$err"; rc=$?
  ...
  PATH="$BASE_PATH" FM_HOME="$home" \
    "$ROOT/bin/fm-relay-link.sh" ok req-1 --carry-count 1 --carry-ts 1700000000 >/dev/null 2>"$err"; rc=$?
  ...
  PATH="$BASE_PATH" FM_HOME="$home" \
    "$ROOT/bin/fm-relay-link.sh" ok req-1 --carry-platform discord >/dev/null 2>"$err"; rc=$?
  ...
  PATH="$BASE_PATH" FM_HOME="$home" \
    "$ROOT/bin/fm-relay-link.sh" ok req-1 --carry-count 1 --carry-ts 1700000000 --carry-max 49 >/dev/null 2>"$err"; rc=$?
  ...
  pass "fm-relay-link rejects malformed or unpaired carry flags"
}
...
test_meta_rewrites_do_not_depend_on_tmpdir() {
  ...
  printf 'window=w\nkind=ship\n' > "$meta"
  out=$(TMPDIR="$badtmp" FM_HOME="$home" FM_RELAY_NOW_OVERRIDE=1700000000 \
    "$ROOT/bin/fm-relay-link.sh" fix-meta-k4 req-local 2>/dev/null); rc=$?
  expect_code 0 "$rc" "link with unusable TMPDIR exit"
  [ "$out" = "linked fix-meta-k4 to Relay request req-local" ] \
    || fail "link with unusable TMPDIR must still succeed (got: $out)"
  assert_grep "relay_request=req-local" "$meta" "link must record request with an unusable TMPDIR"
  out=$(TMPDIR="$badtmp" FM_HOME="$home" FM_RELAY_NOW_OVERRIDE=1700000001 FM_RELAY_FOLLOWUP_MAX_AGE_SECS=0 \
    "$ROOT/bin/fm-relay-followup.sh" --check fix-meta-k4 2>/dev/null); rc=$?
  expect_code 1 "$rc" "expired check with unusable TMPDIR exit"
  [ -z "$out" ] || fail "expired check must stay silent (got: $out)"
  assert_no_grep "relay_request=" "$meta" "clear must remove request with an unusable TMPDIR"
  assert_no_grep "relay_followups=" "$meta" "clear must remove the follow-up counter with an unusable TMPDIR"
...
test_link_rejects_unsafe_and_missing() {
  ...
  printf 'kind=ship\n' > "$home/state/ok.meta"
  PATH="$BASE_PATH" FM_HOME="$home" "$ROOT/bin/fm-relay-link.sh" "../evil" req-1 >/dev/null 2>&1; rc=$?
  expect_code 2 "$rc" "link unsafe task id exit"
  PATH="$BASE_PATH" FM_HOME="$home" "$ROOT/bin/fm-relay-link.sh" ok "../../etc/x" >/dev/null 2>&1; rc=$?
  ...
  PATH="$BASE_PATH" FM_HOME="$home" "$ROOT/bin/fm-relay-link.sh" no-such req-1 >/dev/null 2>&1; rc=$?
  ...
  PATH="$BASE_PATH" FM_HOME="$home" "$ROOT/bin/fm-relay-link.sh" ok >/dev/null 2>&1; rc=$?
  expect_code 2 "$rc" "link missing arg exit"
  pass "fm-relay-link rejects unsafe ids, missing meta, and missing arguments"
}

# A home with no second mates at all learns nothing from the registry, so its
...
test_link_missing_task_without_secondmates_stays_plain() {
  ...
  PATH="$BASE_PATH" FM_HOME="$home" "$ROOT/bin/fm-relay-link.sh" no-such req-1 >/dev/null 2>"$err"; rc=$?
  ...
  pass "fm-relay-link keeps the plain missing-task error when no second mate is registered"
}
...
# public promise silently orphaned; the refusal must name the second mate holding
...
test_link_refuses_secondmate_routed_task_with_promised_final_pointer() {
  ...
  out=$(PATH="$BASE_PATH" FM_HOME="$main" "$ROOT/bin/fm-relay-link.sh" routed-k1 req-routed 2>"$err"); rc=$?
  ...
  assert_no_grep "relay_request=" "$sub/state/routed-k1.meta" \
  ...
  out=$(PATH="$BASE_PATH" FM_HOME="$main" FM_RELAY_NOW_OVERRIDE=1700000000 \
    "$ROOT/bin/fm-relay-link.sh" local-k1 req-local \
      --carry-count 0 --carry-ts 1700000000 --carry-platform discord --carry-max 1900 2>/dev/null); rc=$?
  expect_code 0 "$rc" "local link exit with second mates registered"
  assert_grep "relay_request=req-local" "$main/state/local-k1.meta" \
    "a local task must still link while second mates are registered"
  pass "fm-relay-link refuses a second-mate-routed task and points at the promised-final path"
}

# A remote second mate's home cannot be inspected from here, and neither can a
...
test_link_missing_task_with_secondmates_points_at_promised_final() {
  ...
  PATH="$BASE_PATH" FM_HOME="$home" "$ROOT/bin/fm-relay-link.sh" unknown-k1 req-unknown >/dev/null 2>"$err"; rc=$?
  ...
  pass "fm-relay-link points an unlocatable task at the promised-final path when second mates exist"
}

# --- fm-relay-followup: detect, post up to 3 follow-ups, manage the link --------
...
mk_linked_task() { # <home> <id> <request_id> <link-epoch> [starting-count]
  ...
  printf 'window=w\nworktree=/wt\nkind=ship\nmode=no-mistakes\nyolo=off\n' > "$meta"
  FM_HOME="$home" FM_RELAY_NOW_OVERRIDE="$ts" "$ROOT/bin/fm-relay-link.sh" "$id" "$rid" \
    --carry-count "${count:-0}" --carry-ts "$ts" --carry-platform discord --carry-max 280 >/dev/null
}
...
test_followup_check_states() {
  ...
  out=$(FM_HOME="$home" FM_RELAY_NOW_OVERRIDE=1700003600 \
    "$ROOT/bin/fm-relay-followup.sh" --check task-a); rc=$?
  ...
  out=$(FM_HOME="$home" "$ROOT/bin/fm-relay-followup.sh" --check plain 2>/dev/null); rc=$?
  ...
  out=$(FM_HOME="$home" "$ROOT/bin/fm-relay-followup.sh" --check nope 2>/dev/null); rc=$?
  expect_code 1 "$rc" "check missing-meta exit"
  pass "fm-relay-followup --check reports postable / not-linked correctly"
}
...
test_followup_check_expired_prunes_link() {
  ...
  out=$(FM_HOME="$home" FM_RELAY_NOW_OVERRIDE=$((1700000000 + 8*86400)) \
    "$ROOT/bin/fm-relay-followup.sh" --check task-e 2>/dev/null); rc=$?
  expect_code 1 "$rc" "check expired exit"
  [ -z "$out" ] || fail "check on an expired link must be silent (got: $out)"
  assert_no_grep "relay_request=" "$meta" "expired check must prune the link"
  assert_grep "kind=ship" "$meta" "expired check must preserve other meta lines"
  pass "fm-relay-followup --check prunes a link past the 7-day window"
}
...
test_followup_check_cap_reached_prunes_link() {
  ...
  out=$(FM_HOME="$home" FM_RELAY_NOW_OVERRIDE=1700003600 \
    "$ROOT/bin/fm-relay-followup.sh" --check task-cap 2>/dev/null); rc=$?
  expect_code 1 "$rc" "check cap-reached exit"
  [ -z "$out" ] || fail "check at the cap must be silent (got: $out)"
  assert_no_grep "relay_request=" "$meta" "a cap-reached check must prune the link"
  assert_grep "kind=ship" "$meta" "cap-reached check must preserve other meta lines"
  pass "fm-relay-followup --check prunes a link that already reached the follow-up cap"
}
...
test_followup_post_increments_counter_keeps_link() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    FM_RELAY_NOW_OVERRIDE=1700003600 FAKE_CURL_LOG="$log" FAKE_FOLLOWUP_CODE=200 \
    "$ROOT/bin/fm-relay-followup.sh" task-p --text-file "$home/reply.txt"); rc=$?
  ...
  assert_grep "relay_request=req-p" "$meta" "a post under the cap must keep the link"
  assert_grep "relay_followups=1" "$meta" "a successful post must increment the follow-up counter"
  assert_grep "kind=ship" "$meta" "posting must preserve other meta lines"
  pass "fm-relay-followup posts a follow-up, increments the counter, and keeps the link under the cap"
}
...
test_followup_post_final_clears_link_immediately() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    FM_RELAY_NOW_OVERRIDE=1700003600 FAKE_FOLLOWUP_CODE=200 \
    "$ROOT/bin/fm-relay-followup.sh" task-final --final - <<<"Shipped - all green."); rc=$?
  expect_code 0 "$rc" "followup --final post exit"
  [ "$out" = "req-final" ] || fail "followup --final post must echo the request_id (got: $out)"
  assert_no_grep "relay_request=" "$meta" "--final must clear the link even with follow-ups remaining under the cap"
  assert_grep "kind=ship" "$meta" "clearing the link must preserve other meta lines"
  pass "fm-relay-followup --final clears the link after one post regardless of the remaining count"
}
...
test_followup_post_cap_reached_clears_link() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    FM_RELAY_NOW_OVERRIDE=1700003600 FAKE_FOLLOWUP_CODE=200 \
    "$ROOT/bin/fm-relay-followup.sh" task-cap3 - <<<"Third and final update."); rc=$?
  ...
  assert_no_grep "relay_request=" "$meta" "reaching the cap must clear the link even without --final"
  pass "fm-relay-followup clears the link once the third follow-up reaches the cap"
}
...
test_followup_post_forwards_image_to_reply_client() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    FM_RELAY_NOW_OVERRIDE=1700003600 FAKE_CURL_LOG="$log" FAKE_FOLLOWUP_CODE=200 \
    "$ROOT/bin/fm-relay-followup.sh" task-img --image "$img" --final --text-file "$home/reply.txt"); rc=$?
  ...
  assert_no_grep "relay_request=" "$meta" "a --final image followup must clear the link"
  pass "fm-relay-followup --image forwards the attachment through fm-relay-reply --followup"
}
...
test_followup_post_failure_keeps_link() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    FM_RELAY_NOW_OVERRIDE=1700003600 FAKE_FOLLOWUP_CODE=500 \
    "$ROOT/bin/fm-relay-followup.sh" task-f - <<<"retry me" 2>/dev/null); rc=$?
  [ "$rc" -ne 0 ] || fail "a failed follow-up post must exit non-zero"
  [ -z "$out" ] || fail "a failed post must not echo the request_id (got: $out)"
  assert_grep "relay_request=req-f" "$meta" "a failed post must leave the link for a retry"
  assert_grep "relay_followups=0" "$meta" "a failed post must not increment the follow-up counter"
  pass "fm-relay-followup keeps the link and counter when the post fails"
}
...
SH
  ...
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    FM_RELAY_NOW_OVERRIDE=1700003600 FAKE_FOLLOWUP_CODE=200 FAKE_CURL_TOUCH_AFTER_POST="$flag" \
    FAKE_MV_FAIL_AFTER_FLAG="$flag" FAKE_MV_FAILED_ONCE="$mvflag" \
    "$ROOT/bin/fm-relay-followup.sh" task-rf - <<<"posted but local state write fails" 2>"$err"); rc=$?
  ...
  assert_no_grep "relay_request=" "$meta" "a failed counter write must tombstone the link"
  assert_no_grep "relay_followups=" "$meta" "a failed counter write must remove the stale counter"
  assert_grep "cleared the link to avoid duplicate follow-ups" "$err" "state-record failure must explain the tombstone"
  pass "fm-relay-followup tombstones the link when a post-success counter write fails"
}
...
test_followup_post_relay_rejection_degrades_gracefully() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    FM_RELAY_NOW_OVERRIDE=1700003600 FAKE_FOLLOWUP_CODE=409 \
    "$ROOT/bin/fm-relay-followup.sh" task-409 - <<<"rejected by relay" 2>"$err"); rc=$?
  expect_code 0 "$rc" "relay-rejected post exit"
  [ -z "$out" ] || fail "a relay-rejected post must echo nothing (got: $out)"
  assert_no_grep "relay_request=" "$meta" "a relay rejection must clear the link"
  assert_grep "cap or window" "$err" "a relay rejection must explain itself distinctly from a generic failure"
  pass "fm-relay-followup treats a relay cap/window rejection as an already-exhausted link, not a retry"
}
...
test_followup_post_expired_skips_and_clears() {
  ...
  fakebin=$(make_fake_curl "$home")
  printf 'FMX_PAIRING_TOKEN=tok-fu\n' > "$home/.env"
  mk_linked_task "$home" task-r req-r 1700000000
  meta="$home/state/task-r.meta"
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_RELAY_URL="https://relay.test" \
    FM_RELAY_NOW_OVERRIDE=$((1700000000 + 8*86400)) FAKE_FOLLOWUP_CODE=200 \
    "$ROOT/bin/fm-relay-followup.sh" task-r - <<<"too late" 2>/dev/null); rc=$?
  expect_code 0 "$rc" "expired post exit"
  [ -z "$out" ] || fail "an expired post must post nothing and echo nothing (got: $out)"
  assert_no_grep "relay_request=" "$meta" "an expired post must clear the link"
  assert_absent "$home/state/relay-outbox/req-r.json" "an expired post must not record any reply"
  pass "fm-relay-followup skips silently and clears the link past the 7-day window"
}
...
test_followup_post_not_linked_is_noop() {
  ...
  printf 'kind=ship\n' > "$home/state/plain.meta"
  out=$(FM_HOME="$home" "$ROOT/bin/fm-relay-followup.sh" plain - <<<"nothing to do" 2>/dev/null); rc=$?
  expect_code 0 "$rc" "not-linked post exit"
  [ -z "$out" ] || fail "a not-linked post must be a silent no-op (got: $out)"
  assert_absent "$home/state/relay-outbox" "a not-linked post must not record a reply"
  pass "fm-relay-followup is a no-op for a task with no Relay link"
}
...
test_followup_post_dry_run_increments_counter_keeps_link() {
  ...
  out=$(FM_HOME="$home" FM_RELAY_DRY_RUN=1 FM_RELAY_NOW_OVERRIDE=1700003600 \
    "$ROOT/bin/fm-relay-followup.sh" task-d - <<<"Shipped in dry run." 2>/dev/null); rc=$?
  expect_code 0 "$rc" "dry-run post exit"
  [ "$out" = "req-d" ] || fail "dry-run post must echo the request_id (got: $out)"
  assert_present "$home/state/relay-outbox/req-d.json" "dry-run post must record the would-be follow-up"
  [ "$(jq -r '.endpoint' "$home/state/relay-outbox/req-d.json")" = "followup" ] \
  ...
  assert_grep "relay_request=req-d" "$meta" "dry-run under the cap must keep the link just as a live post would"
  assert_grep "relay_followups=1" "$meta" "dry-run must increment the follow-up counter just as a live post would"
  pass "fm-relay-followup dry-run records the follow-up and increments the counter, keeping the link"
}
...
test_followup_post_dry_run_final_clears_link() {
  ...
  out=$(FM_HOME="$home" FM_RELAY_DRY_RUN=1 FM_RELAY_NOW_OVERRIDE=1700003600 \
    "$ROOT/bin/fm-relay-followup.sh" task-df --final - <<<"Shipped in dry run, for real this time." 2>/dev/null); rc=$?
  expect_code 0 "$rc" "dry-run --final post exit"
  [ "$out" = "req-df" ] || fail "dry-run --final post must echo the request_id (got: $out)"
  assert_no_grep "relay_request=" "$meta" "dry-run --final must clear the link just as a live --final post would"
  pass "fm-relay-followup dry-run --final clears the link just as a live post would"
}
...
test_followup_usage_errors() {
  ...
  err="$home/err.txt"
  PATH="$BASE_PATH" FM_HOME="$home" "$ROOT/bin/fm-relay-followup.sh" >/dev/null 2>"$err"; rc=$?
  ...
  PATH="$BASE_PATH" FM_HOME="$home" "$ROOT/bin/fm-relay-followup.sh" --check >/dev/null 2>&1; rc=$?
  expect_code 2 "$rc" "followup --check no-id exit"
  PATH="$BASE_PATH" FM_HOME="$home" "$ROOT/bin/fm-relay-followup.sh" some-task >/dev/null 2>&1; rc=$?
  expect_code 2 "$rc" "followup post no-text-source exit"
  out=$(PATH="$BASE_PATH" FM_HOME="$home" "$ROOT/bin/fm-relay-followup.sh" --help); rc=$?
  ...
  PATH="$BASE_PATH" FM_HOME="$home" "$ROOT/bin/fm-relay-followup.sh" "../evil" --text-file /dev/null >/dev/null 2>&1; rc=$?
  expect_code 2 "$rc" "followup unsafe-id exit"
  PATH="$BASE_PATH" FM_HOME="$home" "$ROOT/bin/fm-relay-followup.sh" some-task --image >/dev/null 2>"$err"; rc=$?
  ...
  pass "fm-relay-followup rejects malformed invocations"
}
