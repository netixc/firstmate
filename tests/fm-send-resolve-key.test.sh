...
# Stub Herdr: logs submitted text to FM_FAKE_HERDR_LOG and maintains an empty
# bordered composer after Enter.
# FM_FAKE_HERDR_SEND_FAIL=1 makes text delivery fail so the delivery-failure leg
# can assert that a failed send closes nothing.
...
run_send() {
  ...
  env PATH="$fb:$PATH" \
    ...
    FM_FAKE_HERDR_CAPTURE="$(dirname "$fb")/capture" FM_BACKEND_HERDR_SUBMIT_MIN_SLEEP=0 \
    FM_SEND_SETTLE=0 "$SEND" "$@" 2>/dev/null
}
...
test_answer_send_closes_open_decision() {
  ...
  fm_write_meta "$home/state/t1.meta" "backend=herdr" "window=sess:fm-t1" "harness=pi" "kind=ship"
...
test_answer_starts_work_never_orphans() {
  ...
  fm_write_meta "$home/state/t2.meta" "backend=herdr" "window=sess:fm-t2" "harness=pi" "kind=ship"
...
test_routine_steer_never_closes() {
  ...
  fm_write_meta "$home/state/t3.meta" "backend=herdr" "window=sess:fm-t3" "harness=pi" "kind=ship"
...
test_not_open_key_refuses_before_send() {
  ...
  fm_write_meta "$home/state/t4.meta" "backend=herdr" "window=sess:fm-t4" "harness=pi" "kind=ship"
  ...
  env PATH="$fb:$PATH" \
    ...
    FM_SEND_SETTLE=0 "$SEND" t4 --resolve-key mistyped "the answer" >/dev/null 2>"$err"; rc=$?
...
test_failed_send_does_not_close() {
  ...
  fm_write_meta "$home/state/t5.meta" "backend=herdr" "window=sess:fm-t5" "harness=pi" "kind=ship"
  ...
  env PATH="$fb:$PATH" FM_FAKE_HERDR_SEND_FAIL=1 \
    FM_ROOT_OVERRIDE="$home" FM_HOME="$home" FM_FAKE_HERDR_LOG="$log" \
    FM_FAKE_HERDR_CAPTURE="$dir/capture" FM_BACKEND_HERDR_SUBMIT_MIN_SLEEP=0 \
    FM_SEND_SETTLE=0 "$SEND" t5 --resolve-key creds "token is in the vault now" >/dev/null 2>&1; rc=$?
...
test_multiple_keys_close_together() {
  ...
  fm_write_meta "$home/state/t6.meta" "backend=herdr" "window=sess:fm-t6" "harness=pi" "kind=ship"
  {
...
test_reopened_key_survives_delayed_close() {
  ...
  fm_write_meta "$home/state/t8.meta" "backend=herdr" "window=sess:fm-t8" "harness=pi" "kind=ship"
...
test_flag_misuse_refuses() {
  ...
  fm_write_meta "$home/state/t7.meta" "backend=herdr" "window=sess:fm-t7" "harness=pi" "kind=ship"
  printf 'needs-decision [key=k]: choose\n' > "$home/state/t7.status"

  # --resolve-key with --key (both orders) is refused: an answer is text.
  : > "$log"
  env PATH="$fb:$PATH" FM_ROOT_OVERRIDE="$home" FM_HOME="$home" FM_FAKE_HERDR_LOG="$log" \
    FM_FAKE_HERDR_CAPTURE="$dir/capture" FM_BACKEND_HERDR_SUBMIT_MIN_SLEEP=0 FM_SEND_SETTLE=0 \
  ...
  assert_no_grep 'TEXT:' "$log" "a refused misuse still typed text"
