...
make_fake_root() {
  ...
  # Teardown resolves the real Herdr-only endpoint owner through this isolated root.
  ln -s "$ROOT/bin/fm-backend.sh" "$fake/bin/fm-backend.sh"
  ln -s "$ROOT/bin/backends/herdr.sh" "$fake/bin/backends/herdr.sh"
  ...
  # Runtime stubs keep teardown inside the disposable fixture.
  fm_test_fake_herdr "$fake/bin"
...
SH
  ...
  fm_write_herdr_task_meta "$fake/state/$id.meta" \
    "worktree=$TMP_ROOT/nonexistent-worktree-$id" \
    "project=$TMP_ROOT/nonexistent-project-$id" \
    "harness=pi" "kind=ship" "mode=no-mistakes" "yolo=off" "tasktmp=$tasktmp"
  printf '%s' "$fake"
}
...
test_teardown_removes_tasktmp_dir() {
  ...
  FM_HOME="$fake" FM_STATE_OVERRIDE="$fake/state" FM_FAKE_HERDR_AGENT_MISSING=1 \
    PATH="$fake/bin:$PATH" bash "$fake/bin/fm-teardown.sh" "$id" >/dev/null 2>&1 \
...
SH
  ...
  fm_write_herdr_task_meta "$fake/state/$id.meta" \
    "worktree=$TMP_ROOT/nonexistent-wt-$id" "project=$TMP_ROOT/nonexistent-proj-$id" \
