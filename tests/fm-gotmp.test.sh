...
# The isolated fm-spawn subprocess in fm-spawn-dispatch-profile.test.sh covers temp-root
...
make_fake_root() {
  ...
  # fm-public-followup-lib.sh (and the fm-relay-lib.sh it sources): teardown sources
  ...
  ln -s "$ROOT/bin/fm-relay-lib.sh" "$fake/bin/fm-relay-lib.sh"
