...
case "${1:-} ${2:-}" in
  ...
  'session list') printf '{"sessions":[{"name":"default","running":true,"socket_path":"/tmp/fm-gotmp.sock"}]}\n' ;;
...
# live Herdr, Treehouse, or fleet state is touched. A nonexistent worktree path makes both
...
make_fake_root() {
  ...
  # fm-backend.sh and the Herdr adapter are real, while the CLI itself is stubbed.
...
SH
  ...
  write_fake_herdr "$fake/bin/herdr"
