...
test_send_text_submit_preexisting_working_pending_is_queued_enter() {
  ...
  # Native working plus proven pending after the retry budget is a busy-queued
  # Enter: the harness accepted Enter and will submit when the
...
test_dispatch_composer_state_routes_to_herdr() {
  (
    ...
    [ "$(fm_backend_composer_state herdr default:w1:p2)" = empty ]       || fail "composer_state did not dispatch to Herdr"
    [ "$(fm_backend_composer_state legacy-unrecorded x)" = unknown ]       || fail "unsupported metadata should report unknown without fallback"
  ) || fail "composer_state dispatch subshell failed"
  pass "fm_backend_composer_state dispatches only to Herdr and refuses legacy metadata"
}
