...
assert_grep '^backend=herdr$' "$PARENT/state/ios.meta" "parent metadata omitted the explicit endpoint backend"
...
RELAUNCH_CHECKPOINT=$(remote_env "$ROOT/bin/fm-on.sh" ios fm-remote-secondmate-control.sh \
  relaunch ios pi - - 2>&1) && fail "a restart with no accountable checkout should refuse"
