...
test_send_text_submit_borderless_omp_confirms() {
  # The #2029 analogue this adapter never received: a borderless omp
  ...
  out=$( PATH="$FB:$PATH" FM_ORCA_LOG="$LOG" FM_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; fm_backend_orca_send_text_submit term-123 "hello captain" 3 0.01 0.01' "$ROOT" )
  [ "$out" = empty ] || fail "a borderless omp composer should confirm the submit, got '$out'"
  pass "fm_backend_orca_send_text_submit: a borderless omp composer confirms delivery (the missing #2029 shape)"
}
...
test_spawn_preserves_orca_metadata_when_pathless_worktree_cleanup_fails() {
  ...
  printf '{"ok":false,"error":{"code":"worktree_not_removed","message":"worktree not removed"}}\n' > "$RESP/5.out"
  out=$( HOME="$SPAWN_HOME" PATH="$FB:$PATH" FM_ORCA_LOG="$LOG" FM_ORCA_RESPONSES="$RESP" \
    FM_ROOT_OVERRIDE="$ROOT" FM_STATE_OVERRIDE="$state" FM_DATA_OVERRIDE="$data" FM_CONFIG_OVERRIDE="$config" \
    FM_PROJECTS_OVERRIDE="$TMP_ROOT/unused-projects" FM_SPAWN_NO_GUARD=1 \
    "$ROOT/bin/fm-spawn.sh" "$id" "$proj" omp --mode no-mistakes --yolo off --backend orca 2>&1 )
  status=$?
...
test_spawn_writes_orca_metadata_and_launches_harness() {
  ...
  assert_contains "$out" "spawned $id harness=omp kind=ship mode=no-mistakes yolo=off window=fm-$id worktree=$wt" \
  ...
  assert_contains "$(cat "$log")" " --config '$ROOT/.omp/fm-worker-overlay.yml' --auto-approve --cwd '$wt'" \
...
test_spawn_refuses_orca_secondmate_before_home_mutation() {
  ...
  out=$( FM_ROOT_OVERRIDE="$ROOT" FM_HOME="$home" FM_STATE_OVERRIDE="$state" FM_DATA_OVERRIDE="$data" FM_CONFIG_OVERRIDE="$config" \
    ...
    "$ROOT/bin/fm-spawn.sh" "$id" "$subhome" omp --backend orca --secondmate 2>&1 )
  status=$?
  set +e
...
test_spawn_refuses_orca_when_runtime_not_ready() {
  ...
  printf '{"ok":true,"result":{"runtime":{"reachable":false,"state":"starting"}}}\n' > "$RESP/1.out"
  out=$( HOME="$SPAWN_HOME" PATH="$FB:$PATH" FM_ORCA_LOG="$LOG" FM_ORCA_RESPONSES="$RESP" FM_ORCA_STATUS_RESPONSE=sequence \
    FM_ROOT_OVERRIDE="$ROOT" FM_STATE_OVERRIDE="$state" FM_DATA_OVERRIDE="$data" FM_CONFIG_OVERRIDE="$config" \
    FM_PROJECTS_OVERRIDE="$TMP_ROOT/unused-projects" FM_SPAWN_NO_GUARD=1 \
    "$ROOT/bin/fm-spawn.sh" "$id" "$proj" omp --mode no-mistakes --yolo off --backend orca 2>&1 )
  status=$?
...
test_peek_send_and_crew_state_route_through_orca_meta() {
  ...
  fm_write_meta "$state/$id.meta" \
    "window=fm-$id" "endpoint_task_id=$id" "terminal=term-io" "worktree=$wt" "project=$wt" "harness=omp" "kind=scout" "backend=orca"
  touch "$state/.last-watcher-beat"
  orca_case io-path
...
test_peek_and_crew_state_fail_closed_on_orca_error_json() {
  ...
  fm_write_meta "$state/$id.meta" \
    "window=fm-$id" "endpoint_task_id=$id" "terminal=term-stale" "worktree=$wt" "project=$wt" "harness=omp" "kind=scout" "backend=orca"
  touch "$state/.last-watcher-beat"
...
test_ship_teardown_removes_orca_worktree_when_id_path_matches() {
  ...
  fm_write_meta "$state/$id.meta" \
    ...
    "harness=omp" "kind=ship" "mode=local-only" "yolo=off" \
...
test_teardown_refuses_orca_missing_worktree_id() {
  ...
  fm_write_meta "$state/$id.meta" \
    ...
    "harness=omp" "kind=scout" "mode=no-mistakes" "yolo=off" "backend=orca" \
    "decisions_reviewed=1" "decision_keys="
