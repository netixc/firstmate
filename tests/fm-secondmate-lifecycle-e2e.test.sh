...
phase_spawn() {
  ...
  PATH="$FAKEBIN:$PATH" FM_HOME="$HOME_DIR" FM_CONFIG_OVERRIDE="$HOME_DIR/parent-config" \
    ...
    "$ROOT/bin/fm-spawn.sh" design "$SUB" pi --secondmate >/dev/null \
...
phase_send() {
  ...
  assert_grep 'Firstmate instruction waiting:' "$LOG" "send did not ring the native endpoint recorded in this home's metadata"
  assert_no_grep 'route this work' "$LOG" "send typed the payload instead of only the doorbell"
  pass "send: a bare fm-<id> secondmate enqueues a marked request and rings its native endpoint"
}
...
EOF
  local out before
  out=$(PATH="$FAKEBIN:$PATH" FM_HOME="$HOME_DIR" FM_FAKE_LAUNCH_LOG="$LOG" \
    \
  ...
  PATH="$FAKEBIN:$PATH" FM_HOME="$HOME_DIR" FM_FAKE_LAUNCH_LOG="$LOG" \
    \
...
phase_recovery() {
  ...
  rm -f "$HOME_DIR/state/design.meta" "$FAKEBIN/herdr-tab-created"
  PATH="$FAKEBIN:$PATH" FM_HOME="$HOME_DIR" FM_FAKE_LAUNCH_LOG="$LOG" \
    "$ROOT/bin/fm-spawn.sh" design pi --secondmate >/dev/null 2>&1 \
  ...
  assert_grep 'backend=herdr' "$meta" "respawn did not preserve the native session provider"
  assert_grep 'window=lab:w2:p2' "$meta" "respawn did not reconstruct the native direct-report target"
  assert_grep 'herdr_tab_id=w2:t2' "$meta" "respawn did not bind the native tab identity"
...
phase_teardown() {
  ...
  : > "$LOG"
  teardown_out=$(PATH="$FAKEBIN:$PATH" FM_HOME="$HOME_DIR" FM_FAKE_LAUNCH_LOG="$LOG" \
  ...
  assert_present "$FAKEBIN/herdr-pane-closed" "teardown did not close the recorded native pane before removing durable records"
