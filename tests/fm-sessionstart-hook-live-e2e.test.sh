...
SH
  ...
  case "$harness" in
    ...
    pi)
      ...
      cp "$ROOT/bin/fm-host-platform-lib.sh" "$ROOT/bin/fm-operational-input.sh" "$lab/bin/"
...
probe_pi_sessionstart_prerequisite() {
  ...
  cp "$ROOT/bin/fm-host-platform-lib.sh" "$ROOT/bin/fm-operational-input.sh" "$project/bin/"
