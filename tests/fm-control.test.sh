...
test_harness_family_resolution() {
  [ "$(fm_control_harness_family pi)" = pi ] || fail "plain Pi should resolve"
  for retired in claude codex opencode pi-signed grok kimi cursor muse; do
    fm_control_harness_family "$retired" >/dev/null 2>&1 \
      && fail "retired harness $retired must not resolve"
  done
  fm_control_harness_family someagent >/dev/null 2>&1 \
    && fail "an unknown harness must not resolve"
  pass "fm-control-lib: only exact plain Pi resolves to control mechanics"
}
...
# verify the replacement before stopping a running agent.
test_harness_kind_capability() {
  local kind
  for kind in ship scout secondmate; do
    fm_control_harness_supports_kind pi "$kind" \
      || fail "plain Pi should support $kind tasks"
  done
  fm_control_harness_supports_kind claude ship \
    && fail "a retired harness must not claim any kind"
  fm_control_harness_supports_kind someagent ship \
    && fail "an unknown harness must not claim any kind"
  pass "fm-control-lib: plain Pi supports every retained task kind"
}
...
test_resume_is_refused_with_its_reason() {
  ...
  assert_contains "$out" "not a control verb for plain Pi" \
