...
test_guard_warnings() {
  ...
  # Pin Pi so the host test runner's harness ancestry cannot change this fixture.
  printf 'project=x\n' > "$state/task.meta"
  printf 'project=y\n' > "$state/task2.meta"
  ...
  grep -F 'repair a missing or failed watcher cycle with the Pi tool fm_watch_arm_pi' "$err" >/dev/null || fail "guard banner missing neutral automatic-recovery guidance"
  grep -F 'queued wakes pending - drain them' "$err" >/dev/null || fail "guard did not warn about pending queue"
  grep -F 'After draining queued wakes, repair a missing or failed watcher cycle with the Pi tool fm_watch_arm_pi' "$err" >/dev/null || fail "guard did not order neutral automatic recovery after drain"
...
test_arm_self_eviction_is_loud_without_successor() {
  ...
  armout="$dir/arm.out"
  mark_pr_check_migration_complete "$state"
  PATH="$fakebin:$PATH" FM_STATE_OVERRIDE="$state" FM_POLL=0.2 FM_SIGNAL_GRACE=1 FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 FM_ARM_CONFIRM_TIMEOUT=3 "$WATCH_ARM" > "$armout" &
  armpid=$!
  i=0
  while [ "$i" -lt 120 ]; do
  ...
  if ! grep -qF "watcher: started pid=$watcher_pid" "$armout"; then
    kill "$armpid" 2>/dev/null || true
    wait "$armpid" 2>/dev/null || true
    fail "arm did not start before self-eviction check"
  fi
