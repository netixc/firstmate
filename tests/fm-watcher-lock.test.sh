...
test_guard_warnings() {
  ...
  dir=$(make_case guard)
  # Pin Pi repair wording while blinding structural ancestry. Watcher-liveness
  # queries below still reach the real process table.
  ...
  PATH="$blind:$PATH" PI_CODING_AGENT=true FM_PI_HARNESS=pi GROK_AGENT='' FM_ROOT_OVERRIDE="$dir" FM_STATE_OVERRIDE="$state" FM_GUARD_GRACE=1 "$ROOT/bin/fm-guard.sh" 2> "$err" >/dev/null || fail "guard failed"
  ...
  grep -F 'Pi tool fm_watch_arm_pi' "$err" >/dev/null || fail "guard banner missing Pi recovery guidance"
