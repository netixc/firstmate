...
test_guard_warnings() {
  ...
  #       warning follows it, and the Pi repair guidance names the one supported
  #       recovery command (never the old conflicting "restart NOW first").
  ...
  grep -F 'repair a missing or failed watcher cycle with fm_watch_arm_pi' "$err" >/dev/null || fail "guard banner missing Pi recovery guidance"
  grep -F 'queued wakes pending - drain them' "$err" >/dev/null || fail "guard did not warn about pending queue"
  ! grep -F 'Stop-owned automatic recovery' "$err" >/dev/null || fail "guard retained retired automatic-recovery wording"
