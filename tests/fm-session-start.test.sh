...
# make_fake_ps_pi <fakebin>: harness_pid()/holder_alive() (fm-lock.sh) walk
...
case "${1:-} ${2:-}" in
  ...
  "session list")
    ...
    ;;
  "workspace list")
    printf '{"result":{"workspaces":[{"workspace_id":"ws1","label":"2ndmate-%s","focused":true,"active_tab_id":"ws1:t-focus"}]}}\n' "$mate_id"
    ;;
  "tab list")
    if [ -e "$spawned" ]; then
      printf '{"result":{"tabs":[{"tab_id":"ws1:t-focus","workspace_id":"ws1","label":"captain","focused":true},{"tab_id":"ws1:t-new","workspace_id":"ws1","label":"fm-%s","focused":false}]}}\n' "$mate_id"
    elif [ -e "$killed" ]; then
      printf '%s\n' '{"result":{"tabs":[{"tab_id":"ws1:t-focus","workspace_id":"ws1","label":"captain","focused":true}]}}'
    else
      printf '{"result":{"tabs":[{"tab_id":"ws1:t-focus","workspace_id":"ws1","label":"captain","focused":true},{"tab_id":"ws1:t-old","workspace_id":"ws1","label":"fm-%s","focused":false}]}}\n' "$mate_id"
    fi
    ;;
  "tab create")
    : > "$spawned"
    printf '%s\n' '{"result":{"tab":{"tab_id":"ws1:t-new"},"root_pane":{"pane_id":"ws1:p-new"}}}'
    ;;
  "pane list")
    if [ -e "$spawned" ]; then
      printf '%s\n' '{"result":{"panes":[{"pane_id":"ws1:p-new","tab_id":"ws1:t-new"}]}}'
    elif [ ! -e "$killed" ]; then
      printf '%s\n' '{"result":{"panes":[{"pane_id":"ws1:p-old","tab_id":"ws1:t-old"}]}}'
    else
      printf '%s\n' '{"result":{"panes":[]}}'
    fi
    ;;
  "pane get")
    pane=${3:-}
    if [ "$pane" = ws1:p-new ] && [ -e "$spawned" ]; then
      printf '%s\n' '{"result":{"pane":{"pane_id":"ws1:p-new","tab_id":"ws1:t-new","workspace_id":"ws1"}}}'
    elif [ "$pane" = ws1:p-old ] && [ ! -e "$killed" ]; then
      printf '%s\n' '{"result":{"pane":{"pane_id":"ws1:p-old","tab_id":"ws1:t-old","workspace_id":"ws1"}}}'
    else
...
# Pin or clear Pi's marker so the surrounding interactive shell cannot leak
...
test_herdr_backend_diagnostics_follow_real_session_start() {
  local rec root home fakebin out
  rec=$(new_world herdr-configured)
  IFS='|' read -r root home fakebin <<EOF
$rec
EOF
  make_fake_toolchain "$fakebin"
  ...
  pass "session start: configured Herdr homes require only the Herdr toolchain"
}
...
EOF
  ...
  assert_contains "$(cat "$log")" "pane close ws1:p-old" "session start did not close the confirmed Herdr husk"
  assert_contains "$(cat "$log")" "tab create" "session start did not relaunch the Herdr secondmate"
  assert_grep 'herdr_pane_id=ws1:p-new' "$home/state/$SESSION_START_HERDR_SECOND_MATE_ID.meta" \
...
test_unreachable_network_never_blocks_the_digest() {
  ...
  rec=$(prepare_session_start_herdr_secondmate secondmate-slow-network)
...
EOF
  ...
  started=$(date +%s)
  out=$(run_session_start_herdr_secondmate "$root" "$home" "$fakebin" "$mate" "$log" "$spawned" missing)
  ...
  assert_contains "$(cat "$log")" "tab create" \
...
test_deferred_result_reaches_the_agent_when_the_digest_cannot_print_it() {
  ...
  rec=$(prepare_session_start_herdr_secondmate secondmate-wake-once)
  IFS='|' read -r root home fakebin mate log spawned <<EOF
$rec
EOF
  ...
  run_session_start_herdr_secondmate "$root" "$home" "$fakebin" "$mate" "$log" "$spawned" missing >/dev/null
