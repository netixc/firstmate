...
# make_fake_ps_harness <fakebin> <harness>: harness_pid()/holder_alive()
# walks `ps` output looking for a harness command name.
...
make_fake_ps_primary() {
  make_fake_ps_harness "$1" pi
}
...
# codex/pi/grok session fails cases that pin a different fake harness while CI
