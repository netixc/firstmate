...
# --- busy_state on a real pi harness (verified in herdr-verification-p2.md) ---

if [ "${FM_HERDR_SMOKE_REAL_pi:-0}" = 1 ] && command -v pi >/dev/null 2>&1; then
  fm_backend_herdr_send_literal "$TARGET" "pi_CODE_ENABLE_PROMPT_SUGGESTION=false pi --dangerously-skip-permissions --print 'say the word HERDRSMOKEOK and nothing else'"
  sleep 0.2
  ...
  case "$out" in
    *HERDRSMOKEOK*) pass "real herdr: agent_status busy/idle detection tracks a real pi turn, and capture shows its output" ;;
  ...
  echo "note: pi not installed; skipping the real-agent busy_state check" >&2
fi
