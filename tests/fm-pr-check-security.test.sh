...
exit 0
SH
  chmod +x "$fakebin/gh" "$fakebin/gh-axi" "$fakebin/herdr"
  : > "$dir/gh.log"
  : > "$dir/gh-axi.log"
  : > "$dir/guard.log"
  printf '%s\n' "$dir"
...
write_manual_poll_pair() {
  local state=$1 url=${2:-https://github.com/o/r/pull/10} path number
  fm_pr_url_parse "$url" || fail "manual poll fixture URL was invalid"
  path=$FM_PR_PATH
  number=$FM_PR_NUMBER
  cp "$POLL" "$state/task-a.check.sh"
  printf '%s\n%s\n%s\n' "$url" "$path" "$number" > "$state/task-a.pr-poll"
...
# Firstmate is GitHub-only: every GitLab merge request URL below - well-formed
# canonical ones included - must be refused before any command runs.
  ...
  for case_name in copied-pair copied-registration metadata-mismatch task-mismatch forged-registration partial-publication; do
    ...
    case "$case_name" in
      ...
      forged-registration)
        write_manual_poll_pair "$state"
        printf '%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n' \
          fm-pr-poll-registration-v3 task-a https://github.com/o/r/pull/10 o/r 10 \
...
# Firstmate is GitHub-only. A well-formed URL for another forge is the most
# dangerous rejected input, because it looks canonical: it must be refused at
# every entry point before any external command runs or any poll is armed, and
# no forge CLI other than gh may ever be reachable from the watch path.
test_non_github_forge_urls_are_refused() {
  local dir state rc url script
  dir=$(make_case non-github-forge)
  state="$dir/home/state"
  ...
  for url in \
    ...
    https://code.internal/team/tools/ci-runner/-/merge_requests/123456 \
    https://bitbucket.org/o/r/pull-requests/1; do
    ! fm_pr_url_parse "$url" || fail "parser accepted a non-GitHub forge URL"
    ...
    rc=$?
    set -e
    [ "$rc" -eq 2 ] || fail "arming did not refuse a non-GitHub forge URL"
    [ ! -e "$state/task-a.check.sh" ] || fail "refused arming left a poll armed"
    [ ! -e "$state/task-a.pr-poll" ] || fail "refused arming left a sidecar"
    [ ! -e "$state/task-a.pr-poll-registration" ] || fail "refused arming left a registration"
    grep -q '^pr=' "$state/task-a.meta" && fail "refused arming recorded a PR URL"

    set +e
    run_merge_entry "$dir" task-a "$url" >/dev/null 2>&1
    rc=$?
    set -e
    [ "$rc" -eq 2 ] || fail "merge wrapper did not refuse a non-GitHub forge URL"
    [ ! -s "$dir/gh-axi.log" ] || fail "merge wrapper reached the GitHub CLI for a non-GitHub URL"
    [ ! -s "$dir/gh.log" ] || fail "a refused non-GitHub URL reached the GitHub CLI"
  done

  # No merge-request route, and no forge CLI other than gh, survives anywhere
  # on the tracked watch path.
  for script in "$ROOT/bin/fm-pr-lib.sh" "$ROOT/bin/fm-pr-poll.sh" \
    "$ROOT/bin/fm-pr-check.sh" "$ROOT/bin/fm-pr-merge.sh" "$ROOT/bin/fm-pr-check-migrate.sh"; do
    ! grep -qE '(^|[^a-z-])glab([^a-z-]|$)|merge_requests' "$script" \
      || fail "a tracked PR script still routes a merge request or invokes glab"
  done
  pass "non-GitHub forge URLs are refused at every entry point with no side effect"
}

seed_canonical_poll() {
  local dir=$1 id=$2 url=$3 template=${4:-$POLL} state path number
  state="$dir/home/state"
  fm_pr_url_parse "$url" || fail "retirement fixture URL was invalid"
  path=$FM_PR_PATH
  number=$FM_PR_NUMBER
  fm_pr_poll_prepare "$state" "$id" "$url" "$path" "$number" "$template" \
