...
test_unsupported_host_refuses_before_protocol_staging() {
  local out err code runtime_tmp
  ...
  assert_grep 'UNSUPPORTED_HOST: MINGW64_NT-10.0' "$err" \
    "remote entrypoint did not explicitly reject its unsupported host"
  assert_absent "$runtime_tmp" "remote entrypoint created protocol staging on an unsupported host"
  pass "remote entrypoint rejects unsupported hosts before protocol staging"
}
