...
run = run_step && run_step.fetch("run")
tokens = Shellwords.split(run.to_s.gsub(/\\\s*\n/, " "))
command_index = tokens.index("bin/fm-test-run.sh")
raise "required Herdr runner invocation is missing" unless command_index
args = tokens.drop(command_index + 1)
family_index = args.index("--family")
raise "required Herdr family argument is missing" unless family_index
family = args.fetch(family_index + 1)
skip_tokens = []
args.each_cons(2) do |flag, value|
  skip_tokens << value if flag == "--fail-on-gate-skip"
end
expected_skips = [
  "herdr not found",
  "live: herdr absent",
...
raise "required Herdr family mismatch: #{family}" unless family == "real-herdr-gated"
raise "required Herdr skip guards mismatch: #{skip_tokens.inspect}" unless skip_tokens == expected_skips
puts JSON.generate(
...
' "$CI_WORKFLOW") || fail "could not read the required Herdr matrix contract"
  ...
  pass "required Herdr CI semantically enforces all Herdr and lifecycle skip guards"
}
