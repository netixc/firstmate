...
function shouldArm(paths) {
  ...
  if (existsSync(`${paths.config}/relay.env`)) return true;
  try {
...
function spawnArm(paths, sessionID, client, predecessorArmPid = "") {
  ...
  const armChild = spawn("bash", ["-lc", 'config_dir="${FM_CONFIG_OVERRIDE:-$FM_HOME/config}"; [ -f "$config_dir/relay.env" ] && . "$config_dir/relay.env"; exec "$FM_ROOT_OVERRIDE/bin/fm-watch-arm.sh" --restart'], {
    cwd: paths.root,
    env,
    stdio: ["ignore", "pipe", "pipe"],
  });
