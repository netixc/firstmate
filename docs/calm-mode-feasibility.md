...
Pi was reverified at 0.81.1 on 2026-07-23 for the Calm presentation change, and later sections record the subsequent Pi compatibility refreshes.
...
It repeats the timing-sensitive exact-watcher and Calm-loaded-off paths three times each.
Because session persistence can complete before Pi presents the completed follow-up, the viewport assertion waits for the rendered processing result before counting the one captain answer rather than treating an immediate zero-answer capture as a duplicate.
It also asserts exact user-role operational envelopes in order, no replacement custom messages, one processing result, zero operational transcript rows, and the two-row neighboring-assistant geometry for live, adjacent, and restart paths.
