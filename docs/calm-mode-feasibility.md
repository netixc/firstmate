...
Pi was reverified at 0.81.1 on 2026-07-23 for the Calm presentation change, with later API checks covering 0.82.0, 0.84.4, and 0.85.1.
Calm is partially feasible through API-probed exported-class adapters: public APIs control working visibility, collapsed labels, known tool slots, custom entries, and expansion redraws, while exported assistant and interactive-mode classes provide the collapsed-thinking and operational-user layout boundaries.
The adapters gate on the exact method's presence rather than a version number, and generic tool or status filtering remains unavailable.
These conclusions are deliberately limited to the named Pi versions and supported surfaces.
The canonical encoder and both tmux and Herdr transports keep the same semantic input; only Pi's presentation changes.
