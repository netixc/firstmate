...
If a retained adapter's control contract names a post-interrupt clear key, the clear is refused before anything is sent when the recorded backend cannot deliver it.
...
It is not deterministic across the verified adapters: codex, grok, and gemini resume only from a session id printed at exit, opencode continues the most recent session for the cwd, and pi, pi-signed, kimi, and agy have no verified pane-resume contract.
