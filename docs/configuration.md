...
Canonical plain Pi 0.84.4 is the only supported worker and primary harness, and direct primary launch is unsupported; use the parent launcher shown in the README quick start.
...
When the file exists, `fm-spawn.sh` enforces that contract by refusing crewmate and scout spawns that lack the explicit `--harness pi` selection.
...
FM_WATCH_ARM_RETIRE_TIMEOUT_MS=1000   # milliseconds the Pi watcher waits for an unready successor arm to exit before abandoning retries
FM_WATCH_REARM_RETRY_BASE_MS=250   # Pi watcher base delay for continuity restoration retries
FM_WATCH_REARM_RETRY_MAX_MS=4000   # Pi watcher cap for exponential continuity retry delay
FM_WATCH_REARM_RETRY_LIMIT=5   # Pi watcher launch-failure retries before surfacing restoration failure
...
FM_BUSY_REGEX=          # optional override for Pi's rendered delivery-busy guards; semantic worker state ignores it
