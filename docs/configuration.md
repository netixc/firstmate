...
`docs/sessionstart-nudge.md` owns Pi's native session-open invocation of the digest command and its compatibility boundary.
...
An unsupported primary identity never loads this feature and must be migrated to plain Pi.
...
Herdr is Firstmate's only session provider, and Treehouse remains the task-copy provider.
New spawns always select `herdr`.
An explicit `--backend`, `FM_BACKEND`, or local gitignored `config/backend` value is accepted only when it is exactly `herdr`; every other value is rejected with no runtime detection or fallback.
A spawn requires the Herdr CLI, a compatible protocol, a reachable and unambiguous named session/socket, `jq`, and Treehouse.
Missing, incompatible, unauthenticated, ambiguous, or unreachable Herdr is a blocker and never creates another kind of endpoint.
Every new task explicitly records `backend=herdr`, `endpoint_task_id=`, `herdr_session=`, `herdr_workspace_id=`, `herdr_tab_id=`, and `herdr_pane_id=`.
Old metadata with an absent or non-Herdr `backend=` is unsupported and requires explicit migration; readers and cleanup never reinterpret it as Herdr.
Task selectors resolve through exact task metadata.
Ad hoc or legacy provider targets are refused because they lack a durable Herdr task binding.
`fm-teardown.sh <id>` validates the complete metadata-only endpoint identity before runtime dispatch or cleanup mutation and preserves malformed or unsupported records.
...
[`herdr-backend.md`](herdr-backend.md) owns protocol/version/readiness, named-session/socket targeting, launcher-bound placement, native events, focus restoration, presentation spaces, cleanup, and recovery behavior.
`HERDR_SESSION` selects the named session for normal operations, but every adapter call also passes the trailing `--session` flag.
...
Canonical plain Pi 0.84.4 is the only supported worker and primary harness.
`config/crew-harness` accepts only `pi` or `default`.
`config/secondmate-harness` accepts `pi [model] [effort]` or `default`.
Absent or `default` values resolve to the detected primary, which must itself be plain Pi.
Any legacy or excluded identity is reported as unsupported migration input and is never normalized or substituted.
An explicit spawn harness must also be exact `pi`; raw launch commands are unsupported.
Model and effort dispatch remain available through Pi's `--model` and `--thinking` flags.
[`.agents/skills/harness-adapters/SKILL.md`](../.agents/skills/harness-adapters/SKILL.md), [`bin/fm-harness.sh`](../bin/fm-harness.sh), and [`bin/fm-spawn.sh`](../bin/fm-spawn.sh) own the operational mechanics.
...
When the file exists, `fm-spawn.sh` enforces that contract by refusing crewmate and scout spawns that lack an explicit exact Pi harness selected through `--harness` or the positional adapter argument.
  ...
  "rules": [
    {
      ...
      "use": [
        { "harness": "pi", "model": "<optional model>", "effort": "<low|medium|high|xhigh|max, optional>" }
      ],
...
If a selected profile carries an effort value Pi does not accept, `fm-spawn.sh` records the requested `effort=` in task meta for traceability but omits the launch flag, and bootstrap reports the invalid harness/effort pair as a `CREW_DISPATCH` diagnostic when it is visible in the file.
See [`docs/examples/crew-dispatch.json`](examples/crew-dispatch.json) for a starting point to copy into local `config/crew-dispatch.json`.
When the file exists, bootstrap validates it with `jq`.
Valid files stay silent by default; with `FM_BOOTSTRAP_VERBOSE_FACTS=1`, bootstrap emits `BOOTSTRAP_INFO: crew dispatch active config/crew-dispatch.json`, one `BOOTSTRAP_INFO:` fact per rule, and one fact for the optional default profile set.
Malformed JSON, an empty or malformed rule/default array, a harness other than exact `pi`, or an effort value unsupported by Pi is reported as `CREW_DISPATCH: invalid config/crew-dispatch.json - ...`; missing `jq` is reported through the normal `MISSING: jq` install-consent flow.
...
The required provider delta is owned by `fm_backend_required_tools` in `bin/fm-backend.sh`: `herdr`, `jq`, and `treehouse`.
An unsupported provider assertion emits `BACKEND_INVALID` and blocks dispatch without fallback.
...
FM_BACKEND=             # optional exact "herdr" assertion; every other value is rejected without fallback
...
FM_COMPOSER_CAPTURE_LINES=20   # bound for Herdr tail-capture composer reads so stale scrollback banners stay out of the candidate set
...
FM_COMPOSER_GHOST_LUMA_MAX=128   # max perceived luminance for styled Herdr ghost text; dim/faint text is stripped regardless
