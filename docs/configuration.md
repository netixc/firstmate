...
Readable remote state that proves no link exists succeeds without a write, including when the task record disappears before the clear's authoritative locked read, while a present link is cleared only when its Relay request identity matches the registration and the state is writable; an identity mismatch, unreadable state, a symlink or non-regular record, an unavailable write or lock, an older remote copy, or a host that never confirms the clear leaves the loop retained for reconciliation.
