...
After a successful per-home push that changes any allowlisted `config/*` item, it also writes a literal-content reread instruction under the secondmate's `state/` and sends only a single-line pointer to its absolute path, with the file built from the destination's post-write bytes (or `ABSENT` when the copy was removed); a failed send leaves a durable retry marker so a later normal config push retries that pointer, while unchanged allowlisted config sends no message when no retry is pending.
