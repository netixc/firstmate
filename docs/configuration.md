...
After a successful per-home push that changes any allowlisted `config/*` item, it also writes a generation-specific private literal-content reread instruction under the secondmate's `state/` and sends only a single-line pointer to its absolute path, with the file built from the destination's post-write bytes (or `ABSENT` when the copy was removed); a failed publication or marker write retains exact bytes in a bounded private retry queue and surfaces a retry diagnostic, a failed send leaves a per-generation marker when possible so a later normal config push retries that pointer, concurrent pushes serialize per home, and unchanged allowlisted config sends no message when no retry is pending.
