...
- The required remote tool set is `git`, `jq`, `herdr`, compatible `tasks-axi`, `treehouse`, and at least one of `codex`, `opencode`, `pi`, or `pi-signed`; macOS additionally requires `lsof` so the doctor and guard can prove which process owns the session socket.
