...
- The required remote tool set is `git`, `jq`, `herdr`, compatible `tasks-axi`, `treehouse`, and plain `pi`; macOS additionally requires `lsof` so the doctor and guard can prove which process owns the session socket.
