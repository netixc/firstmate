...
Install Herdr 0.9.0 or newer with protocol 22 or newer, plus `jq` and `lsof`; the readiness doctor validates this floor and the exact cleanup dependency before launch.
...
The `gui/<uid>` domain, not the login shell, is what gives that server and every pane it spawns the Aqua audit session and login-keychain access; a server born in any other session cannot read the login keychain, so worker credentials stored there are unavailable.
...
- The required remote tool set is `git`, `jq`, `lsof`, Herdr 0.9.0+ / protocol 22+, compatible `tasks-axi`, `treehouse`, and plain `pi`; on macOS the same `lsof` dependency also proves which process owns the session socket.
