...
| Herdr | family-run step `timeout-minutes: 35`; job `timeout-minutes: 75` backstop | Healthy runs finished below 20 minutes with the required real Pi lifecycle guard, so the step bound remains the hang tripwire (cleanup and timing artifacts still upload) while the job cap stays a last-resort backstop. Refresh this figure from the lane's uploaded timing artifact. |
