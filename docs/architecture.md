...
When a canonical validated PR poll returns exactly `merged`, the watcher appends that durable notification before publishing a private receipt bound to the poll's registration, bytes, file identities, metadata, URL, and task ID.
...
No-verb wakes, such as `working:` notes and bare turn-ended signals, are benign only when `bin/fm-crew-state.sh` reports positive evidence that the crew is still working: an actively running no-mistakes step attributed to that crew's current code or a Herdr busy signature.
...
The shared turn-end predicate remains the final backstop for all five harness protocols; Pi's adapter also owns deterministic session claim and single-flight operational-turn settlement as documented in [sessionstart-nudge.md](sessionstart-nudge.md) and [turnend-guard.md](turnend-guard.md).
...
Herdr is the session-provider layer below Firstmate's scripts.
It owns task endpoint creation, bounded capture, text and key sends, current-path reads for spawn-time worktree discovery, live-pane lookup, registered-agent liveness, transition events, and exact endpoint cleanup.
`bin/fm-backend.sh` centralizes `state/<id>.meta` helpers, selector resolution, and the Herdr-only operation contract.
`bin/backends/herdr.sh` owns the implementation.
...
In Herdr, a secondmate launch lands in that secondmate home's labeled workspace, and crewmates spawned from that home land in the same workspace.
...
Explicit endpoint-target sends and direct human typing stay unmarked, so captain intervention in a secondmate pane remains conversational.
...
Firstmate watches and merges GitHub pull requests only, so a well-formed URL for any other forge is refused by the same canonical parser before any external command runs or any poll is armed.
