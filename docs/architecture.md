...
[docs/pi-supervision-branch.md](pi-supervision-branch.md) owns row eligibility and dispatch architecture, while the generated [Pi supervision protocol](supervision-protocols/pi.md) owns MAIN's captain-visible response and merged-event handling.
...
[`watcher-continuity.md`](watcher-continuity.md) owns the Pi extension's successor, terminal-delivery, and repair contract.
...
On the Pi primary, tracked extension integration gives the session a push-based backstop: when work, a process-event source, or Relay polling needs supervision and no identity-matched watcher lock with a fresh beacon is live, the turn-end integration forces one bounded follow-up.
...
Its supervisor injection path carries and revalidates one exact Herdr session/workspace/tab/pane hierarchy, separate from worker task metadata, and has no inference, default-session probe, or provider fallback.
...
The retries-exhausted queued-Enter decision and composer classification remain owned by `bin/fm-composer-lib.sh`.
...
All are scoped to plain Pi rather than a global pattern union, and none is a recorded worker state source.

## Runtime session provider

Herdr is the sole session-provider layer below Firstmate's scripts.
`bin/fm-backend.sh` retains selection, metadata, cleanup identity validation, selector resolution, and operation dispatch ownership while admitting only `herdr`; `bin/backends/herdr.sh` owns the provider protocol.
There is no ambient provider detection or alternate-provider fallback.
`configuration.md` owns the accepted configuration and explicit legacy-record migration boundary.
Every new task explicitly binds its Herdr session, workspace, tab, pane, and task identity.
Absent or non-Herdr provider metadata is unsupported and never silently reinterpreted.
Herdr's native agent status contributes activity evidence, native events shorten blocked-transition latency, and bounded polling remains the continuity backstop when event capability is unavailable.
The recovery-grade agent classifier distinguishes live, dead, missing, ambiguous, and unreadable endpoints; only positively dead or missing identities license recovery.
[`herdr-backend.md`](herdr-backend.md) owns protocol/version/readiness, named-session/socket targeting, task containers, presentation spaces, focus restoration, cleanup, and recovery scope.
Treehouse remains the sole task-copy provider.
...
The shell scripts validate the JSON shape, exact Pi harness identity, and Pi effort values, but they do not parse task intent, match natural-language rules, or own array selection.
...
Unsupported effort values are still recorded in task meta when passed to `fm-spawn.sh`, but the launch template omits any effort flag that Pi does not accept.
...
Secondmate agents and crewmates both run exact plain Pi, while `config/secondmate-harness` may independently pin the secondmate's Pi model and effort.
Every legacy harness token and raw launch command is refused instead of normalized or substituted.
The [`secondmate-provisioning` skill](../.agents/skills/secondmate-provisioning/SKILL.md) owns the full pin precedence, inherited-local-material allowlist, and propagation contract.
...
Fleet state lives in each task's Herdr endpoint, no-mistakes run records, status event logs, local markdown under `data/` including `data/captain.md`, `data/captain-shared.md`, and `data/learnings.md`, and persistent secondmate homes.
...
At session start, confirmed-dead secondmate agent endpoints are closed and relaunched through the same secondmate spawn path, while ambiguous liveness reads - including a registered Herdr agent contradicted by strict proof of one lone idle shell - are left untouched to avoid duplicate supervisors.
