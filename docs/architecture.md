...
That block owns the live wait shape for the running primary harness: Grok uses background-notify cycles, Codex uses bounded foreground checkpoints, Pi and pi-signed use the same two tracked primary extensions, and OpenCode uses its TUI plugin.
...
The existing turn-end guard remains the final backstop for every harness-engine protocol, with pi-signed sharing Pi's protocol.
...
Codex and standalone Kimi classify unknown behind explicit probes until a semantic source is live-verified for them, and Grok and AGY each keep one clearly isolated rendered-tail fallback that can only ever classify their own task.
...
That keeps spawn launch compatible across codex, opencode, pi, pi-signed, grok, kimi, and agy while preserving the requested profile for later audit.
