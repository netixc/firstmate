...
The existing turn-end guard remains the final backstop for every harness-engine protocol, with pi-signed sharing Pi's protocol, omp's blocking `session_stop` hook compelling one continuation per turn, and the `--claude` mode cooperating with the auto-arm claim.
...
That keeps spawn launch compatible across claude, codex, opencode, pi, pi-signed, grok, kimi, gemini, muse, rovo, omp, and agy while preserving the requested profile for later audit.
...
`/updatefirstmate` fast-forwards the running firstmate repo and eligible registered secondmate homes from `origin` without touching project clones.
...
For a remote route admitted by the current transport protocol, the configured code root updates from its own origin on that host before the persistent home fast-forwards to the code-root commit.
A legacy remote route stays unchanged until the operator completes the [attended-upgrade prerequisite](remote-secondmates.md#retire-a-legacy-worker-before-upgrade).
