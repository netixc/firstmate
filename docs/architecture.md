...
Herdr's default container shape is workspace-per-home plus tab-per-task: the primary home uses workspace label `firstmate`, secondmate homes use `2ndmate-<secondmate-id>`, and recovery/list-live scopes to the current `FM_HOME`.
Its default-off interim grouping instead gives every ordinary crew its own owned workspace associated with that home's supervisor anchor, with flat-list contiguity as a secondary rendering repair.
...
`config/herdr-child-workspaces` is inherited too; the primary's default-off Herdr space-per-crewmate opt-in propagates best-effort across secondmate homes under the `secondmate-provisioning` contract.
