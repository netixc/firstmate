...
### Plain Pi worker selection and liveness

Plain Pi 0.84.4 was verified on 2026-08-31 with tmux 3.6a on macOS.
Firstmate accepts only the exact `pi` worker identity and explicitly rejects every retired harness value instead of normalizing it.
The launch boundary resolves the selected executable and requires its complete `--version` output to equal `0.84.4` before constructing a worker launch.
No package digest, process registration, parent launch record, or executable-image proof participates in this check.

The portable regressions are:

```sh
bin/fm-test-run.sh tests/fm-pi-only-harness.test.sh
bin/fm-test-run.sh tests/fm-tmux-agent-liveness.test.sh
bin/fm-test-run.sh tests/fm-spawn-dispatch-profile.test.sh
```

The credentialed real-Pi lifecycle guard is:

```sh
FM_PI_ONLY_LIVE_E2E=1 bin/fm-test-run.sh tests/fm-pi-only-live-e2e.test.sh
```

Observed bounded output:

```text
ok - real Pi supervision extensions preserve plain-Pi identity (Pi 0.84.4)
ok - real Pi interrupt lifecycle control succeeds (Pi 0.84.4)
ok - real Pi transactional relaunch preserves executable and profile selection (Pi 0.84.4)
ok - real Pi replacement exit lifecycle control succeeds (Pi 0.84.4)
```

Herdr continues to use native registered-agent state rather than tmux process-name matching.
Zellij has no verified recovery-grade agent process probe, while Orca and cmux do not support secondmate spawns, so those backends retain their existing plain Pi launch semantics.
...
The retained portable regressions exercise Pi's empty, pending, wrapped, styled-placeholder, and busy composer shapes through the public classifier:
...
The strict blank-row posture remains unchanged: an unidentified blank shell row is unknown and injection defers.
...
Scope of the earlier evidence: the plain `pi` CLI (0.82.0 at verification time) was a compiled binary whose bundled SDK was not importable from Node, so the importable npm package was the only surface the guard and the typecheck could pin.
The extension executes inside the plain CLI's own runtime, so a proposed supported-version change can drift ahead of the pinned npm surface; refresh this record before changing the required Pi version by re-running the live guard, picker regression, and strict typecheck above (point `FM_PI_PACKAGE_DIR` at a matching npm install when one exists) and by watching the branch's own fallback line - every branch failure degrades to the pre-branch wake-to-main path by construction, which `tests/fm-pi-branch-extension.test.sh` holds with a broken generator and the live guard holds with the real SDK.
