...
The run did not reach `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, or `muse`, which were not installed, so those adapters are unverified by this run.
...
`tests/fm-harness-adapter-instructions-live-e2e.test.sh` is an opt-in development check that sends the directly loaded router and every operation scenario across all eight harness identities to a local Ollama model, requires the generated plan as normalized JSON, and makes no external-provider call.
...
```text
ok - local model ambient-router-gemma4:e4b selected every operation scenario and all eight harness identities
...
# unavailable native tools: pi-signed grok kimi muse
```
...
Claude, Codex, OpenCode, Pi, pi-signed, Grok, Kimi, and Muse share that backend cleanup boundary; their harness-specific hook files, tokens, transcript bindings, and session-log sidecars are cleaned only after it, so no harness needs a separate endpoint parser.
