...
The routed-work regressions in `tests/fm-relay.test.sh` exercise the local-link refusal and promised-final pointer, while `tests/fm-backlog-handoff.test.sh` exercises the pre-move refusal that prevents an existing main-home binding from being orphaned.
