...
| --- | --- |
| Pi primaries | Applicable only at the existing watcher continuation after one shared `check` notification; no package byte, command, state path, or verdict enters a runtime-specific integration. |
| Pi task workers | Not applicable after inspecting runtime detection and launch ownership, because source registration has no task metadata or worker endpoint and the package is never launched through `fm-spawn`. |
| tmux and Herdr session providers | Not applicable because process-event execution calls no backend selector, capture, send, liveness, or cleanup primitive. |
...
The runner is domain-neutral and creates no endpoint, task metadata, or backlog item, so Pi, tmux, and Herdr are unaffected except through the existing `check` and status-signal paths they already consume.
...
Both axes are therefore unaffected by construction rather than by assumption: Pi re-arms from that same exit, and tmux or Herdr supplies endpoint state only to the pane paths this change does not touch.
Away and quiet posture use this same watcher path.
