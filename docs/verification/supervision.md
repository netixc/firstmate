...
The test runs the real model through Firstmate's Pi parent launcher, asks it to execute the public harness detector from a Pi child process, and requires exact `pi` output.
It then launches real Pi through the same parent boundary in the production shell-and-tmux shape, interrupts that live process, transactionally relaunches it with model and effort metadata preserved, and exits the verified replacement.
