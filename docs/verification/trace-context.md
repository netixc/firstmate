...
The spawn-path integration suite `tests/fm-trace-context-spawn.test.sh`, hermetic against an ambient `FM_TRACE_CONTEXT`, drives `bin/fm-spawn.sh` end to end with a fake Herdr endpoint and a real isolated git worktree: enabled, one resolved carrier is recorded as `traceparent=` only after the identical `TRACEPARENT` export is sent before launch; disabled, neither is written nor sent; failed or unsafe delivery preserves the refusal boundary; duplicate Secondmate preflight leaves inherited trace configuration unchanged; relaunch reuses the recorded carrier; and spawns use the frozen home-session decision.
