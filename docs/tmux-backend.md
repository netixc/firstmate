...
It classifies recognized Codex, OpenCode, Pi, pi-signed, Grok, Kimi, and Cursor process identities as `alive`, common shells as `dead`, an authoritatively absent window as `missing`, unreadable state as `unreadable`, and every other process as `ambiguous`.
