...
- Stdin JSON at `.tool_input.command` for Codex and Cursor.
...
source '<scratch-project>/config/relay.env'; bin/fm-watch-checkpoint.sh --seconds 180
...
Every row runs through Codex-shaped stdin, Grok-shaped stdin, OpenCode-shaped CLI, Pi-shaped CLI, and Cursor-shaped stdin entry forms.
