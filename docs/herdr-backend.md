...
A persistent child of that shell forces the plain explicit close, which on those releases moves the active workspace before the restore backstop pulls it back.
...
The floor reads both the installed client and the selected named session's running server while that server is running, requires both applicable releases to pass, and uses only the client when status positively reports no running server.
An unreadable server state or release is treated as unsupported rather than guessed at.
An explicit `on` is honored below the floor, so a home that deliberately opted in is never silently downgraded and accepts the documented focus move with exact prior-tab restore as its backstop.
The floor has a single owner, the spawn-time gate, so cleanup for a projection that already exists always runs and never strands a workspace.
Upgrading Herdr to 0.8.0 or newer is the fix; writing `off` is the immediate mitigation for a home that cannot upgrade yet.
The setting is inherited into secondmate homes through the normal configuration-convergence owner, and absent primary and destination files both mean the same unconfigured default, so generic absence mirroring converges without deciding the release-dependent floor.
...
Native registration can classify Pi without guessing from a generic interpreter name.
...
It rejects every other supervisor value rather than applying the wrong transport.
...
- Herdr remains protocol-gated and fails safely when required capabilities are unavailable.
