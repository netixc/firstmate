...
`bin/fm-cd-pretool-check.sh` supports the Pi entry shape, with pi-signed sharing it:
...
Missing Node, a missing policy owner, or an invalid policy response all step aside with exit 0 and no output.
