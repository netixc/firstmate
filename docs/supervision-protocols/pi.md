...
2. Confirm the plain Pi primary auto-loaded both project extensions after approving project trust once per clone; if not, restart `pi` with `-e __FM_PI_TURNEND_EXT__ -e __FM_PI_EXT__` as a trust-free fallback.
