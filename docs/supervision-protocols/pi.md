...
3. Session start and the first cycle are extension-owned on Pi.
   The turn-end extension runs `bin/fm-session-start.sh` once per Pi runtime and delivers the complete digest as context, then starts the one initial cycle itself when supervision is missing.
   Do not run `bin/fm-session-start.sh` again after that digest arrives, and do not make a first `fm_watch_arm_pi` call for it.
...
4. If the extension says no live session holds the lock, or the digest reports the claim could not run, run `bin/fm-session-start.sh` to reclaim the session lock, then call `fm_watch_arm_pi` again.
...
10. Operational follow-ups are single-flight: while one is queued or running, further wakes coalesce into it rather than starting another turn.
   Always drain first, because one operational turn is expected to handle every durable record that accumulated.
   An operational turn that runs no drain, session start, or named repair counts as a failed delivery, gets one bounded retry, and then stops with a compact escalation.
