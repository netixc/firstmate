...
The injection sits at the unconditional pre-launch export site, so it covers ship, scout, and Secondmate spawns across `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, and `cursor`.
