...
The injection sits at the unconditional pre-launch export site, so it covers plain Pi ship, scout, and secondmate spawns.
This is the same coverage `GOTMPDIR` already has and requires no trace-specific `launch_template()` behavior.
Ship, scout, and Secondmate spawns reach that site through the sole Herdr provider.
