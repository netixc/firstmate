...
The injection sits at the unconditional pre-launch export site, so it covers ship and scout spawns across `codex`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, and `agy`, plus Secondmate spawns across that same set except the deliberately crewmate-only `agy` adapter.
