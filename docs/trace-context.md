...
The injection sits at the unconditional pre-launch export site, so it covers ship and scout spawns across `codex`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, `gemini`, `muse`, `rovo`, and `agy`, plus Secondmate spawns across that same set except the deliberately crewmate-only `gemini`, `muse`, `rovo`, and `agy` adapters.
