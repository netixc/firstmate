...
- Pi runtime ownership spans primary admission and session locking in `bin/fm-session-lock-lib.sh`, launch and hook mechanics in `bin/fm-spawn.sh`, semantic busy sources and trust gates in `bin/fm-busy-lib.sh`, delivery-only rendered guards in `bin/fm-composer-lib.sh`, cleanup in `bin/fm-teardown.sh`, and facts in `.agents/skills/pi-operations/SKILL.md`; the `firstmate-coding-guidelines` skill owns the validation policy for checks that depend on Pi.
- Changes to Herdr session behavior (`bin/fm-herdr.sh` and its callers) keep current setup and limits in [`docs/herdr-backend.md`](docs/herdr-backend.md) and active empirical evidence in [`docs/verification/herdr-runtime.md`](docs/verification/herdr-runtime.md).
...
- `README.md` stays a concise overview plus pointers: it never carries a wall of inline detail.
  Route detail to the most specific `docs/` file (architecture, configuration, or the Herdr guide) and link to it instead.
...
[`docs/configuration.md`](docs/configuration.md#gate-defaults-no-mistakesyaml) owns firstmate's evidence placement and lint pin; local `.no-mistakes/` state must not be committed to a code branch.
...
Tests that need real Herdr or an explicit opt-in (real Herdr smoke tests and the live Pi regression) skip themselves and print the tool or environment gate needed to enable them, so the portable suite remains safe on machines without those tools.
The [Herdr guide](docs/herdr-backend.md#destructive-lab-safety) owns the lane's isolation boundary, while [Herdr runtime verification](docs/verification/herdr-runtime.md) owns active empirical evidence; live Pi credential tests remain opt-in.
