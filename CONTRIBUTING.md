...
- Only shared material is tracked: `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `.agents/skills/`, and `skills/`.
  ...
  A local `config/backend` file explicitly overrides runtime auto-detection for new task endpoints and stays gitignored; spawn-supported values are `tmux` and experimental `herdr`.
...
Local `.no-mistakes/` state stays out of code branches, while `.no-mistakes.yaml` publishes test evidence to the repository's orphan `no-mistakes/evidence` branch and pins the gate's lint command to `bin/fm-lint.sh`, matching the Linux CI lint job.
...
Do not commit `.no-mistakes/evidence/` to a code branch.
