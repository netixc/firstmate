...
- Only shared material is tracked: `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `.agents/skills/`, and `skills/`.
  ...
  Herdr is the only session provider; a local `config/backend` may assert `herdr`, while every other value is rejected without fallback.
...
bin/fm-test-run.sh --lane portable-serial   # portable serial remainder (watcher/AFK/Herdr/stateful)
