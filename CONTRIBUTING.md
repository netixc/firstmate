...
It evaluates every PR opening and body edit independently, reruns on reopening, and validates the live PR body and head instead of archived event facts.
Head synchronization alone does not start an attestation check; the no-mistakes body edit validates the new head without leaving a superseded stale-payload failure, and immutable body-event groups prevent a later edit from replacing an earlier pending compliance check.
