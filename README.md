...
- A verified primary agent harness: Grok, Pi, `pi-signed`, Codex, OpenCode, or Cursor Agent CLI.
...
Grok uses background-notify wake cycles, and Pi uses its tracked primary watcher extension.
...
Grok uses the slash form shown here; codex uses the same names with `$`, such as `$afk`.
...
- [docs/supervision-protocols/](docs/supervision-protocols/) - rendered primary-harness watcher protocols for Codex, OpenCode, Pi and `pi-signed`, Grok, Cursor, and unknown harness fallback.
