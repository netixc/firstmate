...
- A verified primary agent harness: Grok, Pi, `pi-signed`, Oh My Pi (`omp`), Codex, OpenCode, or Cursor Agent CLI.
...
Grok and Cursor use the slash form shown here; Codex uses the same names with `$`, such as `$afk`, while other harnesses accept natural-language invocation when their exact command form is not verified.
...
- [docs/supervision-protocols/](docs/supervision-protocols/) - rendered primary-harness watcher protocols for Codex, OpenCode, Pi and `pi-signed`, omp, Grok, Cursor, and unknown harness fallback.
