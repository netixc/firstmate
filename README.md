...
- A verified primary agent harness: Grok, Pi, `pi-signed`, Oh My Pi (`omp`), Codex, or OpenCode.
...
**Grok and Pi are equal co-primary recommendations** for running the primary firstmate session, with `pi-signed` supported as Pi's distinct signed-wrapper identity.
Grok uses background-notify wake cycles, while Pi uses its tracked primary watcher extension.
...
Codex and OpenCode are also verified and supported as primary harnesses; Codex uses bounded foreground checkpoints, and OpenCode uses a TUI plugin, so both carry more harness-specific supervision tradeoffs than the co-primaries.
...
Then launch one of these recommended primary harnesses; AGENTS.md takes over from there:
...
Grok uses the slash form shown here; Codex uses the same names with `$`, such as `$afk`, while other harnesses accept natural-language invocation when their exact command form is not verified.
...
- [docs/supervision-protocols/](docs/supervision-protocols/) - rendered primary-harness watcher protocols for Codex, OpenCode, Pi and `pi-signed`, omp, Grok, and unknown harness fallback.
