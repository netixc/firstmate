...
- `UNSUPPORTED_HOST: <platform> - ...` - stop all work on that host without dispatching, installing, or attempting fleet repair because mandatory startup has not acquired mutation authority or created local records.
  Tell the captain that Firstmate hosts require macOS or Linux and name the detected platform.
  Recommend a fresh WSL2 setup using Linux Node, Bash, Git, the selected session backend, and the rest of the Linux toolchain when the machine is Windows, or a macOS/Linux host otherwise; Windows clients and Windows-targeted repositories remain unaffected.
