...
Claude Code 2.1.216 reports `auto mode unavailable for this model` for an explicit Haiku profile and falls back to manual permissions, so `fm-spawn.sh` requires every Claude worker to name an explicit verified Sonnet or Opus model and rejects omitted, literal `default`, Haiku, and unsupported model values.
Raw Claude launch commands additionally require `--permission-mode auto` and reject `--dangerously-skip-permissions`.
