...
Its default selection is the SAFE one: `● 1. Trust folder (<name>)`, with `2. Trust parent folder (<parent>)` and `3. Don't trust` unselected.
...
`GEMINI_CLI=1` is load-bearing rather than a fast path, so `../../../../../bin/fm-harness.sh` checks it before process ancestry.
The spawn additionally clears foreign runtime markers at the launch boundary.
...
A verified tool process also carried an inherited launcher's value, so `AGENT` identifies the launcher, not the running harness.
...
It is deliberately NOT the worktree's `.gemini/settings.json`: that path is the PROJECT's own committed settings file, so writing it would clobber a project's configuration and retiring it would delete a tracked file.
...
`AfterAgent` fires on a manual Escape interrupt as well as on normal completion, so the interrupt closes Gemini's busy record.
