...
| Skill invocation | `$<skill>`, for example `$no-mistakes`; unsupported slash forms are rejected as "Unrecognized command". |
...
| Marker | None; identity comes from ancestry. Verified on 2026-09-01 with codex-cli 0.152.0: the pane process is the `node` npm shim and the native `codex` binary runs as its foreground child, so a tool subprocess reaches the native name directly while the shim itself is identified from its script path. |
