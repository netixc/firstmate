...
description: >-
  ...
  Use on an "relay-mention <request_id>" check wake to read the stashed mention, classify it, act autonomously on eligible requests, reply or dismiss, and link spawned work.
  Also use on an "relay-error ..." check wake to report the Relay configuration blocker instead of answering a mention.
...
A mention arrives through the watcher as a `check:` wake whose payload is `relay-mention <request_id>`.
...
If you ever see an `relay-mention` wake without Relay configured, do nothing.
A `check:` wake can also carry `relay-error ...` instead of `relay-mention <request_id>` - that is a poll or relay configuration problem, not a mention to answer.
...
For a reply-worthy mention, the only non-posting path is dry-run (`FM_RELAY_DRY_RUN`; see below) - a testing switch, not a permission gate.
...
- **Work that spawns a real, longer-running job** (dispatching a crewmate, a scout investigation, a ship task) cannot report an outcome yet, so it follows **acknowledge first -> act -> follow up on completion**:
  1. **Acknowledge first.** Post an immediate, public-safe reply that you have the captain's order and are on it (the normal answer endpoint, via `bin/fm-relay-reply.sh`). This is the legitimate, work-backed version of "aye, will do": it is paired with actually starting the work in the same turn, never a promise left empty.
  ...
  3. **Bind the follow-up to wherever the work actually lives, before clearing the inbox.**
     **The decision rule: work that stays in this home takes the lightweight link; work routed to a local second mate takes a promised-final commitment bound to that second mate's home.**
     Each mechanism can only reach the filesystem it was built for, so choosing the wrong one or treating a remote route as reachable orphans the public promise.
     - **Local task (this home spawned it):** `bin/fm-relay-link.sh <task-id> <request_id>` records the request id, a timestamp, a follow-up counter, and Discord reply-budget context.
       Do this right after the task is spawned, and always **before** removing the inbox file (step 2f).
       Linking before cleanup lets `bin/fm-relay-link.sh` copy the context directly from the inbox, while the durable per-request context recorded by the poll preserves it independently for delayed and concurrent follow-ups.
       The exact resolution and fail-safe posting contract is owned by `docs/configuration.md`.
       If a recovery respawns the same relay request onto a successor task, relink with the paired `--carry-count <n> --carry-ts <epoch>` flags plus any prior `relay_platform=` and `relay_reply_max_chars=` as `--carry-platform discord --carry-max <n>` so the successor keeps the consumed follow-up count, original 7-day window, and Discord reply split budget.
     - **Local-second-mate-routed work (the request's project or domain belongs to a registered local second mate, so the work is or will be routed there):** the link cannot be used at all.
       It writes into this home's own `state/<task-id>.meta`, and a routed task's record lives in the second mate's home, so `bin/fm-relay-link.sh` refuses and points you back here.
     ...
     - **Remote-second-mate-routed work:** the current promised-final event transport is same-filesystem only, as documented in `docs/remote-secondmates.md`, so neither mechanism can bind that remote work.
       Do not post an acknowledgement that promises a later result through Relay unless the work is kept in a home reachable by one of the two supported mechanisms.
...
- **Pure acknowledgment** ("thanks", a reaction, a loop-closing nicety with nothing to add) - skip: post nothing, but first **dismiss it at the relay** (`bin/fm-relay-dismiss.sh <request_id>`) so the relay drops the request and stops re-offering it, then clear the inbox file.
...
Compose the reply as one piece of prose; if it is genuinely too long for one message, `bin/fm-relay-reply.sh` automatically splits it into a Discord-sized numbered thread on fenced-code, paragraph, line, and word boundaries.
...
The watcher coalesces same-key `check:` wakes, so one `relay-mention` wake can stand in for several pending mentions.
Treat `state/relay-inbox/` as the source of truth and process **every** file you find there, not just the `request_id` named in the wake.
...
2. **Drain every pending mention.** For each `state/relay-inbox/*.json` file:
   a. Read the object: you need `request_id`, `text`, `in_reply_to`, and - when present - `in_reply_to_chain`.
      ...
      Never look for or name a platform message id; the Relay binds replies through `request_id`.
   ...
   c. **Act on an actionable request through the normal lifecycle.** Treat it exactly as a captain prompt typed in session: run ordinary intake (resolve the project), then file the backlog item, dispatch a crewmate, start a scout, or ship through the gate - whatever the request calls for.
      ...
      **If the request spawned a real, longer-running task in THIS home** (you ran `bin/fm-spawn.sh` here), link that task to this mention so milestone and completion follow-ups can be posted: `bin/fm-relay-link.sh <task-id> <request_id>`.
      **Link here, in step 2c, before the step 2f inbox cleanup** - `bin/fm-relay-link.sh` can copy both the mention's reply platform and explicit budget from the still-present inbox payload without a relay lookup.
      ...
      **If intake routes the work to a local second mate instead**, do not reach for the link: register the typed promised-final commitment bound to `secondmate:<id>` and brief the routed worker with its reporting command (step 3 of "acknowledge first, act, then follow up on completion", with the commands in "Promised final replies").
      A remote second mate cannot receive this same-filesystem reporting command, so keep the work on a supported route or do not post an acknowledgement that promises a later Relay result.
   ...
   e. **Submit it without ever inlining the reply into a shell command.**
      ...
      ```sh
      bin/fm-relay-reply.sh <request_id> --text-file <path-to-reply-file>
      ```

      (`bin/fm-relay-reply.sh <request_id> -`, reading the reply on stdin, is equally fine.) It echoes the `request_id` and exits 0 on success; non-zero on a failed live post or failed dry-run record.
   ...
   e-skip. **For a skip, dismiss it at the relay instead of replying.** A pure acknowledgment gets no reply, but clearing only the local inbox file is not enough: the relay keeps re-offering that request on every poll until it times out to a polite "offline" auto-reply. So before clearing the file, tell the relay to drop the request:
      ...
      ```sh
      bin/fm-relay-dismiss.sh <request_id>
      ```

      It posts nothing, stops the re-offer, and prevents the offline auto-reply; it echoes the `request_id` and exits 0 on success (it honors `FM_RELAY_DRY_RUN` like `bin/fm-relay-reply.sh`, recording the would-be dismiss to `state/relay-outbox/` instead of posting). Do **not** call `bin/fm-relay-reply.sh` for a skip.
   f. **On success (a posted reply, or a relay dismiss for a skip), remove that inbox file:** `rm -f state/relay-inbox/<request_id>.json` (and your temporary reply file).
   ...
   g. **On failure** (a non-zero exit from `bin/fm-relay-reply.sh` or `bin/fm-relay-dismiss.sh`), leave that inbox file in place, move on to the next, and do not retry blindly.
...
When `FM_RELAY_DRY_RUN` is set (truthy, in the environment or `.env`), `bin/fm-relay-reply.sh` does **not** post and `bin/fm-relay-dismiss.sh` does **not** call the relay.
The reply client records the full would-be reply payload to `state/relay-outbox/<request_id>.json` (`{request_id, text}` for one message, or `{request_id, text, texts}` for a thread), prints a `DRY RUN` summary to stderr, and still echoes the `request_id` and exits 0.
...
Your procedure does not change: compose as usual and call `bin/fm-relay-reply.sh ... --text-file <path>`, or call `bin/fm-relay-dismiss.sh <request_id>` for a skip.
...
Inspect `state/relay-outbox/` to see exactly what would have been posted.
The completion follow-up honors `FM_RELAY_DRY_RUN` the same way (it flows through `bin/fm-relay-reply.sh --followup`): the would-be follow-up is recorded to `state/relay-outbox/`, and the local counter and link mutate exactly as a live post would.
A non-final dry-run follow-up increments `relay_followups` and keeps the link while under the cap; `--final`, the cap, or an expired window clears it, so the whole acknowledge -> act -> follow-up loop is testable without a public post.
...
- If a linked task is replaced by a successor for the same relay request, carry the prior `relay_followups=`, `relay_request_ts=`, `relay_platform=`, and `relay_reply_max_chars=` values with `bin/fm-relay-link.sh <new-task-id> <request_id> --carry-count <n> --carry-ts <epoch> --carry-platform discord --carry-max <n>` so recovery preserves the consumed budget, original window, and Discord reply split budget after the inbox file is gone.
- On each such milestone, firstmate checks whether a follow-up is still due with `bin/fm-relay-followup.sh --check <task-id>` (prints the `request_id` when the link exists, the count is under the cap, and the window has not lapsed; silent otherwise, pruning an exhausted or expired link).
- If due, it composes a short, public-safe update and posts it with `bin/fm-relay-followup.sh <task-id> --text-file <path>` (or stdin), which posts via the relay's follow-up endpoint; a successful non-final post increments the counter and keeps the link so a later milestone can still post against it.
  When the update carries one real visual artifact, add `--image <path>`; the helper forwards it to `bin/fm-relay-reply.sh --followup` so the same image contract used for ordinary replies applies here too.
- On a terminal wake (PR merged / scout report / local merge / failed), firstmate posts the task's **final** outcome ("done, here's the result"; for a failure, an honest "this one didn't pan out") with `bin/fm-relay-followup.sh <task-id> --final --text-file <path>` only when no promised-final public commitment is registered for that work. When the promised-final procedure above applies, `bin/fm-public-followup.sh consume` and `deliver` own the terminal reply and clear the legacy link at the validated receipt boundary, so do not call `fm-relay-followup.sh --final` for the same outcome. If delivery reports that link cleanup needs reconciliation, do not post anything else; `bin/fm-relay-followup.sh --clear <task-id>` is the clear-only recovery command in the bound work home.
...
This is the only mechanism that reaches work in another home on the same filesystem.
The originating Relay home creates and retains the obligation, consent, and Discord thread binding; `--work-home secondmate:<id>` identifies the local home doing the work and does not transfer ownership of the public reply.
The routed worker only emits the typed terminal result named by `brief`, and the originating home alone reconciles and posts it.
The marked secondmate request's missing-report recovery remains owned by `bin/fm-pending-reply-lib.sh`; it does not transfer or close the public-followup obligation.

**When you promise a final (including every Relay request whose work is routed to a local second mate):**
...
3. Put `bin/fm-public-followup.sh brief <obligation-id>` output straight into the worker's brief.
   ...
   For local-second-mate-routed work, carry that output in the routed item's instructions so the command reaches the worker that owns the terminal result.
...
2. For each ready commitment, run `bin/fm-public-followup.sh deliver <obligation-id>`.
   ...
   Delivery clears the bound task's Relay task link at the validated receipt boundary; if it reports a cleanup failure, use its reconciliation message and do not post a duplicate final.
...
- The direct author is always your own captain (owner-only routing), and in live mode you answer and act on eligible requests **autonomously**: enabling Relay is the captain's standing authorization, so never ask the captain before posting and never hold a worthwhile reply for a chat-side OK. For reply-worthy mentions, dry-run (`FM_RELAY_DRY_RUN`) is the only non-posting path; pure acknowledgments use the relay dismiss path instead.
- An actionable mention is **acted on** through the normal lifecycle (intake, backlog, dispatch, investigate, ship), not merely replied to. Work that finishes now gets one outcome reply; work that spawns a real task gets an **acknowledgement now** plus up to three **completion follow-ups** over time, ending with a `--final` one when no typed promised-final commitment exists. Bind those follow-ups by where the work lives: a task in this home takes `bin/fm-relay-link.sh`, and work routed to a local second mate takes the promised-final procedure above. A reply alone, with no work behind an actionable ask, is the bug to avoid.
...
- One answered mention = one reply (plus up to three completion follow-ups for a spawned task, spent only on genuine milestones); a skipped mention posts no reply but is **dismissed at the relay** (`bin/fm-relay-dismiss.sh`) so the relay drops it rather than re-offering it (which would otherwise churn every poll and end in an "offline" auto-reply). A single wake may cover several pending mentions - drain them all.
...
- Never edit `bin/fm-relay-poll.sh`, `bin/fm-relay-reply.sh`, or the watcher to "answer faster"; the cadence is handled by the locked session-start bootstrap step.
