...
`harness-adapters` owns Pi model/provider discovery and thinking fallback.
...
For each candidate, preserve explicit Pi `model` and `provider`; `harness-adapters` owns the catalog evidence:
...
A provider's prepaid balance is unrelated to `percentRemaining`; never read it as exhaustion.
...
You establish those relations yourself, in the open, from Pi's authoritative catalog plus the one intake snapshot.
...
A candidate authenticates through its own Pi model/provider surface; another command-line tool can never gate it.
...
- Reserve login wording for that proven-unusable case, and name the model, surface, and evidence.

When a credential classification is the only thing standing between a candidate and a block, keep it as uncertainty unless Pi's authenticated catalog or `quota-axi auth --json` provides concrete contradictory evidence.
Never launch another provider tool or probe a credential store the candidate does not use.
...
9. Genuine ties: stop and report every tied candidate for captain choice.
   Do not select by array order, model name, or another arbitrary identity ordering.
...
A blocked credential report must name the Pi model, authentication surface, and concrete failure evidence; never emit a bare provider-authentication statement.
