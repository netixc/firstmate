...
- A user-owned local skill: a directory under `.agents/skills/<freeform-name>/` whose path is appended to the active home clone's repository-local exclude file, never to a `.gitignore`.
  ...
  The name is freeform with no user-vs-firstmate naming convention, the skill stays per-home and untracked, and migration requires confirming that the active runtime discovers and loads that directory without relying on git status.
