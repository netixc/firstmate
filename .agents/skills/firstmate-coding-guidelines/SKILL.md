...
Before changing shared tracked behavior, review every affected supported primary harness and the Herdr task-runtime integration rather than checking only the adapters active in the current fleet.
...
- A portable regression in `tests/` that pins the logic with real processes and no harness, so CI enforces the classifier independently of any live runtime.
