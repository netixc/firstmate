...
Delivery uses the existing routed secondmate path (`fm-send`) with only a single-line `CONFIG_REREAD: <absolute generation-specific instruction path>` pointer; a failed publication or retry-marker write retains the exact generation in a bounded private retry queue until it can be delivered, a failed send records a per-generation durable retry marker when possible, and both failures surface the same concrete `CONFIG_REREAD:` diagnostic without claiming the live agent already re-read the values.
