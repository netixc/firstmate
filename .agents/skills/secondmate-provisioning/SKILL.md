...
For a local or remote route, an explicit per-spawn `--harness pi` selection starts clean on model and effort too unless the caller also passes explicit `--model` or `--effort`; every other harness value and every raw launch command is refused.
...
Because this resolves from the file on every spawn, the pin is durable across every respawn (recovery, `/updatefirstmate`, restart) exactly like the harness axis itself - e.g. `config/secondmate-harness` containing `pi anthropic/claude-opus-4` keeps a secondmate pinned to that Pi model even if the primary's own default model later changes.
