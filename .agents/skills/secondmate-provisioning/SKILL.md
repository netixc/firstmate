...
For a remote route admitted by the current transport protocol, `/updatefirstmate` is the one path that still follows that copy: it first updates the remote code root from its own origin, then syncs the home to that refreshed code-root commit.
Legacy routes remain unchanged until the operator completes the attended-upgrade prerequisite in [`docs/remote-secondmates.md`](../../../docs/remote-secondmates.md#retire-a-legacy-worker-before-upgrade).
