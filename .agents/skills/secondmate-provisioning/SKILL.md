...
A remote second-mate agent always runs on the Herdr backend and every seed, launch, and liveness relaunch first gates its host on `bin/fm-remote-doctor.sh` readiness, so an unready host refuses with that doctor's own gap text rather than half-creating a route; workers that second mate supervises use the same Herdr-only fresh-endpoint contract as every other home.
...
Inherited `config/backend` is a request for that secondmate home's future spawns only; it never retargets, rewrites, migrates, stops, or restarts an already-live worker endpoint.
A present primary value always converges byte-exact into validated secondmate homes, and primary absence removes the destination so those homes use the Herdr fresh-work default.
Explicit per-spawn `--backend` and `FM_BACKEND` remain stronger sources than an inherited file, but every source may select only Herdr for fresh work; any source that requests tmux refuses before endpoint creation with the rollback-only diagnostic.
This includes the secondmate agent itself on local routes and every worker a local or remote secondmate later starts, so a future secondmate cannot silently reintroduce tmux through inherited configuration or runtime markers.
...
Inherited harness defaults must not harden `fm-spawn` to reject a deliberate supported runtime choice that differs from those defaults.
The runtime-backend boundary is intentionally stricter: Herdr is the only fresh choice, while explicit existing tmux records remain rollback-operable.
