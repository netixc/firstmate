...
A bare `<harness>` (for example `codex`) behaves exactly as before - harness only, no model/effort flag - so this is fully backward-compatible.
...
Because this resolves from the file on every spawn, the pin is durable across every respawn (recovery, `/updatefirstmate`, restart) exactly like the harness axis itself - e.g. `config/secondmate-harness` containing `codex gpt-5.5` keeps a secondmate pinned to that model even if the primary's own default model later changes.
