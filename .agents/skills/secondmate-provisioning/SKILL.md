...
A Claude secondmate entry must use `opus`, `sonnet`, `claude-opus-*`, or `claude-sonnet-*`, such as `claude opus`; bare Claude, literal `default`, Haiku, Fable, and every other model value fail closed before launch.
...
Raw launch commands own their axes directly, but they cannot invoke Claude and cannot include `--dangerously-skip-permissions`; Claude secondmates must use the verified adapter.
