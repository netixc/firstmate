...
The same launch and the same locked bootstrap sweep also propagate the primary's declared inherited local material: `config/crew-dispatch.json`, `config/crew-harness`, `config/backlog-backend`, `config/herdr-child-workspaces`, and the one shared captain-preference file `data/captain-shared.md`.
...
Inheritance also copies the literal default-off `config/herdr-child-workspaces` feature flag, so successful propagation makes the primary's explicit `on` opt-in apply to ordinary crews throughout the supervisor tree and makes primary absence restore the default tab-per-task layout.
This flag is best-effort at secondmate launch: a guarded skip or copy failure prints one grouping-specific warning and does not block the launch, while later launch, bootstrap, or explicit config-push propagation can converge the home.
