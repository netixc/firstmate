...
# launch it in a non-visible tracked Herdr terminal, record its exact id,
# tear it down by that exact id, and reconcile a leaked one after a crash.
#
# Why this exists (docs/herdr-backend.md "Away-mode supervision"):
...
# script fixes. Instead this creates a non-visible Herdr workspace with
# --no-focus that never touches the captain's active tab and never uses shell
# `&` (which Herdr or the harness can reap).
#
...
fm_afk_launch_record_write() {  # <target> <workspace-id-or-native>
  local pending
  mkdir -p "$FM_AFK_LAUNCH_STATE" || return 1
...
# Read the recorded terminal into FM_AFK_REC_TARGET and FM_AFK_REC_WORKSPACE.
...
fm_afk_launch_record_read() {
  ...
  if [ "$FM_AFK_REC_TARGET" = - ]; then
    [ "$FM_AFK_REC_WORKSPACE" = native ] || return 2
  else
    case "$FM_AFK_REC_TARGET" in *:*) : ;; *) return 2 ;; esac
  fi
}
...
fm_afk_launch_terminal_absent() {  # <target>
  local target=$1 session pane out result code
  [ "$target" != - ] || return 0
  session=${target%%:*}
  pane=${target#*:}
  [ -n "$session" ] && [ -n "$pane" ] && [ "$pane" != "$target" ] || return 1
...
fm_afk_launch_wait_ready() {  # <target>
  ...
  while [ "$attempt" -lt 100 ]; do
    ...
    fm_afk_launch_terminal_alive "$target" || return 1
    sleep 0.05
  done
  return 1
}
...
fm_afk_launch_commit_terminal() {  # <target> <workspace-id> [already-recorded]
  local target=$1 workspace=$2 already_recorded=${3:-0}
  if [ "$already_recorded" -ne 1 ] && ! fm_afk_launch_record_write "$target" "$workspace"; then
    fm_afk_launch_log "failed to persist daemon terminal record; closing $target"
    fm_afk_launch_close_terminal "$target"
    return 1
  fi
  if ! fm_afk_launch_wait_ready "$target"; then
...
fm_afk_launch_reconcile() {
  ...
  if [ "$read_result" -eq 0 ]; then
    fm_afk_launch_log "reconciling leaked daemon terminal ${FM_AFK_REC_TARGET}"
    fm_afk_launch_close_recorded
...
fm_afk_launch_create_herdr() {  # <captain-target>
  ...
  if [ "$create_result" -ne 0 ] && [ -n "$wsid" ] && [ -n "$pane" ]; then
    ...
    if fm_afk_launch_record_write "$session:$pane" "$wsid"; then
...
fm_afk_launch_start() {
  ...
  if [ "$result" -eq 0 ]; then
    fm_afk_launch_create_herdr "$captain_target"
    result=$?
  fi
  if [ "$result" -ne 0 ]; then
...
fm_afk_launch_start_native() {
  ...
  if [ "$result" -eq 0 ]; then
    fm_afk_launch_record_write - native || result=1
  fi
  if [ "$result" -ne 0 ]; then
    fm_afk_launch_restore_backup "$backup" "$had_afk" || result=1
  else
