...
# launch it in a NON-VISIBLE tracked Herdr terminal, record its exact id, tear
...
# NEVER uses shell `&` (which Herdr can reap).
#
...
# captured Firstmate pane (an isolated lab pane in tests).
set -u
...
fm_afk_launch_close_terminal() {  # <target>
  ...
  fm_herdr_cli "$session" pane close "$pane" >/dev/null 2>&1
}

fm_afk_launch_terminal_absent() {  # <target>
  local target=$1 session=${1%%:*} pane=${1#*:} out result code
  [ -n "$session" ] && [ -n "$pane" ] && [ "$pane" != "$target" ] || return 1
...
fm_afk_launch_terminal_alive() {  # <target>
  local target=$1 session=${1%%:*} pane=${1#*:}
  [ -n "$session" ] && [ -n "$pane" ] && [ "$pane" != "$target" ] || return 1
  fm_herdr_cli "$session" pane get "$pane" >/dev/null 2>&1
}
...
fm_afk_launch_herdr_recover_created() {  # <session> <label>
  ...
  while [ "$attempt" -lt 20 ]; do
    attempt=$((attempt + 1))
    workspaces=$(fm_herdr_cli "$session" workspace list 2>/dev/null) || { sleep 0.05; continue; }
    ...
    [ -n "$wsid" ] || return 1
    panes=$(fm_herdr_cli "$session" pane list --workspace "$wsid" 2>/dev/null) || { sleep 0.05; continue; }
...
fm_afk_launch_reconcile() {
  ...
  if [ "$read_result" -eq 0 ]; then
    fm_afk_launch_log "reconciling leaked daemon terminal ${FM_AFK_REC_KIND}:${FM_AFK_REC_TARGET}"
    fm_afk_launch_close_recorded
