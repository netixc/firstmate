...
#
# When ShellCheck is absent from PATH, this script provisions the pinned build
# into ignored worktree-local state through bin/fm-install-shellcheck.sh and runs
# it by path, so a cached local gate command cannot silently skip the CI rule set.
#
...
# ShellCheck on PATH, and invoke it by path, so a cached pre-push gate command
# runs the SAME ShellCheck as CI without the caller updating PATH first.
if SHELLCHECK_BIN=$(command -v shellcheck); then
  :
else
  LOCAL_SHELLCHECK_DIR="$ROOT/.no-mistakes/shellcheck-$REQUIRED_SHELLCHECK/bin"
  SHELLCHECK_BIN="$LOCAL_SHELLCHECK_DIR/shellcheck"
  if [ ! -x "$SHELLCHECK_BIN" ] && ! "$ROOT/bin/fm-install-shellcheck.sh" "$LOCAL_SHELLCHECK_DIR" >&2; then
    printf 'fm-lint.sh: ShellCheck not found and the pinned %s build could not be provisioned.\n' \
