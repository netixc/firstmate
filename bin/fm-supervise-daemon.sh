...
#          FM_SUPERVISOR_TARGET     Herdr supervisor pane target override;
#                                   otherwise auto-discovered as
#                                   "<session>:<pane-id>" from Herdr's runtime
#                                   markers. There is no guessed fallback.
...
# Supervisor-pane discovery is shared with the script-owned away launcher
...
# FM_BUSY_REGEX overrides the shared Herdr busy-footer fallback.
...
# The away launcher reuses the identical resolution to pass the captain pane in
# as FM_SUPERVISOR_TARGET.
...
# Busy + composer-empty detection use the shared Herdr endpoint primitives.
...
pane_is_busy() {  # <target>
  local target=$1 bs tail40
  bs=$(fm_backend_busy_state "$target" 2>/dev/null)
  case "$bs" in
    busy) return 0 ;;
  esac
  tail40=$(fm_backend_capture "$target" 40 2>/dev/null) || return 1
  printf '%s' "$tail40" | grep -v '^[[:space:]]*$' | tail -6 \
    | grep -qiE "${FM_BUSY_REGEX:-$FM_BACKEND_BUSY_REGEX_DEFAULT}"
}

# pane_input_pending: the standalone "is there real unsubmitted text" predicate,
# dispatching through fm_backend_composer_state. inject_msg
...
pane_input_pending() {  # <target>
  local target=$1
  [ "$(fm_backend_composer_state "$target" 2>/dev/null)" = pending ]
}

stale_window_is_busy() {  # <window> <state>
  local win=$1 state=$2 label tail40 bs
  label="fm-$(window_to_task "$win" "$state")"
  tail40=$(fm_backend_capture "$win" 40 "$label" 2>/dev/null) || return 2
  bs=$(fm_backend_busy_state "$win" 2>/dev/null)
  case "$bs" in
    busy) return 0 ;;
  esac
  printf '%s' "$tail40" | grep -v '^[[:space:]]*$' | tail -6 \
...
# --- active wedge alert ------------------------------------------------------
# A wedged primary once had only the passive state/.subsuper-inject-wedged
# marker, which nothing surfaced until the next fleet action. These helpers add
# a configurable active alert that does not depend on any pane: a macOS notification, a
...
# skipped, never crashing the daemon loop - while the durable marker remains.
#
...
inject_wedge_alarm() {  # <state> <age-seconds>
  ...
  # This can reach the captain even when every pane is unreadable. The marker
  # above stays the durable record whether or not any channel fires.
  if [ "$notify" -eq 1 ]; then
...
housekeeping() {  # <state>
  ...
  for marker in "$state"/.subsuper-stale-*; do
    ...
    # Reconstruct the Herdr target from metadata.
...
#     classifier may have missed). Cheap: status files only, no Herdr call. The
...
#   - SUBMIT ACK = Herdr reports `empty` after Enter, meaning native agent-state
#     observed a real turn start.
...
inject_msg() {  # <message> [state]
  ...
  [ -n "$target" ] || return 1
  fm_backend_target_exists "$target" || return 1
  ...
  composer=$(fm_backend_composer_state "$target" 2>/dev/null)
  ...
  # Dispatches through the shared Herdr submit primitive.
  retries=${FM_INJECT_CONFIRM_RETRIES:-$INJECT_CONFIRM_RETRIES_DEFAULT}
  sleep_s=${FM_INJECT_CONFIRM_SLEEP:-$INJECT_CONFIRM_SLEEP_DEFAULT}
  verdict=$(fm_backend_send_text_submit "$target" "$msg" "$retries" "$sleep_s" "$sleep_s")
  if [ "$verdict" = empty ]; then
    return 0  # Herdr confirmed the submit.
  fi
...
fm_super_main() {
  ...
  # Priority: FM_SUPERVISOR_TARGET override, then Herdr's injected pane markers.
  # No guessed fallback can safely identify the captain's pane.
  ...
  if ! discovered=$(discover_supervisor_target); then
    echo "error: could not resolve the Herdr supervisor pane; set FM_SUPERVISOR_TARGET or launch from a Herdr pane" >&2
    log "startup failed: supervisor target unresolved"
  ...
  if ! fm_backend_target_exists "$TARGET"; then
