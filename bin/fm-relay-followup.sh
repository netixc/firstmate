#!/usr/bin/env bash
# Post a completion follow-up for a Relay-linked task, up to three within a
# 7-day window, and manage the link's counter.
#
# A Relay mention that spawned real work is linked to its task by fm-relay-link.sh
# (relay_request/relay_request_ts/relay_followups plus optional reply context in
...
#   fm-relay-followup.sh --check <task-id>
...
#   fm-relay-followup.sh --clear <task-id>
#     idempotently removes only the Relay follow-up metadata for a typed terminal
#     outcome.
#
# Post (after composing the reply to a file or stdin):
#   fm-relay-followup.sh <task-id> [--image <path>] [--final] --text-file <path>
#   fm-relay-followup.sh <task-id> [--image <path>] [--final] -
#     Linked, within window, and under the cap: posts ONE follow-up via
#       fm-relay-reply.sh --followup.
...
#       fm-relay-reply.sh): clears the link and skips quietly, exactly like a
...
#       of retrying forever.
#       On fm-relay-reply's fail-safe refusal (exit 8: platform or explicit budget
...
#
# Dry-run (FM_RELAY_DRY_RUN) flows through fm-relay-reply.sh: the follow-up is recorded to
# state/relay-outbox/<request_id>.json instead of posted, and the counter/link are
...
#
# The window is FM_RELAY_FOLLOWUP_MAX_AGE_SECS (default 604800, 7 days). The cap is
# FM_RELAY_FOLLOWUP_MAX_COUNT (default 3). FM_RELAY_NOW_OVERRIDE pins "now" for
# deterministic tests. Meta read/write lives in fm-relay-lib.sh.
set -u
...
usage() {
  echo "usage: fm-relay-followup.sh --check <task-id> | --clear <task-id> | <task-id> [--image <path>] [--final] --text-file <path> | <task-id> [--image <path>] [--final] -" >&2
}

help() {
  cat <<'EOF'
usage: fm-relay-followup.sh --check <task-id>
       fm-relay-followup.sh --clear <task-id>
       fm-relay-followup.sh <task-id> [--image <path>] [--final] --text-file <path>
       fm-relay-followup.sh <task-id> [--image <path>] [--final] -

Post a completion follow-up (up to 3 per link, within a 7-day window) for an
Relay-linked task and manage the link's follow-up counter.

Options:
  --check          Print the request_id when a follow-up is due.
  --clear          Clear only the Relay follow-up link; never post.
...
MAX_COUNT=${FM_RELAY_FOLLOWUP_MAX_COUNT:-3}
case "$MAX_COUNT" in
  ''|*[!0-9]*) MAX_COUNT=3 ;;
esac
  ...
  while [ "$#" -gt 0 ]; do
    case "$1" in
      ...
      --image)
        ...
        shift
        if [ "$#" -lt 1 ] || [ -z "$1" ]; then
          echo "fm-relay-followup: missing --image path" >&2
          usage
          exit 2
        fi
        TS_ARGS+=("$1")
...
case "$ID" in
  ''|.*|*[!A-Za-z0-9._-]*) echo "fm-relay-followup: unsafe task id: $ID" >&2; exit 2 ;;
esac

META="$STATE/$ID.meta"
if [ "$MODE" = clear ]; then
  fm_relay_meta_link_clear "$META" \
    || { echo "fm-relay-followup: could not clear the link in state/$ID.meta" >&2; exit 1; }
  printf '%s\n' "$ID"
  exit 0
fi

RID=$(fm_relay_meta_get "$META" relay_request)
TS=$(fm_relay_meta_get "$META" relay_request_ts)
COUNT=$(fm_relay_meta_get "$META" relay_followups)
REQ_PLATFORM=$(fm_relay_meta_get "$META" relay_platform)
REQ_REPLY_MAX=$(fm_relay_meta_get "$META" relay_reply_max_chars)
case "$COUNT" in
  ''|*[!0-9]*) COUNT=0 ;;
esac

# Not linked: this task did not originate from a Relay mention. Detection fails;
# a post is simply a no-op success (firstmate need not special-case it).
if [ -z "$RID" ]; then
  if [ "$MODE" = check ]; then
    exit 1
  fi
  echo "fm-relay-followup: $ID is not Relay-linked; nothing to post" >&2
  exit 0
fi

NOW=${FM_RELAY_NOW_OVERRIDE:-$(date +%s)}
case "$NOW" in
  ''|*[!0-9]*) echo "fm-relay-followup: could not read the current time" >&2; exit 1 ;;
esac
...
if [ "$EXPIRED" = 1 ]; then
  fm_relay_meta_link_clear "$META" || echo "fm-relay-followup: warning: could not clear the elapsed link in state/$ID.meta" >&2
  if [ "$MODE" = check ]; then
    exit 1
  fi
  echo "fm-relay-followup: $REASON for $ID; skipped and cleared the link" >&2
  exit 0
fi
...
# Post the follow-up. fm-relay-reply owns text reading, thread-split, dry-run, the
...
case "$REQ_PLATFORM" in
  discord) REPLY_ENV+=("FM_RELAY_REQUEST_PLATFORM=$REQ_PLATFORM") ;;
esac
case "$REQ_REPLY_MAX" in
  ''|*[!0-9]*) ;;
  *) REPLY_ENV+=("FM_RELAY_REQUEST_REPLY_MAX_CHARS=$REQ_REPLY_MAX") ;;
esac
if [ "${#REPLY_ENV[@]}" -gt 0 ]; then
  env "${REPLY_ENV[@]}" "$FM_ROOT/bin/fm-relay-reply.sh" "$RID" --followup "${TS_ARGS[@]}" >/dev/null
else
  "$FM_ROOT/bin/fm-relay-reply.sh" "$RID" --followup "${TS_ARGS[@]}" >/dev/null
fi
post_rc=$?
...
case "$post_rc" in
  0)
    ...
    if [ "$FINAL" = 1 ] || [ "$NEWCOUNT" -ge "$MAX_COUNT" ]; then
      if ! fm_relay_meta_link_clear "$META"; then
        echo "fm-relay-followup: error: posted but could not clear the link in state/$ID.meta" >&2
        exit 1
      fi
    elif ! fm_relay_meta_followups_set "$META" "$NEWCOUNT"; then
      if ! fm_relay_meta_link_clear "$META"; then
        echo "fm-relay-followup: error: posted but could not record the follow-up count or clear the link in state/$ID.meta" >&2
        exit 1
      fi
      echo "fm-relay-followup: warning: posted but could not record the follow-up count in state/$ID.meta; cleared the link to avoid duplicate follow-ups" >&2
    fi
    printf '%s\n' "$RID"
    exit 0
    ;;
  8)
    # fm-relay-reply.sh refused this follow-up (exit 8) because it could not
    ...
    # link here.
    echo "fm-relay-followup: follow-up for $ID held: reply context lacks an authoritative platform or explicit budget; left the link in place to retry once both values are recoverable" >&2
    exit 1
    ;;
  9)
    # fm-relay-reply.sh distinguishes a relay rejection of this specific follow-up
    ...
    fm_relay_meta_link_clear "$META" || echo "fm-relay-followup: warning: could not clear the rejected link in state/$ID.meta" >&2
    echo "fm-relay-followup: relay rejected the follow-up for $ID (cap or window exhausted); skipped and cleared the link" >&2
    exit 0
    ;;
  *)
    # Post failed for another reason (network, auth, transport): leave the link
    # so firstmate can retry on a later pass.
    echo "fm-relay-followup: follow-up post failed for $ID; left the link in place to retry" >&2
    exit 1
    ;;
esac
