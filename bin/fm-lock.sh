...
# Pi identity (ancestry walk and holder liveness) is owned by
# the shared session-lock lib so every caller applies the exact
...
if [ "${1:-}" = "status" ]; then
  ...
  if fm_pi_pid_alive "$old"; then echo "lock: held by live Pi pid $old"; else echo "lock: stale (pid $old dead or not Pi)"; fi
  exit 0
fi

me=$(fm_pi_ancestry_pid) || { echo "error: cannot locate Pi process in ancestry" >&2; exit 1; }
...
if [ -e "$LOCK" ] || [ -L "$LOCK" ]; then
  ...
  if [ "$old" != "$me" ] && fm_pi_pid_alive "$old"; then
    echo "error: another live firstmate session holds the lock (pid $old); operate read-only until resolved" >&2
    exit 1
  fi
fi
