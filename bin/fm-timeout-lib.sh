...
# Provides one hard-bound runner so no caller has to re-derive the
# coreutils/BSD/perl selection, and so every bounded call in this repo agrees
# on what "the bound was hit" means.
#
#   fm_timeout_mechanism
#       Prints the mechanism fm_run_timed will use on this host: "timeout",
#       "gtimeout", "perl", or "bash".
#       Set FM_TIMEOUT_MECHANISM_OVERRIDE=bash to force the dependency-free
#       fallback.
#
#   fm_run_timed <seconds> <command> [args...]
#       Runs the command with a hard bound.
#       Exit status is the command's own, except 124, which means the bound was
#       hit (GNU timeout's convention, reproduced by the perl and bash
#       fallbacks).
#
...
# fetch spawned by a sweep) cannot outlive the bound.
# GNU/BSD `timeout` does this by default because it does not run the command in
# the foreground process group; the perl fallback does it explicitly with
# setpgrp plus a negative pid, and the bash fallback uses monitor mode to give
# the bounded child its own process group before signaling its negative pid.
set -u
