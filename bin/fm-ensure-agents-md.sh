#!/usr/bin/env bash
# Ensure a project worktree follows the AGENTS.md memory-file convention.
# Creates a minimal AGENTS.md skeleton when the file is absent and owns the
# canonical "## Maintaining this file" self-governance wording.
#
# Existing files receive that section idempotently unless they carry either the
# exact heading or this project-owned mark as their first line:
# <!-- firstmate:maintained-by-project -->
# The mark declares equivalent guidance is present, not permission to remove
# governance.
#
# A case-variant real memory file such as lowercase agents.md is refused so the
# convention remains portable across case-sensitive and case-insensitive filesystems.
# This is a worktree utility for workers, not a supervision script, so it does
  ...
  if [ "$entry" != "$AGENTS" ]; then
    case "$entry" in
      [Aa][Gg][Ee][Nn][Tt][Ss].[Mm][Dd])
        echo "conflict: memory file is named $entry in $DIR but the convention is AGENTS.md; rename it to AGENTS.md for portable discovery" >&2
        exit 1
        ;;
    esac
  fi
