  ...
  for child_meta in "$sub_state"/*.meta; do
    ...
    if [ -n "$child_t" ]; then
      if [ "$child_backend" = herdr ]; then
        ...
        echo "error: child $child_id has unsupported session provider '$child_backend'; explicit migration is required" >&2
        return 1
      fi
    fi
    if [ "$child_kind" = secondmate ]; then
...
# destructive sequence below (pipeline abort, process cleanup, worktree return,
# pane close, and record removal) runs under the named-session presentation
# lock. Exact live hierarchy is checked before any cleanup mutation; a proven
# dead pane remains eligible for cleanup.
