...
#             When omitted, the source is read from a Claude/Codex-shaped JSON
#             hook payload on stdin (the `source` field).
...
# exit 2 blocks session initialization, so a failed session start must reach
# the agent as digest text it can act on, never as a refusal to open the
# session.
...
if [ -z "$SOURCE" ] && [ ! -t 0 ]; then
  ...
  # Parsed without jq so a host missing it still gets correct routing rather
  # than silent full runs.
