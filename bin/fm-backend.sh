...
# through polling, while Herdr also exposes native transition push.
...
# fm_backend_detect: detect the runtime firstmate is currently executing inside.
# Nesting resolves innermost-first: tmux wins over Herdr.
fm_backend_detect() {
  FM_BACKEND_DETECTED=""
  if [ -n "${TMUX:-}" ]; then
...
# fm_backend_name: resolve the backend for a new spawn.
# Precedence: FM_BACKEND, config/backend, runtime auto-detection, then tmux.
fm_backend_name() {
...
# fm_backend_required_tools: backend-specific dependencies beyond the universal toolchain.
...
# boundaries keep runtime dispatch from importing both adapter ASTs into
...
# input guard or a submit acknowledgement. It is exposed so a caller other
# than the send path (the away-mode daemon's supervisor-pane pending-input
# guard in bin/fm-supervise-daemon.sh) can ask the same question
...
# the shared shape owner in bin/fm-composer-lib.sh.
