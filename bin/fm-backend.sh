...
# Pi's environment-marker detection layer). Prints the detected backend name
...
#   - the session-provider CLI itself (tmux or Herdr);
#   - jq for Herdr's JSON output;
#   - the treehouse worktree provider.
...
#                      (`window=`) - backend-independent, a stored value, NOT re-verified
...
# exposed so callers such as fm-spawn.sh's launch checks can ask the same question
...
#   alive      - an exact Pi agent is running.
...
# Coarse three-state projection for callers that need only live, dead, or
# unknown. An authoritatively missing endpoint is confidently not live, while
# every ambiguous, unreadable, or unverified result stays unknown.
