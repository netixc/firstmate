...
# Resolved once at source time because fm_pid_identity and fm_path_mtime run
# inside hot confirmation and attachment polls.
...
fm_pid_identity() {
  ...
  # On Linux, /proc stat field 22 (starttime, clock ticks since boot) is immune
  # to wall-clock steps that re-render the ps lstart fallback's date (observed
  # as WSL2 btime drift) and would evict a live watcher. Combining the full
  # NUL-separated cmdline keeps PID reuse a mismatch on a tick collision.
  if [ "$_FM_UNAME" = Linux ] && [ -r "$proc_root/$pid/stat" ] && [ -r "$proc_root/$pid/cmdline" ]; then
