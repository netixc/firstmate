...
family_for_basename() {
  case "$1" in
    ...
    fm-operational-input.test.sh|fm-pi-primary-tools.test.sh|fm-pi-primary-types.test.sh|\
...
# Measured portable-serial script durations in milliseconds from the timing
# evidence recorded in docs/fm-test-portable-shards.md. These are balance hints
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    README.md|VISION.md|LICENSE|assets/*|docs/*|.gitignore)
      ;;
    *)
      # A deleted path has no surviving implementation or direct consumer to
      # select. Other changed paths in the same deletion still select their
      # surviving contract families; a newly added or modified unknown path
      # remains a hard mapping error.
      if [ -e "$path" ]; then
