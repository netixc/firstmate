...
#      (generation state is sufficient for busy, not for idle), then the Cursor
#      transcript pull source, then the Grok-only temporary regex fallback
...
# armed and reads its rendered tail on demand in the classifier rather than
