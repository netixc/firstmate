...
#   endpoint-gone, herdr-native, grok-regex, agy-regex, missing,
...
#      Grok/AGY temporary regex fallbacks classify their own task from its
#      rendered tail, then unknown missing
#   5. malformed, stale, or untrusted records -> unknown, never a fallback
# Grok and AGY are the ONLY rendered-text classifications that survive the
# redesign, because neither structured lifecycle was credited-live-verified in
# the approved audit (agy 1.2.0 exposes no hook surface at all, see
# references/harness/agy.md); each is scoped to its own harness= and can never
# classify another adapter. The delivery
...
# the grok and agy arms; when absent each captures through
...
fm_busy_classify() {  # <backend> <target> <harness> <id> <state-dir> [tail40]
  ...
  case "$harness" in
    ...
    agy)
      ...
      # Best-effort: a long turn can scroll the busy marker out of
