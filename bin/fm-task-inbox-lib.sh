...
fm_task_inbox_ring() {  # <backend> <target> <record-path> [expected-label] [explicit-meta]
  local backend=$1 target=$2 rec=$3 label=${4:-} explicit_meta=${5:-} line cstate verdict
  line=$(fm_task_inbox_doorbell_line "$rec")
  cstate=$(fm_backend_composer_state "$backend" "$target" "$label" "$explicit_meta" 2>/dev/null) || cstate=unknown
  case "$cstate" in
    pending) return 1 ;;
  esac
  if ! verdict=$(fm_backend_send_text_submit "$backend" "$target" "$line" 1 0.4 0.3 "$label" "$explicit_meta" 2>/dev/null); then
    return 2
  fi
