#!/usr/bin/env bash
# Dismiss a pending Relay mention at the relay WITHOUT replying to it.
#
# Usage: fm-relay-dismiss.sh <request_id>
#
...
# (state/relay-context/<id>.json; a dismissed mention never gets a follow-up); on a
...
#
# Preview / dry-run: with FM_RELAY_DRY_RUN set (truthy), nothing is posted. Instead the
# would-be POST body ({request_id}) is recorded to state/relay-outbox/<request_id>.json
...
usage() {
  echo "usage: fm-relay-dismiss.sh <request_id>" >&2
}
...
case "$REQ" in
  ''|.*|*[!A-Za-z0-9._-]*) echo "fm-relay-dismiss: unsafe request_id: $REQ" >&2; exit 2 ;;
esac

command -v jq >/dev/null 2>&1 || { echo "fm-relay-dismiss: jq not found" >&2; exit 1; }
...
PAYLOAD=$(jq -cn --arg rid "$REQ" '{request_id:$rid}') || {
  echo "fm-relay-dismiss: failed to build request payload" >&2; exit 1; }
...
if [ -n "$FM_RELAY_DRY" ]; then
  ...
  OUTREC=$(printf '%s' "$PAYLOAD" | jq -c '. + {endpoint:"dismiss"}') || {
    echo "fm-relay-dismiss: failed to build dry-run outbox record" >&2; exit 1; }
  printf '%s\n' "$OUTREC" \
    | fm_relay_private_artifact_publish_stdin "$outbox_dir" "$REQ.json" 600 || {
    echo "fm-relay-dismiss: cannot write dry-run outbox: $outbox_dir/$REQ.json" >&2
    exit 1
  }
  ...
  fm_relay_context_registry_clear "$STATE" "$REQ"
  printf 'fm-relay-dismiss: DRY RUN - would POST to %s/connector/dismiss (recorded: state/relay-outbox/%s.json)\n' \
    "$FM_RELAY_BASE_URL" "$REQ" >&2
  printf '%s\n' "$REQ"
  exit 0
fi

if [ -z "$FM_RELAY_TOKEN" ]; then
  echo "fm-relay-dismiss: Relay not configured (no FMX_PAIRING_TOKEN)" >&2
  exit 1
fi
command -v curl >/dev/null 2>&1 || { echo "fm-relay-dismiss: curl not found" >&2; exit 1; }
AUTH_HEADER_FILE=$(fm_relay_auth_header_file) || {
  echo "fm-relay-dismiss: invalid FMX_PAIRING_TOKEN" >&2
  exit 1
}
...
code=$(curl -m 10 -s -o /dev/null -w '%{http_code}' \
  ...
  --data "$PAYLOAD" \
  "$FM_RELAY_BASE_URL/connector/dismiss" 2>/dev/null) || {
  echo "fm-relay-dismiss: request to relay failed" >&2
  exit 1
}
...
case "$code" in
  2[0-9][0-9])
    ...
    printf '%s\n' "$REQ"
    ;;
  *) echo "fm-relay-dismiss: relay returned HTTP $code" >&2; exit 1 ;;
esac
