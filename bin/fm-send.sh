...
# Herdr's composer/submit core is shared with the away-mode daemon.
...
# never reads. A crewmate/scout target, an explicit Herdr-target escape hatch,
...
fm_send_resolve_target() {  # <raw-target>
  ...
  if [ -n "$meta" ]; then
    RESOLUTION_TRIED="meta=$meta"
    target=$(fm_backend_target_of_meta "$meta") || return 1
    if [ -z "$target" ]; then
      echo "error: no Herdr target recorded in $meta (tried $RESOLUTION_TRIED)" >&2
      return 1
    fi
  ...
  case "$raw" in
    fm-*)
      RESOLUTION_TRIED="meta=$STATE/$raw.meta; legacy-meta=$STATE/${raw#fm-}.meta"
      echo "error: no metadata for $raw in $STATE (tried $RESOLUTION_TRIED); pass a well-formed explicit Herdr target only when targeting outside this Firstmate home" >&2
      return 1
      ;;
  esac
  ...
  if [ -n "$pane_meta" ]; then
    ...
    echo "error: target '$raw' matches herdr_pane_id in $pane_meta but is missing its Herdr session prefix; expected <herdr-session>:<pane-id> such as '$hint' or use 'fm-$id'" >&2
    return 1
  fi
  ...
  if [ -n "$meta" ]; then
    target=$(fm_backend_target_of_meta "$meta") || return 1
    if [ -z "$target" ]; then
      echo "error: no Herdr target recorded in $meta (tried explicit target '$raw' via recorded window metadata)" >&2
      return 1
    fi
    RESOLVED_TARGET=$target
    TARGET_META=$meta
    TARGET_HARNESS=$(fm_meta_get "$meta" harness)
    RESOLUTION_TRIED="explicit target '$raw' matched $meta"
    return 0
  fi

  case "$raw" in
    *:*)
      if ! fm_backend_target_exists "$raw"; then
        echo "error: explicit target '$raw' is not a live Herdr endpoint (tried meta=$STATE/$raw.meta and recorded window metadata). Use fm-<id> for a recorded task, or pass a verifiable <session>:<pane-id> target." >&2
        return 1
      fi
      RESOLVED_TARGET=$raw
      RESOLUTION_TRIED="meta=$STATE/$raw.meta; recorded window lookup; Herdr endpoint verified"
      return 0
      ;;
  esac

  echo "error: target '$raw' is not resolvable (tried meta=$STATE/$raw.meta and recorded window metadata). Use fm-$raw for a recorded task, or pass a well-formed <session>:<pane-id> target." >&2
  return 1
}
...
# meta; an explicit Herdr-target escape hatch has none, so its harness is
...
# Herdr readiness through its session-aware target-ready path. A failed send is
# still surfaced below as a hard
# error with the attempted resolution attached.

if [ "${1:-}" = "--key" ]; then
  if ! fm_backend_send_key "$T" "$2" "$EXPECTED_LABEL"; then
    echo "error: key '$2' not sent to $T (Herdr send failed; tried $RESOLUTION_TRIED)" >&2
    exit 1
  fi
else
  MESSAGE=$*
  ...
  # needlessly slow plain text to claude/opencode/pi. Herdr's verified submit
  # retry still backs the settle up either way.
  case "$*" in
    /*) settle=1.2 ;;
    \$*)
  ...
  if ! verdict=$(fm_backend_send_text_submit "$T" "$MESSAGE" "$retries" "$sleep_s" "$settle" "$EXPECTED_LABEL"); then
    if [ "$PENDING_REPLY_CREATED" = 1 ] && [ -n "$PENDING_REPLY_CORR" ]; then
      fm_pending_reply_discard_undelivered "$STATE" "$PENDING_REPLY_CORR" || true
    fi
    echo "error: text not sent to $T (Herdr send failed; tried $RESOLUTION_TRIED)" >&2
    exit 1
  fi
  case "$verdict" in
    pending)
