...
# submission - the caller sends Enter separately (fm-spawn.sh pauses between
# the literal send and Enter for Pi to settle).
...
fm_backend_tmux_classify_process_name() {  # <path> -> shell|other
  local path=$1 base
  base=${path##*/}
  base=${base#-}
  case "$base" in
...
#
# This is the foreground-process-group half of the liveness probe.
#
...
fm_backend_tmux_agent_state() {  # <target>
  ...
  if fm_tmux_foreground_pi_pid "$target" >/dev/null; then
    printf 'alive'
    return 0
  fi
