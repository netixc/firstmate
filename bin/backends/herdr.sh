#!/usr/bin/env bash
# bin/backends/herdr.sh - the Herdr task-workspace adapter.
#
...
# session provider only (D3): Treehouse remains the worktree provider.
# Sourced only through bin/fm-backend.sh's fm_backend_source in normal
# operation; unit tests source it directly, so the FM_HOME fallback
...
# default-on projection. Projected cleanup therefore serializes under the
# session lock, repositions a doomed workspace behind the focused one when
# needed, and ends its verified lone idle shell so Herdr removes the emptied
# workspace through the focus-preserving pane-death path, with the exact
# pre-close tab restore as the backstop and a refusal to close the active tab
# itself.
#
...
# exact task-id, fm-<id>, and explicit backend-target forms (that
...
# through fm_backend_required_tools as part of the required Herdr workspace layer;
...
# The version floor for default-on presentation projection.
# Projection turns every crewmate cleanup into a workspace-emptying removal,
# and the focus-safe removal plan can only avoid Herdr's focus-stealing explicit
# close while the doomed pane holds a provably lone idle childless shell.
# A persistent child of that shell makes the plan fall back to the plain explicit
# close, which steals focus on releases without the upstream focus fixes.
# Herdr 0.8.0 is the first release carrying both fixes, so a home that configured
# nothing is projected only at or above it.
# An explicit "on" remains honored below the floor.
# Protocol 19 is the structural signal for that floor, measured against the
# real release binaries and recorded in docs/verification/runtime-backends.md.
FM_BACKEND_HERDR_MIN_PRESENTATION_PROTOCOL=19
FM_BACKEND_HERDR_MIN_PRESENTATION_VERSION=0.8.0
# One-warning-per-release dedupe marker prefix, under the state dir.
# The projection decision is remade on every spawn, so an undeduplicated
# below-floor warning would repeat on every crewmate.
# The key is the detected release, so an upgrade or a downgrade is announced
# again.
...
# Echoes exactly one of "off", "on" (a deliberate opt-in, honored even below
# the version floor), or "default" (this home configured nothing, so the floor
# decides).
# Values are read with the whole-file whitespace-stripped convention the other
# scalar config items already use (config/backlog-backend, config/crew-profile),
# plus case folding.
# An empty file is the historical presence-based opt-in form and still means an
# explicit "on", so no home that deliberately enabled the projection can lose
# it.
# An unrecognized value warns and falls back to the default rather than failing
# a spawn over a purely visual setting, so a typo is visible instead of silently
# deciding anything.
...
# comparison.
# Return codes: 0 candidate >= floor, 1 candidate < floor, 2 the candidate is
# unparseable.
# Any prerelease or build suffix is stripped first, so a 0.8.0-preview build
# compares as 0.8.0 while a 0.7.5-preview build compares as 0.7.5.
...
# classifier for the presentation version floor.
# Return codes: 0 at or above the floor, 1 provably below it, 2 indeterminate.
# Two independent signals are read so no single field is load-bearing, and either
# one can carry a positive verdict: the protocol number, which is the structural
# signal this adapter already uses for every other capability gate, and the
# release core of the version string.
# A signal that is unreadable or unparseable simply cannot carry a verdict; a
# readable protocol below the floor is decisive on its own, and only losing both
# signals reports indeterminate.
...
# running server.
# A running server and client compose conservatively: both must be supported.
# When status positively reports no running server, only the client that will
# start it is applicable.
# Same return codes as fm_backend_herdr_release_floor_verdict, and sets
# FM_BACKEND_HERDR_PRESENTATION_RELEASE to the identifier a caller's warning
# names.
# An unreadable server-running state is indeterminate rather than permission to
...
# usable state dir is given.
# Without one the warning is emitted every call, which is what a one-shot caller
# wants.
...
# compose the applicable release verdict and the shared warning contract for one
# unconfigured home.
fm_backend_herdr_presentation_default_supported() {  # <state-dir> [<session>]
...
# bin/fm-spawn.sh consults before projecting this home's children into disposable
# one-task workspaces (docs/herdr-backend.md "Presentation spaces" owns the full
# contract).
# An explicit "off" or "on" is obeyed as written; a home that configured nothing
# is projected only at or above the version floor, and otherwise falls back to
# the flat layout with one warning.
# Sets FM_BACKEND_HERDR_PRESENTATION_PREFERENCE for the new-projection boundary
# to distinguish an unconfigured default from an explicit opt-in.
...
fm_backend_herdr_tool_check() {
  command -v herdr >/dev/null 2>&1 || { echo "error: the Herdr workspace layer requires the 'herdr' CLI (https://herdr.dev) (dual-licensed AGPL-3.0-or-later/commercial)" >&2; return 1; }
  command -v jq >/dev/null 2>&1 || { echo "error: the Herdr workspace layer requires 'jq' to parse JSON output" >&2; return 1; }
  return 0
}
...
# fm_backend_herdr_session: resolve which named Herdr session a normal
# spawn or operation uses. An operator or isolated test harness may set
# HERDR_SESSION explicitly; absent means Herdr's own
...
# Both fixes first shipped in Herdr 0.8.0 (protocol 19).
...
# other workspace's relative order, so no presentation ordering change persists.
# That reasoning covers the pane-death route only. The plan's plain-close
# fallback is reachable when the doomed pane's shell cannot be proved lone,
# childless, and idle, and on a release without both fixes that fallback steals
# focus. Default-on projection is therefore floored at
# FM_BACKEND_HERDR_MIN_PRESENTATION_VERSION, where every removal primitive
# preserves focus. The spawn-time gate is the floor's sole owner; session-start
# cleanup retires a leftover projection on every release so it never strands a
# workspace.
...
# headless (no TUI client) if it is not already running.
# A bare socket CLI call does not auto-start the server, so this must run before
# any workspace, tab, or pane call. Bounded poll for the server to report running.
...
#       so there is no launcher workspace to inherit and the caller uses its
#       per-home container. HERDR_ENV=1 on its own is never a parent binding;
#       Herdr always injects the pane id alongside it.
...
# sweep. It reuses the husk classifier rather than creating a second Herdr
# state machine: a structurally gone pane is `missing`,
...
# uniqueness itself, so Firstmate performs the duplicate check.
#
...
# fm_backend_herdr_send_text_line: send and submit one line of text atomically.
# Used for fixed spawn-time commands such as treehouse acquisition and
# environment exports. `pane run` types and submits the command in one call.
...
# The caller sends Enter separately because `pane send-text` does not submit.
...
# fm_backend_herdr_send_key: send one named special key for fm-send.sh --key.
...
# fm_backend_herdr_capture: bounded plain-text pane capture.
#
...
# fm_backend_herdr_composer_state: classify a Pi composer's own row as
# empty, pending, or unknown from a generous tail-window capture.
# Herdr has no cursor-row primitive, so the adapter accepts a bordered composer
# or Pi's native-identity-bound separator region and keeps the bottom-most match.
# A missing identity, a working agent, a malformed region, or a shell prompt is
# unknown rather than injectable.
# The shared ANSI extractor removes de-emphasized placeholder text before the
# final content verdict.
FM_BACKEND_HERDR_COMPOSER_LINES=${FM_BACKEND_HERDR_COMPOSER_LINES:-20}
# Pi's structural separator region is authoritative for unbordered composers.
# A bare prompt glyph is not accepted as an independent composer shape.
# A bordered composer can also render the generic idle ghost text below; it is
# not user input and must not block a safe Pi-directed message.
FM_BACKEND_HERDR_IDLE_RE=${FM_BACKEND_HERDR_IDLE_RE:-'^Type a message\.\.\.$'}
# Pi's historical unbordered composer uses this glyph; shell prompts remain
# unsafe because this exact Pi-only shape is the only accepted bare candidate.
...
EOF
    case "$agent:$agent_status" in
      ...
      pi:*|:*)
        ...
        found=0
        ;;
      *) found=0 ;; # A non-Pi agent is never an accepted Firstmate endpoint.
    esac
  ...
  # Content extraction drops dim and dark-truecolor placeholder runs.
  # Shape detection stays on the plain row because styling can also affect borders.
  ...
  # Delegate the final empty, pending, or unknown decision to the shared owner.
...
# completion popup within about 0.1 seconds, so the caller's <settle> before
# the first Enter matters.
#
...
# semantic signal regardless of what text Pi's idle composer happens to
# display.
#
# Incident (2026-07-07, followed up on 2026-07-08): a redelivery loop in the
# away-mode daemon showed that composer-content submit confirmation was too
# sensitive to rendering details. The ANSI-aware classifier remains the
# pre-injection guard, while idle-baseline submit confirmation deliberately
# stays on native agent state so delivery does not depend on composer text.
# Composer content is retained for the away-mode daemon's pre-injection
# empty-box guard and for submit attempts whose pre-Enter agent-state baseline
# is not legibly idle.
#
...
#     are packed into the budget. Pi verification observed first-working inside
#     a several-hundred-ms, multiply-sampled window, so this has not been
#     observed in practice. On the (unobserved)
...
# fm_backend_herdr_kill: remove the task's pane best-effort.
# Closing a tab's only pane closes the tab too, so a separate tab close is unnecessary.
...
# agent-state API. See fm_backend_herdr_classify_agent_status for the
# status-to-busy/idle/unknown mapping.
...
#             whatever the composer's own text happens to show. Returned the
#             instant it is seen, without waiting out the
#             rest of the budget.
...
# confirmation"): Pi verification observed first-working inside the repeated
# several-hundred-ms budget. The remaining, inherent gap - a turn so fast it starts AND
...
# fm_backend_herdr_resolve_bare_selector: resolve an ad hoc selector with no
# metadata by searching every running named Herdr session for a tab whose label
# matches <name>. This is a rare, best-effort path because tasks normally carry
# exact endpoint metadata.
