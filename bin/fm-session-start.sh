...
# session behind one hung subprocess.
# Not every step is individually bounded: bootstrap's fleet sync is, but its
# `gh auth status` probe, tool version probes, secondmate liveness reads, the
# backlog listing, and the per-task endpoint reads are not.
# The whole digest therefore runs as ONE bounded child of this script
# (FM_SESSION_START_TIMEOUT, default 120s).
# The child writes the digest straight to this script's stdout, so everything it
...
# The child records its progress in FM_SESSION_START_STAGE_FILE, which is also
# the flag that tells a child it is the child - the parent never recurses.
...
#             mutating sweeps that startup already reconciled - stale Herdr
#             projection cleanup and bootstrap's six mutating sweeps - and
#             re-emit the rest. The wake-queue drain is NOT skipped: queued
#             records are this turn's work queue, and a session that owns the
#             lock is exactly the session that must take them. Lock acquisition
#             still runs, because ownership must be re-verified rather than
#             assumed.
set -u
...
if [ "$REEMIT" -eq 1 ]; then
  ...
  printf 'retry, X-mode artifacts, and stale Herdr child cleanup - are NOT repeated.\n'
