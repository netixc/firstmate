...
cmd_launch() {
  ...
  validate_id "$id"
  validate_home "$id"
  [ "$harness" = pi ] || die "unsupported remote secondmate harness '$harness'; migrate the record explicitly to plain pi"
...
cmd_observe() {
  ...
  remote_endpoint_require "$id"
  harness=$(fm_meta_get "$REMOTE_ENDPOINT_META" harness)
  fm_pending_reply_backend_observation "$REMOTE_ENDPOINT_BACKEND" "$REMOTE_ENDPOINT_TARGET" "fm-$id" "$harness" "$REMOTE_ENDPOINT_META"
  printf '\n'
}
