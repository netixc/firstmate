...
#          rather than because it has no lock at all.
#          The two cases differ in exactly one place: repair ownership.
#          A locked session is told to restore a tangled primary checkout itself,
#          while an unlocked one is told to leave that work to the lock holder.
#          Unset/0 (the default) keeps detect-only meaning unlocked, exactly as
#          before.
...
secondmate_liveness_sweep() {
  ...
  # fm_backend_agent_state. A missing pane is not enough: the adapter must prove
  # the endpoint absent. This preserves duplicate prevention for
...
# Source this before the active Pi protocol starts a watcher process so
