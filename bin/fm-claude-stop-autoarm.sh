...
#   - Need: arms only while work is in flight (state/*.meta) or Relay has a
#     relay poll to run (state/relay-watch.check.sh); an idle home exits 0.
...
# --- need: in-flight work or a Relay poll ----------------------------
need_supervision() {
...
# Relay cadence: source the generated config so a Relay-enabled instance polls at its
# 30s cadence (fm-bootstrap.sh relay_setup contract).
...
# The need may have vanished mid-cycle (fleet torn down, Relay opted out): nothing
