...
'3. Launch or attach the full outer Herdr TUI only through `"$HERDR_LAB_HELPER" client "$HERDR_LAB_SESSION"`.' \
'   Direct full-client `herdr` invocation is forbidden; the helper freshly verifies the non-default lab and constructs the session selection instead of trusting ambient `HERDR_SESSION`.' \
'4. Teardown only through `"$HERDR_LAB_HELPER" teardown "$HERDR_LAB_SESSION"`.' \
'   It re-checks refuse-default immediately before stop and again immediately before delete, and fails closed on ambiguity.' \
'5. If an experiment requires a deliberate mid-run session stop, use only `"$HERDR_LAB_HELPER" stop "$HERDR_LAB_SESSION"`; it performs the same immediate refuse-default check.' \
'6. Forbidden commands: direct `herdr server stop`, every other server-global operation such as `herdr server live-handoff` or reload/update operations, direct `herdr session stop`, direct `herdr session delete`, and any Herdr call scoped only by ambient or inline `HERDR_SESSION`.' \
