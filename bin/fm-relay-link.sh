#!/usr/bin/env bash
# Link a spawned task to the Relay mention that triggered it, so firstmate can
# post up to THREE completion follow-ups when the task lands (within a 7-day window).
#
# Usage: fm-relay-link.sh <task-id> <request_id> [--carry-count <n> --carry-ts <epoch> [--carry-platform discord] [--carry-max <n>]]
#
...
#   relay_request_ts=<epoch>       link time, for the 7-day follow-up window
#   relay_followups=<n>            follow-ups already posted against this binding
#   relay_platform=<platform>      target platform, when known
#   relay_reply_max_chars=<n>      target split budget, when known
#
# A fresh link always starts relay_followups at 0 and uses the current time for
# relay_request_ts. --carry-count <n> and --carry-ts <epoch> are a required pair for
...
# relay_followups and relay_request_ts before its meta goes away and passes both here,
...
# task's relay_platform and relay_reply_max_chars when the original inbox file is gone.
#
...
#
# This is a separate step the relay-respond skill runs AFTER fm-spawn.sh, so it
...
# fm-relay-followup.sh on the task's captain-relevant wakes. The meta read/write
# lives in fm-relay-lib.sh.
#
...
usage() {
  echo "usage: fm-relay-link.sh <task-id> <request_id> [--carry-count <n> --carry-ts <epoch> [--carry-platform discord] [--carry-max <n>]]" >&2
}
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    --carry-count)
      ...
      CARRY_COUNT=${1:-}
      case "$CARRY_COUNT" in
        ''|*[!0-9]*) echo "fm-relay-link: --carry-count needs a non-negative integer" >&2; exit 2 ;;
      esac
      ;;
    --carry-ts)
      shift
      CARRY_TS=${1:-}
      case "$CARRY_TS" in
        ''|*[!0-9]*) echo "fm-relay-link: --carry-ts needs a non-negative epoch integer" >&2; exit 2 ;;
      esac
      ;;
    --carry-platform)
      shift
      ...
      case "$CARRY_PLATFORM" in
        discord) ;;
        *) echo "fm-relay-link: --carry-platform needs discord" >&2; exit 2 ;;
      esac
      ;;
    --carry-max)
      shift
      CARRY_MAX=${1:-}
      case "$CARRY_MAX" in
        ''|*[!0-9]*) echo "fm-relay-link: --carry-max needs an integer of at least 50" >&2; exit 2 ;;
        *) [ "$CARRY_MAX" -ge 50 ] 2>/dev/null || { echo "fm-relay-link: --carry-max needs an integer of at least 50" >&2; exit 2; } ;;
      esac
      ;;
    *) usage; exit 2 ;;
  esac
...
if [ -n "$CARRY_COUNT" ] && [ -z "$CARRY_TS" ]; then
  echo "fm-relay-link: --carry-count requires --carry-ts to preserve the original follow-up window" >&2
  exit 2
fi
if [ -n "$CARRY_TS" ] && [ -z "$CARRY_COUNT" ]; then
  echo "fm-relay-link: --carry-ts requires --carry-count to preserve the consumed follow-up count" >&2
  exit 2
fi
if { [ -n "$CARRY_PLATFORM" ] || [ -n "$CARRY_MAX" ]; } && { [ -z "$CARRY_COUNT" ] || [ -z "$CARRY_TS" ]; }; then
  echo "fm-relay-link: --carry-platform and --carry-max require --carry-count and --carry-ts" >&2
  exit 2
fi
...
fm_pr_task_id_valid "$ID" || { echo "fm-relay-link: unsafe task id: $ID" >&2; exit 2; }
case "$RID" in
  ''|.*|*[!A-Za-z0-9._-]*) echo "fm-relay-link: unsafe request_id: $RID" >&2; exit 2 ;;
esac
...
META="$STATE/$ID.meta"
if [ ! -f "$META" ]; then
  echo "fm-relay-link: no such task: state/$ID.meta" >&2
  scan_secondmate_routes "$ID"
  if [ -n "$ROUTE_MATCHES" ]; then
    printf 'fm-relay-link: %s is a second mate task (found in: %s), so this home cannot link it - a link only binds work whose record lives here.\n' \
      "$ID" "$ROUTE_MATCHES" >&2
  elif [ "$ROUTE_REGISTERED" -eq 1 ]; then
    printf 'fm-relay-link: this home has registered second mates and no record of %s, so the work may be routed to one - a link only binds work whose record lives here.\n' \
      "$ID" >&2
  fi
...
if [ -z "$CARRY_TS" ]; then
  REPLY_CONTEXT=$(fm_relay_resolve_reply_context "$STATE" "$RID" 1) || {
    echo "fm-relay-link: failed to resolve request reply context" >&2
    exit 1
  }
...
if [ -n "$CARRY_TS" ] && { [ -z "$REQ_PLATFORM" ] || [ -z "$REQ_REPLY_MAX" ]; }; then
  echo "fm-relay-link: relink requires carried reply context; pass --carry-platform and --carry-max from the prior task" >&2
  exit 2
fi

if [ -z "$CARRY_TS" ] && { [ -z "$REQ_PLATFORM" ] || [ -z "$REQ_REPLY_MAX" ]; }; then
  echo "fm-relay-link: WARNING: incomplete authoritative reply context for request $RID; every completion follow-up will be HELD until both platform and explicit budget can be resolved. Ensure the relay request-context lookup supplies both values." >&2
fi
...
if [ -n "$CARRY_TS" ]; then
  ...
  # FM_RELAY_NOW_OVERRIDE keeps tests deterministic; production uses the wall clock.
  LINK_TS=${FM_RELAY_NOW_OVERRIDE:-$(date +%s)}
  case "$LINK_TS" in
    ''|*[!0-9]*) echo "fm-relay-link: could not read the current time" >&2; exit 1 ;;
  esac
fi

if ! fm_relay_meta_link_set "$META" "$RID" "$LINK_TS" "$FOLLOWUPS" "$REQ_PLATFORM" "$REQ_REPLY_MAX"; then
  echo "fm-relay-link: failed to record the link in state/$ID.meta" >&2
  exit 1
fi

printf 'linked %s to Relay request %s\n' "$ID" "$RID"
