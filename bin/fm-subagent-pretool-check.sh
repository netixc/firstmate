#!/usr/bin/env bash
# Plain Pi built-in delegation guard. Pi passes --tool with the exact tool name.
# Exit 0 allows; exit 2 with a stderr reason blocks.
set -u
...
usage() {
  cat <<'EOF'
Usage: fm-subagent-pretool-check.sh [--tool <tool-name>]

With no --tool, reads Pi's tool-call JSON payload on stdin (`tool_name`).
