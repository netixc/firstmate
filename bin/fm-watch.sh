...
# production owner: the shared wake queue, the wake classifier the away-mode
# daemon also uses, the Herdr endpoint operations, and the normalized-transition
# accessors. The Herdr event-wait smoke test consumes this same owner without
# sourcing the entire watcher graph.
#
# The DEFAULT EVENT SOURCE remains this watcher's poll loop over the pull
# primitives (capture, recorded windows, Herdr busy-state, and the BUSY_REGEX
# fallback), which synthesizes the signal/stale/check/heartbeat wake vocabulary.
# Herdr also replaces this watcher's blind terminal sleep with a bounded wait on
# its native event stream (event_wait_or_sleep below), so a crew entering
# `blocked` wakes its supervisor sub-second; the poll loop stays live every cycle
# as the permanent backstop. See bin/fm-backend.sh and docs/herdr-backend.md.
