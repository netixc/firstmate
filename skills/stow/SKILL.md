...
5. **Write only into locations that already exist as a real convention, the step-3 fallback (plus its `.gitignore` line), or a destination the user just approved in step 4.**
   ...
   Never store, create, or edit a skill as a destination for a finding: there is no "graduate this to a skill" move, even in a repo whose existing `.agents/skills/` or `skills/` directory makes one look like a convention.
...
8. **Finish with an honest safe-to-end verdict and a resume pointer for the next session.**
   ...
   The real payoff of stowing is not this session but the next one: close with a short, copy-pasteable RESUME POINTER naming exactly which files a fresh session should load to pick this back up cold, e.g. `To pick this back up in a new session, load: AGENTS.md (project conventions), .stow-notes.md (private notes, not shared)`.
