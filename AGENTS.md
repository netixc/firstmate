...
The verified harnesses are `codex`, `opencode`, `pi`, `pi-signed`, `grok`, and `kimi`, plus `muse`, `gemini`, `rovo`, and `agy` for crewmates and scouts only; never dispatch on an unverified adapter.
