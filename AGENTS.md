...
config/backend  optional runtime session-provider assertion for new tasks; LOCAL, gitignored; absent or exact "herdr" selects the sole Herdr provider, while every other value is rejected without detection or fallback; inherited by secondmate homes under the primary-authoritative contract in secondmate-provisioning
...
Dispatch only through Herdr; an explicit per-spawn `--backend` may assert `herdr` but no other value is accepted ([`docs/configuration.md`](docs/configuration.md) "Runtime backend").
Missing, incompatible, unauthenticated, ambiguous, or unreachable Herdr is a blocker; never create another provider endpoint.
