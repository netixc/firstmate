...
config/backend  runtime session-provider backend request for new tasks; LOCAL, gitignored; absent or `herdr` = Herdr, while `tmux` is refused for fresh work and retained only for explicit existing rollback records; see docs/configuration.md "Runtime backend"; inherited by secondmate homes under the primary-authoritative contract in secondmate-provisioning
