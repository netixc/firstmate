...
config/secondmate-harness  global PRIMARY fallback for SECONDMATE harness/model/effort resolution ("<harness> [<model>] [<effort>]"; section 4); LOCAL, gitignored; absent or "default" harness falls back to config/crew-harness then firstmate's own. The primary's own setting; NOT inherited into secondmate homes (secondmates do not spawn secondmates)
config/secondmate-profiles/  per-secondmate PRIMARY launch profiles, one strict `<id>` file overriding the global fallback; LOCAL, gitignored, and NOT inherited. `docs/configuration.md` owns the schema and `bin/fm-harness.sh` owns parsing
