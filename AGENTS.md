...
Once a Firstmate-managed delivery path is identified as GitHub, capture `FM_GITHUB_REPOSITORY=$(bin/fm-github-owner-policy.sh <project-dir>)` before any outward mutation and refresh it immediately before every subsequent `no-mistakes axi run` or native creation; proceed only on success, run native `gh-axi pr create` or `gh-axi issue create` with `GH_HOST=github.com GH_REPO="$FM_GITHUB_REPOSITORY"` and no `-R`, `--repo`, or `--hostname` override, leave `GH_HOST` and `GH_REPO` unset for no-mistakes, preserve GitLab delivery unchanged, and treat that script as the single owner of the personal-edition repository policy.
