...
state/               runtime records and signals; gitignored
  ...
  .afk-mode.lock  short-lived serialization for away/quiet mode flag writes; never touch
...
An `UNSUPPORTED_HOST:` line ends startup before fleet mutation; load that skill and do not dispatch or install on the refused host.
...
The verified harnesses are `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, and `omp`, plus `muse`, `gemini`, `rovo`, and `agy` for crewmates and scouts only; never dispatch on an unverified adapter.
