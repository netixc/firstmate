...
Once a Firstmate-managed delivery path is identified as GitHub, run `bin/fm-github-owner-policy.sh <project-dir>` before any outward mutation and again immediately before every subsequent `no-mistakes axi run` or native `gh-axi pr create` or `gh-axi issue create`, proceeding only when it confirms the canonical GitHub owner; GitLab delivery remains unchanged, and that script is the single owner of the personal-edition repository policy.
